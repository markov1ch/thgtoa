...
Version 0.9.0 (draft), April 2021 by AnonymousPlanet.
...
This guide is a non-profit open-source initiative, licensed under Creative Commons Attribution 4.0 International ([cc-by-4.0][] <sup>[[Archive.org]][22]</sup>).

Find it online at:

-   Original: [https://anonymousplanet.org][] <sup>[[Archive.org]][23]</sup> <sup>[[Archive.today]][24]</sup>

-   Mirror: <https://mirror.anonymousplanet.org> <sup>[[Archive.org]][25]</sup> <sup>[[Archive.today]][26]</sup>
...
-   Light Theme: <https://anonymousplanet.org/guide.pdf> <sup>[[Mirror]][27]</sup> <sup>[[Archive.org]][28]</sup> <sup>[[Tor Mirror]][29]</sup>

-   Dark Theme: <https://anonymousplanet.org/guide-dark.pdf> <sup>[[Mirror]][30]</sup> <sup>[[Archive.org]][31]</sup> <sup>[[Tor Mirror]][32]</sup>
...
-   All external links to YouTube Videos have an [Invidious] link next to them for accessing content through an Invidious Instance (in this case yewtu.be hosted in the NL) for increased privacy. See <https://github.com/iv-org/invidious> <sup>[[Archive.org]][33]</sup> for more information.
...
The EFF provides a few security scenarios of what you should consider depending on your activity. While some of those tips might not be within the scope of this guide (more about Privacy than Anonymity), they are still worth reading as examples. See <https://ssd.eff.org/en/module-categories/security-scenarios> <sup>[[Archive.org]][37]</sup>.

There are also quite a few more serious ways of making your threat model such as:

-   LINDDUN <https://www.linddun.org/> <sup>[[Archive.org]][38]</sup>

-   STRIDE <https://en.wikipedia.org/wiki/STRIDE_%28security%29> <sup>[[Archive.org]][39]</sup>

-   DREAD <https://en.wikipedia.org/wiki/DREAD_%28risk_assessment_model%29> <sup>[[Archive.org]][40]</sup>

-   PASTA <https://versprite.com/tag/pasta-threat-modeling/> <sup>[[Archive.org]][41]</sup>

You can find some introduction on these on these projects:

-   OWASP <https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html> <sup>[[Archive.org]][42]</sup>

-   Online Operations Security <https://github.com/devbret/online-opsec/> <sup>[[Archive.org]][43]</sup>
...
You might think this guide has no legitimate use but there are many[^16]'[^17]'[^18]'[^19]'[^20]'[^21] such as:
...
* **Note that if you are having trouble accessing any of the many academic article referenced in this guide, feel free to use Sci-Hub (<https://en.wikipedia.org/wiki/Sci-Hub>** <sup>[[Archive.org]][44]</sup>**) for reading them. Because science should be free. All of it.**
...
**Use at your own risk. Anything in here is not legal advice and you should verify compliance with your local law before use (IANAL**[^22]**). "Trust but verify"**[^23] **all the information yourself (or even better, "Never Trust, always verify"**[^316]**).**
...
-   Using the Tor Anonymity Network[^26] (free).

-   Using VPN[^27] services anonymously (anonymously paid with cash or Monero).

All those will be explained later in this guide.

## Your DNS requests:

DNS stands for "Domain Name System"[^28] and is a service used by your browser (and other apps) to find the IP addresses of a service. It is pretty much a huge "contact list" (phone book for older people) that works like asking it a name and it returns the number to call. Except it returns an IP instead.
...
Usually, the DNS service is provided by your ISP and automatically configured by the network you are connecting to. This DNS service could also be subject to data retention regulations or will just keep logs for other reasons (data collection for advertising purposes for instance). Therefore, this ISP will be capable of telling everything you did online just by looking at those logs which can in turn be provided to an adversary. Conveniently this also the easiest way for many adversaries to apply censoring or parental control by using DNS blocking[^29]. The provided DNS servers will give you a different address (than their real one) for some websites (like redirecting thepiratebay to some government website). Such blocking is widely applied worldwide for certain sites[^30].
...
Because it is not encrypted, your ISP and/or any other adversary could still intercept (using a Man-in-the-middle attack[^61]) your request will know and possibly log what your IP was looking for. The same ISP can also tamper with the DNS responses even if you are using a private DNS. Rendering the use of a private DNS service useless.

As a bonus, many devices and apps will use hardcoded DNS servers bypassing any system setting you could set. This is for example the case with most (70%) Smart TVs and a large part (46%) of Game Consoles[^31]. For these devices, you will have to force them[^32] to stop using their hardcoded DNS service which could make them stop working properly.

A solution to this is to use encrypted DNS using DoH (DNS over HTTPS[^33]), DoT (DNS over TLS[^34]) with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^35], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some middle-man from snooping on your requests ... except it might not.
...
Unfortunately, the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave/Ungoogled-Chromium among them) will leak the DNS again through SNI[^36] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> <sup>[[Archive.org]][48]</sup> ). **As of the writing of this guide**, **only Firefox based browsers supports eSNI (encrypted SNI**[^37] **soon to be renamed ECH**[^38] **and soon to be supported by Firefox too**[^39]**) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to hide your DNS requests from a third party**[^40]**.** And this option is not enabled by default either so you will have to enable it yourself.

In addition to limited browser support, only Web Services and CDNs[^41] behind Cloudflare CDN support eSNI at this stage (and will support ECH in the future[^42]). This means that eSNI (or soon ECH) is not supported (as of January 2021) by many mainstream platforms such as:
...
Some countries like Russia[^43] and China[^44] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you will not be able to establish an HTTPS connection with a service if you do not allow them to see what it was.

Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^45] have shown it is still possible to reliably fingerprint and block unwanted requests. Only DNS over Tor was able to demonstrate efficient DNS Privacy in recent studies but even that can still be defeated by other means (see [Your Anonymized Tor/VPN traffic:]).

One could also decide to use a Tor Hidden DNS Service or ODoH (Oblivious DNS over HTTPS[^46]) to further increase privacy/anonymity but **unfortunately**, as far as I know, these methods are only provided by Cloudflare as of this writing (<https://blog.cloudflare.com/welcome-hidden-resolver/> <sup>[[Archive.org]][49]</sup>, <https://blog.cloudflare.com/oblivious-dns/> <sup>[[Archive.org]][50]</sup>). I **[personally]{.ul}** think these are viable and reasonably secure technical options but there is also a moral choice if you want to use Cloudflare or not (despite the risk posed by some researchers[^47]).

Lastly, there is also this new option called DoHoT which stands for DNS over HTTPS over Tor which could also further increase your privacy/anonymity and which you could consider if you are more skilled with Linux. See <https://github.com/alecmuffett/dohot> <sup>[[Archive.org]][51]</sup>. This guide will not help you with this one at this stage but it might be coming soon.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^56].

-   Government agencies around the world interested in your phone number can and do use[^57] special devices called "IMSI catchers"[^58] like the Stingray[^59] or more recently the Nyxcell[^60]. These devices can impersonate (to spoof) a cell phone Antenna and force a specific IMSI (your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^61] (Man-In-The-Middle Attacks) that will allow them to:
...
Here is also a good YouTube video on this topic: DEFCON Safe Mode - Cooper Quintin - Detecting Fake 4G Base Stations in Real Time <https://www.youtube.com/watch?v=siCk4pGGcqA> <sup>[[Invidious]][54]</sup>
...
Some commercial devices will keep record of MAC addresses roaming around for various purposes such as road congestion[^64].
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^65].
...
-   Android/Google:
    ...
    -   Just have a read at their privacy policy <https://policies.google.com/privacy> <sup>[[Archive.org]][55]</sup>

    -   School of Computer Science & Statistics, Trinity College Dublin, Ireland Mobile Handset Privacy: Measuring The Data iOS and Android Send to Apple And Google <https://www.scss.tcd.ie/doug.leith/apple_google.pdf> <sup>[[Archive.org]][56]</sup>

-   IOS/Apple:

    -   More information at <https://www.apple.com/legal/privacy/en-ww/> <sup>[[Archive.org]][57]</sup> and <https://support.apple.com/en-us/HT202100> <sup>[[Archive.org]][58]</sup>

    -   School of Computer Science & Statistics, Trinity College Dublin, Ireland Mobile Handset Privacy: Measuring The Data iOS and Android Send to Apple And Google <https://www.scss.tcd.ie/doug.leith/apple_google.pdf> <sup>[[Archive.org]][56]</sup>

    -   Apple does claim[^66] that they anonymize this data using differential privacy[^67] but you will have to trust them on that.

-   Windows/Microsoft:

    -   Full list of required diagnostic data: <https://docs.microsoft.com/en-us/windows/privacy/required-windows-diagnostic-data-events-and-fields-2004> <sup>[[Archive.org]][59]</sup>

    -   Full list of optional diagnostic data: <https://docs.microsoft.com/en-us/windows/privacy/windows-diagnostic-data> <sup>[[Archive.org]][60]</sup>

-   MacOS:

    -   More details on <https://support.apple.com/guide/mac-help/share-analytics-information-mac-apple-mh27990/mac> <sup>[[Archive.org]][61]</sup>

-   Ubuntu:

    -   Ubuntu despite being a Linux distribution also collects Telemetry Data nowadays. This data however is quite limited compared to the others. More details on <https://ubuntu.com/desktop/statistics> <sup>[[Archive.org]][62]</sup>
...
But that is not what all those Wi-Fis access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it is possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^70] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> <sup>[[Archive.org]][63]</sup> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE> <sup>[[Invidious]][64]</sup>
...
Your metadata is all the information about your activities without the actual content of those activities. For instance, it is like knowing you had a call from an oncologist before then calling your family and friends successively. You do not know what was said during the conversation but you can guess what it was just from the "metadata"[^71].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^72]/IOS), Browsers, Apps, Websites. Odds are there are several companies knowing exactly where you are at any time[^73] because of your smartphone[^74].

This location data has been used in many judicial cases[^75] already as part of "geofence warrants" [^76] that allows law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time. In addition, this location data is even sold by private companies to the military who can then use it conveniently[^77].
...
The question is: Is there someone somewhere that would possibly have both pieces of information available[^78] for correlation in a convenient database?

Have you heard of Edward Snowden[^79]? Now is the time to google him and read his book[^80]. I recommend reading about XKEYSCORE[^81]'[^82], MUSCULAR[^83] and PRISM[^84].

See "We kill people based on Metadata"[^85] or this famous tweet from the IDF <https://twitter.com/idf/status/1125066395010699264> <sup>[[Archive.org]][66]</sup>.
...
Data is being transmitted even if you opt-out[^86], processed, and stored indefinitely (most likely unencrypted[^87]) by various third parties[^88].
...
-   Your Fitness Devices and Apps[^89]? (Strava[^90]'[^91], Fitbit[^92], Garmin, Polar[^93], ...)

-   Your Smart Speaker? (Amazon Alexa[^94], Google Echo, Apple Homepod ...)

-   Your Smart Transportation? (Car? Scooter?)

-   Any other Smart device? There is even a convenient search engine dedicated to finding them online: <https://www.shodan.io/> <sup>[[Archive.org]][67]</sup>
...
-   iPhones and iPads (IOS 13 and above)[^95]'[^96]

-   Samsung Phones (Android 10 and above)[^97]

-   MacBooks (MacOS 10.15 and above)[^98]

Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^99]. They do not have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^100]. They are basically using peer-to-peer short-range Bluetooth communication to broadcast their status through nearby online devices.
...
RFID stands for Radio-frequency identification[^101], it is the technology used for instance for contactless payments and various identification systems. Of course, your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^102]. As with everything else, such capabilities can be used for tracking by various actors.
...
More information over at Wikipedia: <https://en.wikipedia.org/wiki/Radio-frequency_identification#Security_concerns> <sup>[[Archive.org]][68]</sup> and <https://en.wikipedia.org/wiki/Radio-frequency_identification#Privacy> <sup>[[Archive.org]][68]</sup>

The only way to mitigate this problem is to have no RFID tags on you or to shield them again using a type of faraday cage. You could also use specialized wallets/pouches that specifically block RFID communications. Many of those are now made by well-known brands such as Samsonite[^103].

See [Appendix N: Warning about smartphones and smart devices]

## Your Files Properties/Metadata:

This can be obvious to many but not to all. Most files have metadata attached to them. A good example are pictures which store EXIF[^104] information which can contain a lot of information such as GPS coordinates, which camera/phone model took it and when it was taken precisely. While this information might not directly give out who you are, it could tell exactly where you were at a certain moment which could allow others to use different sources to find you (CCTV or other footage taken at the same place at the same time during a protest for instance). It is important that you verify any file you would put on those platforms for any properties that might contain any information that might lead back to you.
...
-   What is a File Format? <https://www.youtube.com/watch?v=VVdmmN0su6E> <sup>[[Invidious]][70]</sup>

-   Ange Albertini: Funky File Formats: <https://www.youtube.com/watch?v=hdCs6bPM4is> <sup>[[Invidious]][71]</sup>
...
So, you are using Tor Browser or Brave Browser over Tor. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^105] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^106] case in 2013 where the FBI inserted malware[^107] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^108] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^109] which can then be used in combination with other data for state surveillance[^110].
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization (See [Appendix W: Virtualization]) so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^111] without being able to access identifying information, or compromise your system.
...
So, if you are a whistleblower and thinking about leaking some picture/audio/video file. Think twice. There are chances that those might contain invisible watermarking within them that would include information about you as a viewer. Such watermarks can be enabled with a simple switch in like Zoom (Video[^112] or Audio[^113]) or with extensions[^114] for popular apps such as Adobe Premiere Pro. These can be inserted by various content management systems.

For a recent example where someone leaking a Zoom meeting recording was caught because it was watermarked: <https://theintercept.com/2021/01/18/leak-zoom-meeting/> <sup>[[Archive.org]][72]</sup>

Such watermarks can be inserted by various products[^115]'[^116]'[^117]'[^118] using Steganography[^119] and can resist compression[^120] and re-encoding[^121]'[^122].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^123] which could lead to de-anonymization.
...
Yes ... Your printers can be used to de-anonymize you as well as explained by the EFF here <https://www.eff.org/issues/printers> <sup>[[Archive.org]][73]</sup>

With this (old but still relevant) video explaining how from the EFF as well: <https://www.youtube.com/watch?v=izMGMsIZK4U> <sup>[[Invidious]][74]</sup>

Basically, many printers will print an invisible watermark allowing for identification of the printer on every printed page. This is called Printer Steganography[^124].There is no real way to mitigate this but to inform yourself on your printer and make sure it does not print any invisible watermark. This is obviously important if you intend to print anonymously.

Here is an (old but still relevant) list of printers and brands who do not print such tracking dots provided by the EFF <https://www.eff.org/pages/list-printers-which-do-or-do-not-display-tracking-dots> <sup>[[Archive.org]][75]</sup>

Here are also some tips from the Whonix documentation (<https://www.whonix.org/wiki/Printing_and_Scanning> <sup>[[Archive.org]][76]</sup>):

**Do not ever print in Color, usually watermarkings are not present without color toners/cartridges**[^125]**.**
...
Here is for example an open-source project you could use yourself for recovering text from some blurred images yourself: <https://github.com/beurtschipper/Depix> <sup>[[Archive.org]][77]</sup>
...
There are also tutorials for using such techniques using Photo Editing tools such as GIMP such as: <https://medium.com/@somdevsangwan/unblurring-images-for-osint-and-more-part-1-5ee36db6a70b> <sup>[[Archive.org]][79]</sup> followed by <https://medium.com/@somdevsangwan/deblurring-images-for-osint-part-2-ba564af8eb5d> <sup>[[Archive.org]][80]</sup>

![][81]

Finally, you will find plenty of deblurring resources here: <https://github.com/subeeshvasu/Awesome-Deblurring> <sup>[[Archive.org]][82]</sup>

Some online services could even help you do this automatically to some extent like MyHeritage.com enhance tool:

<https://www.myheritage.com/photo-enhancer> <sup>[[Archive.org]][83]</sup>
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted Tor traffic over the years[^126]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^127] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It can do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][85]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^128] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^129] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
I also strongly recommend reading this very good, complete and thorough guide on many Attack Vectors on Tor: <https://github.com/Attacks-on-Tor/Attacks-on-Tor> <sup>[[Archive.org]][88]</sup> as well as this recent research publication <https://www.researchgate.net/publication/323627387_Shedding_Light_on_the_Dark_Corners_of_the_Internet_A_Survey_of_Tor_Research> <sup>[[Archive.org]][89]</sup>

(In their defense, it should also be noted that Tor is not designed to protect against a Global adversary. For more information see <https://svn-archive.torproject.org/svn/projects/design-paper/tor-design.pdf> <sup>[[Archive.org]][90]</sup> and specifically, "Part 3. Design goals and assumptions.".)

Lastly, do remember that using Tor can already be considered a suspicious activity[^130] and its use could be considered malicious by some[^131].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^132]. Most crypto currencies can be tracked accurately through various methods[^133].

Remember what they say on their own page: <https://bitcoin.org/en/you-need-to-know> <sup>[[Archive.org]][91]</sup> and <https://bitcoin.org/en/protect-your-privacy> <sup>[[Archive.org]][92]</sup>:

"Bitcoin is not anonymous "

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You will have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^134] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy/anonymity in mind like Monero but even those have some and warnings to consider[^135]'[^136].

Even if you use Mixers or Tumblers[^137] (services that specialize in "anonymizing" crypto currencies by "mixing them"), keep in mind this is only obfuscation[^138] and not actual anonymity[^139]. Not only are they only obfuscation but they could also put you in trouble as you might end up exchanging your crypto against "dirty" crypto that was used in various questionable contexts[^140].
...
All modern CPUs[^141] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^142] and the AMD Platform Security Processor[^143].

Those management platforms are basically small operating systems running directly on your CPU as long as they have power. These systems have full access to your computer's network and could be accessed by an adversary to de-anonymize you in various ways (using direct access or using malware for instance) as shown in this enlightening video: BlackHat, How to Hack a Turned-Off Computer, or Running Unsigned Code in Intel Management Engine <https://www.youtube.com/watch?v=mYsTBPqbya8> <sup>[[Invidious]][93]</sup>.

These have already been affected by several security vulnerabilities in the past[^144] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^145].

There are some not so easy ways[^146] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

Note that to AMD's defense, so far and AFAIK, there were no security vulnerabilities found for ASP and no backdoors eithers: See <https://www.youtube.com/watch?v=bKH5nGLgi08&t=2834s> <sup>[[Invidious]][94]</sup>.

If you are feeling a bit more adventurous, you could install your own BIOS using Libreboot[^147] or Coreboot[^236] if your laptop supports it.
...
<https://en.wikipedia.org/wiki/Transient_execution_CPU_vulnerability> <sup>[[Archive.org]][95]</sup>

-   If you are using Linux you can check the vulnerability status of your CPU to Spectre/Meltdown attacks by using <https://github.com/speed47/spectre-meltdown-checker> <sup>[[Archive.org]][96]</sup> which is available as a package for most Linux distros including Whonix.

-   If you are using Windows, you can check the vulnerability status of your CPU using inSpectre <https://www.grc.com/inspectre.htm> <sup>[[Archive.org]][97]</sup>

Some of these can be avoided using Virtualization Software settings that can mitigate such exploits. See this guide for more information <https://www.whonix.org/wiki/Spectre_Meltdown> <sup>[[Archive.org]][98]</sup> (warning: these can severely impact the performance of your VMs).
...
These have been used since at least since 2008 using an attack called "Jasager"[^148] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^149].
...
-   YouTube, Hak5, Wi-Fi Pineapple Mark VII <https://www.youtube.com/watch?v=7v3JR4Wlw4Q> <sup>[[Invidious]][99]</sup>

These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance, a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^150]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^151] with the exact same layout as the Wi-Fi you are trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they will themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^152]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.

-   Hak5, USB Rubber Ducky <https://shop.hak5.org/products/usb-rubber-ducky-deluxe> <sup>[[Archive.org]][100]</sup>

-   Hak5, O.MG Cable <https://www.youtube.com/watch?v=V5mBJHotZv0> <sup>[[Invidious]][101]</sup>

-   Keelog <https://www.keelog.com/> <sup>[[Archive.org]][102]</sup>

-   AliExpress <https://www.aliexpress.com/i/4000710369016.html> <sup>[[Archive.org]][103]</sup>

Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^153] in 2005.

While you could inspect an USB key physically, scan it with various utilities, check the various components to see if they are genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^154].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^155], Cellebrite Cloud[^156]) that will help an adversary analyze your cloud data with ease.

Notable Examples:

-   Apple iCloud: <https://support.apple.com/en-us/HT202303> <sup>[[Archive.org]][104]</sup> : "Messages in iCloud also uses end-to-end encryption. If you have iCloud Backup turned on**, your backup includes a copy of the key protecting your Messages**. This ensures you can recover your Messages if you lose access to iCloud Keychain and your trusted devices. ".

-   Google Drive and WhatsApp: <https://faq.whatsapp.com/android/chats/about-google-drive-backups/> <sup>[[Archive.org]][105]</sup> : "**Media and messages you back up aren't protected by WhatsApp end-to-end encryption while in Google Drive**. ".

-   Dropbox: <https://www.dropbox.com/privacy#terms> <sup>[[Archive.org]][106]</sup> "To provide these and other features, **Dropbox accesses, stores, and scans Your Stuff**. You give us permission to do those things, and this permission extends to our affiliates and trusted third parties we work with".

-   Microsoft OneDrive: <https://privacy.microsoft.com/en-us/privacystatement> <sup>[[Archive.org]][107]</sup> : Productivity and communications products, "When you use OneDrive, we collect data about your usage of the service, as well as the content you store, to provide, improve, and protect the services. **Examples include indexing the contents of your OneDrive documents so that you can search for them later and using location information to enable you to search for photos based on where the photo was taken**".
...
This is the part where you should watch the documentary "The Social Dilemma"[^157] on Netflix as they cover this topic much better than anyone else IMHO.

This includes is the way you write[^158]'[^159], the way you behave[^160]'[^161]. The way you click. The way you browse. The fonts you use on your browser[^162]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
-   For example, as a basis of authentication, a user's typing speed, keystroke depressions, patterns of error (say accidentally hitting an "l" instead of a "k" on three out of every seven transactions) and mouse movements establishes that person's unique pattern of behavior[^163].

-   This technology is also widely used in CAPTCHAS[^296] services to verify that you are "human" and can be used to fingerprint a user.
...
-   2017, Behavior Analysis in Social Networks, <https://link.springer.com/10.1007/978-1-4614-7163-9_110198-1> <sup>[[Archive.org]][108]</sup>

-   2017, Social Networks and Positive and Negative Affect <https://www.sciencedirect.com/science/article/pii/S1877042811013747/pdf?md5=253d8f1bb615d5dee195d353dc077d46&pid=1-s2.0-S1877042811013747-main.pdf> <sup>[[Archive.org]][109]</sup>

-   2015, Using Social Networks Data for Behavior and Sentiment Analysis <https://www.researchgate.net/publication/300562034_Using_Social_Networks_Data_for_Behavior_and_Sentiment_Analysis> <sup>[[Archive.org]][110]</sup>

-   2016, A Survey on User Behavior Analysis in Social Networks <https://www.academia.edu/30936118/A_Survey_on_User_Behaviour_Analysis_in_Social_Networks> <sup>[[Archive.org]][111]</sup>

-   2019, In­fluence and Behavior Analysis in Social Networks and Social Media <https://sci-hub.do/10.1007/978-3-030-02592-2> <sup>[[Archive.org]][112]</sup>
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^164] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^165].Have a look at their very informative toolkit here: <https://docs.google.com/spreadsheets/d/18rtqh8EG2q1xBo2cLNyhIDuK9jrPGwYr9DI2UncoqJQ/edit#gid=930747607> <sup>[[Archive.org]][113]</sup>

You can also view a very convenient list of available OSINT tools here <https://github.com/jivoi/awesome-osint> <sup>[[Archive.org]][114]</sup>

As well as this interesting Playlist on YouTube: <https://www.youtube.com/playlist?list=PLrFPX1Vfqk3ehZKSFeb9pVIHqxqrNW8Sy> <sup>[[Invidious]][115]</sup>
...
Companies like Facebook have used advanced face recognition for years[^166]'[^167] and have been using other means (Satellite imagery) to create maps of "people" around the world[^168]. This evolution has been going on for years to the point we can now say "We lost control of our faces"[^169].

If you are walking in a touristy place, you will most likely appear in someone's selfie within minutes without knowing it. That person will then proceed to upload that selfie to various platforms (Twitter, Google Photos, Instagram, Facebook, Snapchat ...). Those platforms will then apply face recognition algorithms to those pictures under the pretext of allowing better/easier tagging or to better organize your photo library. In addition to this, the same picture will provide a precise timestamp and in most cases geolocation of where it was taken. Even if the person does not provide a timestamp and geolocation, it can still be guessed with other means[^170]'[^171].

Here are a few resources for even trying this yourself:

-   Bellingcat, Guide To Using Reverse Image Search For Investigations: <https://www.bellingcat.com/resources/how-tos/2019/12/26/guide-to-using-reverse-image-search-for-investigations/> <sup>[[Archive.org]][116]</sup>

-   Bellingcat, Using the New Russian Facial Recognition Site SearchFace <https://www.bellingcat.com/resources/how-tos/2019/02/19/using-the-new-russian-facial-recognition-site-searchface-ru/> <sup>[[Archive.org]][117]</sup>

-   Bellingcat, Dali, Warhol, Boshirov: Determining the Time of an Alleged Photograph from Skripal Suspect Chepiga <https://www.bellingcat.com/resources/how-tos/2018/10/24/dali-warhol-boshirov-determining-time-alleged-photograph-skripal-suspect-chepiga/> <sup>[[Archive.org]][118]</sup>

-   Bellingcat, Advanced Guide on Verifying Video Content <https://www.bellingcat.com/resources/how-tos/2017/06/30/advanced-guide-verifying-video-content/> <sup>[[Archive.org]][119]</sup>

-   Bellingcat, Using the Sun and the Shadows for Geolocation <https://www.bellingcat.com/resources/2020/12/03/using-the-sun-and-the-shadows-for-geolocation/> <sup>[[Archive.org]][120]</sup>

-   Bellingcat, Navalny Poison Squad Implicated in Murders of Three Russian Activists <https://www.bellingcat.com/news/uk-and-europe/2021/01/27/navalny-poison-squad-implicated-in-murders-of-three-russian-activists/> <sup>[[Archive.org]][121]</sup>

-   Bellingcat, Berlin Assassination: New Evidence on Suspected FSB Hitman Passed to German Investigators <https://www.bellingcat.com/news/2021/03/19/berlin-assassination-new-evidence-on-suspected-fsb-hitman-passed-to-german-investigators/> <sup>[[Archive.org]][122]</sup>

Even if you are not looking at the camera, they can still figure out who you are[^172], make out your emotions[^173], analyze your gait[^174] and probably guess your political affiliation[^175]'[^176].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^177]'[^178]'[^179]'[^180]'[^181].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^182]) in their CCTV networks to look for "persons of interest"[^183]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^184] including possibly identifying ethnic minorities[^185]. A simple face recognition error by some algorithm can ruin your life[^186].
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^187].

Same goes with your voice which can be analyzed by for various purposes as shown in the recent Spotify patent[^188].
...
-   Consider wearing special sunglasses (expensive unfortunately) called "Reflectacles" <https://www.reflectacles.com/> <sup>[[Archive.org]][129]</sup>. There was a small study showing their efficiency against IBM and Amazon facial recognition[^199].
...
Your Browser and Device Fingerprints[^308] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance, websites will be able to provide a "mobile experience" if you are using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^200] based browsers (such as Chrome) or Firefox[^201] unless taking special measures.

You can find a lot of detailed information and publications about this on these resources:

-   <https://amiunique.org/links> <sup>[[Archive.org]][130]</sup>

-   <https://brave.com/brave-fingerprinting-and-privacy-budgets/> <sup>[[Archive.org]][131]</sup>
...
It should also be noted that while some browsers and extensions will offer fingerprint resistance, this resistance in itself can also be used to fingerprint you as explained here <https://palant.info/2020/12/10/how-anti-fingerprinting-extensions-tend-to-make-fingerprinting-easier/> <sup>[[Archive.org]][132]</sup>
...
Phishing[^202] is a social engineering[^203] type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^61] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
These have been used countless times and the usual one is called the "419 scam" (see <https://en.wikipedia.org/wiki/Advance-fee_scam> <sup>[[Archive.org]][133]</sup>).

Here is a good video if you want to learn a bit more about phishing types: Black Hat, Ichthyology: Phishing as a Science <https://www.youtube.com/watch?v=Z20XNp-luNA> <sup>[[Invidious]][134]</sup>.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they are encrypted[^204]. These techniques are widely used by law enforcement all over the world and should be considered.

Here are some recent resources you should read about your smartphone:

-   UpTurn, The Widespread Power of U.S. Law Enforcement to Search Mobile Phones <https://www.upturn.org/reports/2020/mass-extraction/> <sup>[[Archive.org]][135]</sup>

-   New-York Times, The Police Can Probably Break Into Your Phone <https://www.nytimes.com/2020/10/21/technology/iphone-encryption-police.html> <sup>[[Archive.org]][136]</sup>

-   Vice, Cops Around the Country Can Now Unlock iPhones, Records Show <https://www.vice.com/en/article/vbxxxd/unlock-iphone-ios11-graykey-grayshift-police> <sup>[[Archive.org]][137]</sup>

I also highly recommend that you read some documents from a forensics examiner perspective such as:

-   EnCase Forensic User Guide, <http://encase-docs.opentext.com/documentation/encase/forensic/8.07/Content/Resources/External%20Files/EnCase%20Forensic%20v8.07%20User%20Guide.pdf> <sup>[[Archive.org]][138]</sup>

-   FTK Forensic Toolkit, <https://accessdata.com/products-services/forensic-toolkit-ftk> <sup>[[Archive.org]][139]</sup>

-   SANS Digital Forensics and Incident Response Videos, <https://www.youtube.com/c/SANSDigitalForensics/videos>

And finally, here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^205]
...
And there are reasons[^206]'[^207]'[^208] for that:
...
-   You should be wary of Apps/Services using a "modified" or proprietary cryptographic method[^209].

-   By default, you should not trust any "Roll your own crypto" until it was audited, peer-reviewed, vetted, and accepted by the cryptography community[^210]'[^211].

-   There is no such thing as "military grade crypto"[^212]'[^213]'[^214]
...
-   Telegram: <https://buttondown.email/cryptography-dispatches/archive/cryptography-dispatches-the-most-backdoor-looking/> <sup>[[Archive.org]][141]</sup>
...
-   Many other examples can be found here: <https://www.cryptofails.com/> <sup>[[Archive.org]][142]</sup>
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^215] or a national security letter[^216]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^217].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^218].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^219].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^220].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^221].

Some providers have implemented the use of a Warrant Canary[^222] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^223] and Omnisec story[^224]).
...
(illustration: excellent movie I highly recommend: Das Leben der Anderen[^225])

There are many advanced techniques that can be used by skilled adversaries[^226] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> <sup>[[Archive.org]][144]</sup> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:

-   Attacks that require a malware implanted in some device:

    -   Exfiltration of Data through a Malware infected Router: <https://www.youtube.com/watch?v=mSNt4h7EDKo> <sup>[[Invidious]][145]</sup>

    -   Exfiltration of Data through observation of Light variation in a Backlit keyboard with a compromised camera: <https://www.youtube.com/watch?v=1kBGDHVr7x0> <sup>[[Invidious]][146]</sup>

        -   Exfiltration of Data through a compromised Security Camera (that could first use the previous attack) <https://www.youtube.com/watch?v=om5fNqKjj2M> <sup>[[Invidious]][147]</sup>

        -   Communication from outsider to compromised Security Cameras through IR light signals: <https://www.youtube.com/watch?v=auoYKSzdOj4> <sup>[[Invidious]][148]</sup>

    -   Exfiltration of data from a compromised air-gapped computer through acoustic analysis of the FAN noises with a smartphone <https://www.youtube.com/watch?v=v2_sZIfZkDQ> <sup>[[Invidious]][149]</sup>

    -   Exfiltration of data from a malware infected air-gapped computer through HD Leds with a Drone <https://www.youtube.com/watch?v=4vIu8ld68fc> <sup>[[Invidious]][150]</sup>

    -   Exfiltration of data from a USB malware on an air-gapped computer through electromagnetic interferences <https://www.youtube.com/watch?v=E28V1t-k8Hk> <sup>[[Invidious]][151]</sup>

    -   Exfiltration of data from a malware infected HDD drive through covert acoustic noise <https://www.youtube.com/watch?v=H7lQXmSLiP8> <sup>[[Invidious]][152]</sup>

    -   Exfiltration of data through GSM frequencies from a compromised (with malware) air-gapped computer <https://www.youtube.com/watch?v=RChj7Mg3rC4> <sup>[[Invidious]][153]</sup>

    -   Exfiltration of data through electromagnetic emissions from a compromised Display device <https://www.youtube.com/watch?v=2OzTWiGl1rM&t=20s> <sup>[[Invidious]][154]</sup>

    -   Exfiltration of data through magnetic waves from a compromised air-gapped computer to a Smartphone stored inside a Faraday bag <https://www.youtube.com/watch?v=yz8E5n1Tzlo> <sup>[[Invidious]][155]</sup>

    -   Communication between two compromised air-gapped computers using ultrasonic soundwaves <https://www.youtube.com/watch?v=yz8E5n1Tzlo> <sup>[[Invidious]][155]</sup>

    -   Exfiltration of Bitcoin Wallet from a compromised air-gapped computer to a smartphone <https://www.youtube.com/watch?v=2WtiHZNeveY> <sup>[[Invidious]][156]</sup>

    -   Exfiltration of Data from a compromised air-gapped computer using display brightness <https://www.youtube.com/watch?v=ZrkZUO2g4DE> <sup>[[Invidious]][157]</sup>

    -   Exfiltration of Data from a compromised air-gapped computer through vibrations <https://www.youtube.com/watch?v=XGD343nq1dg> <sup>[[Invidious]][158]</sup>

    -   Exfiltration of Data from a compromised air-gapped computer by turning RAM into a Wi-Fi emitter <https://www.youtube.com/watch?v=vhNnc0ln63c> <sup>[[Invidious]][159]</sup>

    -   Exfiltration of Data from a compromised air-gapped computer through power lines <https://arxiv.org/abs/1804.04014> <sup>[[Archive.org]][160]</sup>

-   **Attacks that require no malware:**

    -   Observing a light bulb from a distance to listen to the sound in the room[^227] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw> <sup>[[Invidious]][161]</sup>

Here is also a good video from the same authors to explain those topics: Black Hat, The Air-Gap Jumpers <https://www.youtube.com/watch?v=YKRtFgunyj4> <sup>[[Invidious]][162]</sup>
...
Have a look at the Whonix Documentation concerning Data Collection techniques here: <https://www.whonix.org/wiki/Data_Collection_Techniques> <sup>[[Archive.org]][163]</sup>

You might also enjoy looking at this service <https://tosdr.org/> <sup>[[Archive.org]][164]</sup> (Terms of Services, Didn't Read) that will give you a good overview of the various ToS of many services.

Have a look at <https://www.eff.org/issues/privacy> <sup>[[Archive.org]][165]</sup> for some more resources.

Have a look at <https://en.wikipedia.org/wiki/List_of_government_mass_surveillance_projects> <sup>[[Archive.org]][166]</sup> to have an overview of all known mass-surveillance projects, current and past.

Finally, check <https://www.freehaven.net/anonbib/date.html> <sup>[[Archive.org]][167]</sup> for the latest academic papers related to Online Anonymity.
...
-   Google Transparency Report <https://transparencyreport.google.com/user-data/overview> <sup>[[Archive.org]][168]</sup>

-   Facebook Transparency Report <https://transparency.facebook.com/> <sup>[[Archive.org]][169]</sup>

-   Apple Transparency Report <https://www.apple.com/legal/transparency/> <sup>[[Archive.org]][170]</sup>

-   Cloudflare Transparency Report <https://www.cloudflare.com/transparency/> <sup>[[Archive.org]][171]</sup>

-   Snapchat Transparency Report <https://www.snap.com/en-US/privacy/transparency> <sup>[[Archive.org]][172]</sup>

-   Telegram Transparency Report <https://t.me/transparency> <sup>[[Archive.org]][173]</sup> (requires telegram installed)

-   Microsoft Transparency Report <https://www.microsoft.com/en-us/corporate-responsibility/law-enforcement-requests-report> <sup>[[Archive.org]][174]</sup>

-   Amazon Transparency Report <https://www.amazon.com/gp/help/customer/display.html?nodeId=GYSDRGWQ2C2CRYEF> <sup>[[Archive.org]][175]</sup>

-   Dropbox Transparency Report <https://www.dropbox.com/transparency> <sup>[[Archive.org]][176]</sup>

-   Discord Transparency Report <https://blog.discord.com/discord-transparency-report-jan-june-2020-2ef4a3ee346d> <sup>[[Archive.org]][177]</sup>

-   GitHub Transparency Report <https://github.blog/2021-02-25-2020-transparency-report/> <sup>[[Archive.org]][178]</sup>

-   Snapchat Transparency Report <https://www.snap.com/en-US/privacy/transparency/> <sup>[[Archive.org]][179]</sup>

-   TikTok Transparency Report <https://www.tiktok.com/safety/resources/transparency-report?lang=en> <sup>[[Archive.org]][180]</sup>

-   Reddit Transparency Report <https://www.reddit.com/wiki/transparency> <sup>[[Archive.org]][181]</sup>

-   Twitter Transparency Report <https://transparency.twitter.com/> <sup>[[Archive.org]][182]</sup>
...
Zero-Trust Security[^316] ("Never trust, always verify").

Here are some various resources about what is Zero-Trust Security:

-   DEFCON, Zero Trust a Vision for Securing Cloud, <https://www.youtube.com/watch?v=euSsqXO53GY> <sup>[[Invidious]][183]</sup>

-   From the NSA themselves, Embracing a Zero Trust Security Model, <https://media.defense.gov/2021/Feb/25/2002588479/-1/-1/0/CSI_EMBRACING_ZT_SECURITY_MODEL_UOO115131-21.PDF> <sup>[[Archive.org]][184]</sup>
...
-   If you absolutely want system wide plausible deniability[^245]'[^228] despite the risks[^229]'[^248].
...
-   <https://www.whonix.org/wiki/Warning> <sup>[[Archive.org]][186]</sup>

-   <https://www.whonix.org/wiki/Dev/Threat_Model> <sup>[[Archive.org]][187]</sup>

-   <https://www.whonix.org/wiki/Comparison_with_Others> <sup>[[Archive.org]][188]</sup>
...
-   First read more about it at the EFF here: <https://ssd.eff.org/en/module/understanding-and-circumventing-network-censorship> <sup>[[Archive.org]][189]</sup>

-   Check some data yourself here on the Tor Project OONI[^230] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/> <sup>[[Archive.org]][190]</sup>

-   Have a look at <https://censoredplanet.org/> <sup>[[Archive.org]][191]</sup> and see if they have data about your country.

-   Test for yourself using OONI (this can be risky in a hostile environment).

## Steps for all routes:

**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^239]**.**
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^231].

So here is a list of places where you can still get them now: <https://prepaid-data-sim-card.fandom.com/wiki/Registration_Policies_Per_Country> <sup>[[Archive.org]][193]</sup>
...
-   **Recommended**: Do not require any identification (even e-mail):
    ...
    -   (UK based) <https://dtmf.io/> <sup>[[Archive.org]][194]</sup> **preferred** because they even provide an onion hidden service address for direct access through the Tor Network at <http://dtmfiovjh42uviqez6qn75igbagtiyo724hy3rdxm77dy2m5tt7lbaqd.onion/>

    -   (Ukraine based) <https://virtualsim.net/> <sup>[[Archive.org]][195]</sup>

-   Do require identification (valid e-mail):

    -   (Germany based) <https://www.sms77.io/> <sup>[[Archive.org]][196]</sup>

    -   (Russia based) <https://onlinesim.ru/> <sup>[[Archive.org]][197]</sup>

There are some other possibilities listed here <https://cryptwerk.com/companies/sms/xmr/> <sup>[[Archive.org]][198]</sup>. **Use at your own risk.**
...
Please do not buy or use gimmicky self-encrypting devices such as these: <https://syscall.eu/blog/2018/03/12/aigo_part1/> <sup>[[Archive.org]][199]</sup>

Some might be very efficient[^232] but many are gimmicky gadgets that offer no real protection[^233].
...
TAILS[^234] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
You should also read Tails Documentation, Warnings, and limitations, before going further <https://tails.boum.org/doc/about/warning/index.en.html> <sup>[[Archive.org]][200]</sup>
...
<https://tails.boum.org/install/index.en.html> <sup>[[Archive.org]][201]</sup> , pick your flavor and proceed.
...
If you're having issue accessing Tor due to censorship or other issues, you can try using Tor Bridges by following this TAILS tutorial: <https://tails.boum.org/doc/first_steps/welcome_screen/bridge_mode/index.en.html> <sup>[[Archive.org]][202]</sup> and find more information about these on Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][203]</sup>
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^235]**.**
...
-   Download the latest HiddenVM release from <https://github.com/aforensics/HiddenVM/releases> <sup>[[Archive.org]][209]</sup>

-   Download the latest Whonix XFCE release from <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][210]</sup>
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm> <sup>[[Archive.org]][211]</sup>) or System76 laptops (<https://system76.com/> <sup>[[Archive.org]][212]</sup>) while using Coreboot[^236] (where Intel IME is disabled from factory).
...
-   <https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/> <sup>[[Archive.org]][213]</sup>

-   <https://libreboot.org/docs/hardware/> <sup>[[Archive.org]][214]</sup>

-   <https://coreboot.org/status/board-status.html> <sup>[[Archive.org]][215]</sup>
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. And, because AFAIK, AMD PSP was audited and contrary to IME was not found to have any "evil" functionalities[^237]. However, if you are going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^238].
...
These settings can be accessed through the boot menu of your laptop. Here is a good tutorial from HP explaining all the ways to access the BIOS on various computers: <https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs> <sup>[[Archive.org]][216]</sup>
...
-   Enable BIOS/UEFI password and use a long passphrase[^239] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^240]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^241]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^242]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
-   Defcon 22, <https://www.youtube.com/watch?v=QDSlWa9xQuA> <sup>[[Invidious]][217]</sup>

-   BlackHat 2016, <https://www.youtube.com/watch?v=0fZdL3ufVOI> <sup>[[Invidious]][218]</sup>
...
Take a moment to set a firmware password according to the tutorial here: <https://support.apple.com/en-au/HT204455> <sup>[[Archive.org]][219]</sup>

You should also enable firmware password reset protection (available from Catalina) according to the documentation here: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][220]</sup>
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> <sup>[[Archive.org]][221]</sup> [^243] (with pictures).
...
This route will make extensive use of Virtual Machines[^244], they will require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to conveniently use Plausible Deniability[^245] (aka Deniable Encryption[^246]) easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^247] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It is the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^248].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^249]'[^250] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
This also means you could set-up your own advanced "plausible deniability" setup using any Host OS by storing for instance Virtual Machines on a Veracrypt hidden volume container (be careful for traces in the Host OS tho that would need to be cleaned if the host OS is persistent, see [Some additional measures against forensics:] section later). There is a project for achieving this within TAILS (<https://github.com/aforensics/HiddenVM> <sup>[[Archive.org]][204]</sup>) which would make your Host OS non persistent and use plausible deniability within TAILS.

In the case of Windows, plausible deniability is also the reason you should ideally have Windows 10 Home (and not Pro). This is because Windows 10 Pro natively offers a full-disk encryption system (Bitlocker[^251]) where Windows 10 Home offers no full-disk encryption at all. We will later use a third-party open-source software for encryption that will allow full-disk encryption on Windows 10 Home. This will give you a good (plausible) excuse to use this software. While using this software on Windows 10 Pro would be suspicious.

**Note about Linux:** So, what about Linux and plausible deniability? Yes, it is kind of possible to achieve plausible deniability with Linux too[^252]. But it is complicated to set-up and IMHO requires a skill level high enough that you probably do not need this guide to help you try it.
...
Cold Boot attacks[^254] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^255], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^256] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^257] and by the maintainer of Veracrypt[^258] so I would not worry too much about those yet.
...
-   You should limit the use of Sleep stand-by and instead use Shutdown or Hibernate to prevent the encryption keys from staying in RAM when your computer goes to sleep. This is because sleep will maintain power to your memory for resuming your activity faster. Only hibernation and shutdown will actually clear the key from the memory[^259].

See also <https://www.whonix.org/wiki/Cold_Boot_Attack_Defense> <sup>[[Archive.org]][224]</sup> and <https://www.whonix.org/wiki/Protection_Against_Physical_Attacks> <sup>[[Archive.org]][225]</sup>

Here are also some interesting tools to consider for Linux users to defend against these:

-   <https://github.com/0xPoly/Centry> <sup>[[Archive.org]][226]</sup> (unfortunately unmaintained it seems so I made a fork and pull request updating for Veracrypt <https://github.com/AnonymousPlanet/Centry> <sup>[[Archive.org]][227]</sup> which should still work)

-   <https://github.com/hephaest0s/usbkill> <sup>[[Archive.org]][228]</sup> (unfortunately unmaintained as well it seems)

-   <https://github.com/Lvl4Sword/Killer> <sup>[[Archive.org]][229]</sup>

-   <https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks> <sup>[[Archive.org]][230]</sup>

-   (Qubes OS, Intel CPU only) <https://github.com/QubesOS/qubes-antievilmaid> <sup>[[Archive.org]][231]</sup>
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^260].

-   Windows Indexing keeping traces of the files present in your User folder by default[^261].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^262].

-   Many more traces in various logs, please see this convenient interesting poster for more insight: <https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download> <sup>[[Archive.org]][232]</sup>

###### MacOS:

-   Gatekeeper[^263] and XProtect keeping track of your download history in a local database and file attributes.
...
Consider also reading **<https://www.whonix.org/wiki/Full_Disk_Encryption#Encrypting_Whonix_VMs>** <sup>[[Archive.org]][233]</sup>
...
You should always remember that despite the reputation, Linux mainstream distributions (Ubuntu for instance) are not necessarily better at security than other systems such as MacOS and Windows. See this reference to understand why <https://madaidans-insecurities.github.io/linux.html> <sup>[[Archive.org]][234]</sup>.

#### Full disk encryption:

There are two possibilities here with Ubuntu:

-   (Recommended and easy) Encrypt as part of the installation process: <https://ubuntu.com/tutorials/install-ubuntu-desktop> <sup>[[Archive.org]][235]</sup>
...
-   (Tedious but possible) Encrypt after installation: <https://help.ubuntu.com/community/ManualFullSystemEncryption> <sup>[[Archive.org]][236]</sup>
...
-   If you are not sure, just make sure you did not enable any telemetry and follow this tutorial if needed <https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/> <sup>[[Archive.org]][237]</sup>
...
-   Disable Bluetooth if enabled by following this guide <https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/> <sup>[[Archive.org]][238]</sup> or issuing the following command:

    -   ```sudo systemctl disable bluetooth.service --force```

-   Disable Indexing if enabled by default (Ubuntu >19.04) by following this guide <https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html> <sup>[[Archive.org]][239]</sup> or issuing the following commands:
...
Follow this tutorial to enable Hibernate: <https://help.ubuntu.com/16.04/ubuntu-help/power-hibernate.html> <sup>[[Archive.org]][240]</sup>

After Hibernate is enabled, change the behavior so that your laptop will hibernate when you close the lid by following this tutorial for Ubuntu 20.04 <http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/> <sup>[[Archive.org]][241]</sup> and this tutorial for Ubuntu 18.04 <https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/> <sup>[[Archive.org]][242]</sup>

Unfortunately, this will not clean the key from memory directly from memory when hibernating. To avoid this at the cost of some performance, you might consider encrypting the swap file by following this tutorial: <https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap> <sup>[[Archive.org]][243]</sup>

These settings should mitigate cold boot attacks if you can hibernate fast enough.

#### Enable MAC address randomization:

-   Ubuntu, follow these steps <https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses> <sup>[[Archive.org]][244]</sup>.

-   Any other distro: you will have to find the documentation yourself but it should be quite similar to the Ubuntu tutorial.

-   Consider this tutorial which should still work: <https://josh.works/shell-script-basics-change-mac-address> <sup>[[Archive.org]][245]</sup>

#### Hardening Linux:

Please refer to this guide <https://madaidans-insecurities.github.io/guides/linux-hardening.html> <sup>[[Archive.org]][246]</sup>
...
For securing and hardening your MacOS, I recommend reading this GitHub guide which should cover many of the issues: <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][247]</sup>
...
First you should set-up a firmware password following this guide from Apple: <https://support.apple.com/en-us/HT204455> <sup>[[Archive.org]][248]</sup>

Unfortunately, some attacks are still possible and an adversary could disable this password so you should also follow this guide to prevent disabling the firmware password from anyone including Apple: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][220]</sup>
...
These are the infamous "unblockable telemetry" calls from MacOS Big Sur disclosed here: <https://sneak.berlin/20201112/your-computer-isnt-yours/> <sup>[[Archive.org]][249]</sup>
...
But you should probably document yourself on the actual issue before acting. This page is a good place to start: <https://blog.jacopo.io/en/post/apple-ocsp/> <sup>[[Archive.org]][250]</sup>
...
You should enable full disk encryption on your Mac using Filevault according to this part of the guide: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#full-disk-encryption> <sup>[[Archive.org]][247]</sup>
...
Alternatively, you could use this free piece of software: <https://technitium.com/tmac/> <sup>[[Archive.org]][251]</sup>
...
Veracrypt[^264] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^265] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
Go ahead and download and install Veracrypt from: <https://www.veracrypt.fr/en/Downloads.html> <sup>[[Archive.org]][252]</sup>

After installation, please take a moment to review the following options that will help mitigate some attacks:

-   Encrypt the memory with a Veracrypt option[^266] (settings > performance/driver options > encrypt RAM) at a cost of 5-15% performance. This setting will also disable hibernation (which does not actively clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings > Preferences > Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^267] and prevent some local data leaks.
...
By default, Windows 10 might not offer you this possibility so you should enable it by following this Microsoft tutorial: <https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation> <sup>[[Archive.org]][253]</sup>
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^268] Operations[^269]. This will severely degrade the performance/health of your SSD drive over time.

**As you can see, Route C only offers two privacy advantages over the others and it will only be of use against a soft lawful adversary. Remember <https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis>** <sup>[[Archive.org]][208]</sup>**.**
...
Read the Veracrypt Documentation <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][254]</sup> (Process of Creation of Hidden Operating System part) and <https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html> <sup>[[Archive.org]][255]</sup> (Security Requirements and Precautions Pertaining to Hidden Volumes).
...
Remember to read <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][254]</sup>
...
-   Use a strong passphrase[^239] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^276] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
Before going to next step, you should learn the way to mount your Outer Volume safely for writing content on it. This is also explained in this official Veracrypt Documentation <https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html> <sup>[[Archive.org]][257]</sup>
...
Take some time to read again the "Possible Explanations for Existence of Two Veracrypt Partitions on Single Drive" of the Veracrypt documentation here <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][254]</sup>
...
In this route, we will make extensive use of the free Oracle Virtualbox[^277] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the virtualization software with the best track record in terms of security and some of the reported issues[^278] have not be completely fixed to this date[^279] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM> <sup>[[Archive.org]][258]</sup>
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^280].
...
VPN/Proxy over Tor falls on the right-side[^281] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
**You should however consider not using this method when your aim is just to browse random various unauthenticated websites as you will not benefit from Stream Isolation and this could make correlation attacks easier over time for an adversary between each of your sessions (see [Your Anonymized Tor/VPN traffic:][264]). If your goal however is to use the same identity at each session on the same authenticated services, the value of Stream isolation is lessened as you can be correlated through other means.**
...
-   <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][265]</sup>

-   <https://tails.boum.org/contribute/design/stream_isolation/> <sup>[[Archive.org]][266]</sup>

-   <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][267]</sup>
...
This route will use Virtualization and Whonix[^282] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^283] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
Also consider applying these mitigations from VirtualBox to mitigate Spectre[^285]/Meltdown[^286] vulnerabilities by running this command from the VirtualBox Program Directory. All of these are described here: <https://www.whonix.org/wiki/Spectre_Meltdown> <sup>[[Archive.org]][98]</sup> (be aware these can impact severely the performance of your VMs but should be done for best security).
...
-   Import Whonix file Into Virtualbox following the instructions on <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][210]</sup>

-   Start the Whonix VMs

Remember at this stage that if you are having issues connecting to Tor due to censorship or blocking, you should consider connecting using Bridges as explained in this tutorial <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][286]</sup>.

-   Update the Whonix VMs by following the instructions on <https://www.whonix.org/wiki/Operating_System_Software_and_Updates#Updates> <sup>[[Archive.org]][287]</sup>
...
**Important Note: You should also read these very good recommendations over there <https://www.whonix.org/wiki/DoNot>** <sup>[[Archive.org]][288]</sup> **as most of those principles will also apply to this guide. You should also read their general documentation here <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][289]</sup> **which will also provide tons of advice like this guide.**
...
**It is also the only VM that will provide Stream Isolation pre-configured for most apps by default**[^287]**.**

If you want additional software on the Workstation (such as another Browser), follow their guide here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][290]</sup>

Consider running Whonix in Live Mode if for extra malware protection, See <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][291]</sup>

Do not forget to apply the VM hardening recommendations here: Virtualbox Hardening recommendations:

#### Linux (any distro):

**Be careful, any customization you make to the Non-Whonix guest VMs (keyboard layout, language, time zone, screen resolution or other) could be used to fingerprint your VMs later. See <https://www.whonix.org/wiki/VM_Fingerprinting>** <sup>[[Archive.org]][292]</sup>
...
Refer to this tutorial <https://www.whonix.org/wiki/Other_Operating_Systems> <sup>[[Archive.org]][293]</sup> for detailed instructions.
...
-   Download and install Brave browser from <https://brave.com/download/> <sup>[[Archive.org]][294]</sup>
...
Basically, follow the tutorial here: <https://www.android-x86.org/documentation/virtualbox.html> <sup>[[Archive.org]][295]</sup>
...
-   Windows Host OS:
    ...
    -   Virtualbox Catalina Tutorial: <https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/> <sup>[[Archive.org]][296]</sup>

    -   Virtualbox Big Sur Tutorial: <https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/> <sup>[[Archive.org]][297]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^288] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^289] authentication with the authenticator feature.
...
-   Whonix: [https://www.whonix.org/wiki/KeePassXC][] <sup>[[Archive.org]][299]</sup>

-   Linux:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][300]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_linux> <sup>[[Archive.org]][301]</sup>

-   Windows:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][300]</sup>

    -   Follow the tutorial here [https://KeePassXC.org/docs/KeePassXC_GettingStarted.html#_microsoft_windows][] <sup>[[Archive.org]][301]</sup>

-   MacOS:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][300]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_macos> <sup>[[Archive.org]][301]</sup>
...
-   Qube OS Introduction, <https://www.qubes-os.org/intro/> <sup>[[Archive.org]][302]</sup>

-   Qube OS Video Tours, <https://www.qubes-os.org/video-tours/> <sup>[[Archive.org]][303]</sup>

-   Qube OS Getting Started, <https://www.qubes-os.org/doc/getting-started/> <sup>[[Archive.org]][304]</sup>

-   YouTube, Life Behind the Tinfoil: A Look at Qubes and Copperhead - Konstantin Ryabitsev, The Linux Foundation <https://www.youtube.com/watch?v=8cU4hQg6GvU> <sup>[[Invidious]][305]</sup>

-   YouTube, I used the reasonably-secure Qubes OS for 6 months and survived - Matty McFatty [@themattymcfatty] <https://www.youtube.com/watch?v=sbN5Bz3v-uA> <sup>[[Invidious]][306]</sup>

-   YouTube, Qubes OS: How it works, and a demo of this VM-centric OS <https://www.youtube.com/watch?v=YPAvoFsvSbg> <sup>[[Invidious]][307]</sup>

This OS is recommended by prominent figures such as Edward Snowden and Privacytools.io.

Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^290], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You should also check their hardware compatibility here <https://www.qubes-os.org/hcl/> <sup>[[Archive.org]][308]</sup> before proceeding. Your mileage might vary and you might experience several issues with regards to hardware compatibility that you will have to troubleshoot and solve yourself.

I think that if you can afford it and are comfortable with the idea of using Linux, you should go with this route as it is probably the best one in terms of security and privacy. The only disadvantage of this route is that it does not provide a way to enable OS wide plausible deniability[^245] unlike the Whonix route.
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^291].

Stream isolation is a mitigation technique used to prevent some correlation attacks by having different Tor Circuits for each application. Here is an illustration to show what stream isolation is:

![][263]

VPN/Proxy over Tor falls on the right-side[^292] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:

-   When your destination service does not allow Tor Exit nodes.

-   When you do not mind using a shared Tor circuit for various services. Like for instance for using various authenticated services.

**You should however consider not using this method when your aim is just to browse random various unauthenticated websites as you will not benefit from Stream Isolation and this could make correlation attacks easier for an adversary between each of your sessions (see [Your Anonymized Tor/VPN traffic:][264]).**
...
Note, if you're having issues accessing the Tor Network due to blocking/censorship, you could try using Tor Bridges (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][203]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][286]</sup>).
...
See [Appendix O: Get an anonymous VPN/Proxy][311]

### Installation:

We will follow the instructions from their own guide <https://www.qubes-os.org/doc/installation-guide/> <sup>[[Archive.org]][312]</sup>:

Secure Boot is not supported as per their FAQ: <https://www.qubes-os.org/faq/#is-secure-boot-supported> <sup>[[Archive.org]][313]</sup> so it should be disabled in the BIOS/UEFI settings.
...
-   Install Qubes OS according to the installation guide:
    ...
    -   **If you want to use Tor or VPN over Tor: Check the** "**Enabling system and template updates over the Tor anonymity network using Whonix" during the last step. This will force all Qubes OS updates to go through Tor. While this will significantly reduce your update speed, it will increase your anonymity from the start.** (If you are having issues connecting to Tor due to censorship or blocking, consider using Tor Bridges as recommended earlier. Just follow the tutorial provided here: <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][286]</sup>)
...
This is based on the tutorial provided by Qube OS themselves (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/vpn.md> <sup>[[Archive.org]][314]</sup>). If you are familiar with this process, you can follow their tutorial. Here is mine:
...
-   Test your Connectivity:
    ...
    -   If you are going for VPN over Tor, Test the VM connectivity to Tor by launching a Browser within the ProxyVM and going to <https://check.torproject.org> <sup>[[Archive.org]][315]</sup> (It should say you are connected to Tor)
...
> (These will block outbound traffic when the VPN is down, it is a kill switch, more information here <https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux> <sup>[[Archive.org]][316]</sup> )
...
-   Test the ProxyVM VPN connectivity by starting a Browser within it and going to your VPN provider test page. It should now say you are connected to a VPN:
    ...
    -   Mullvad: <https://mullvad.net/en/check/> <sup>[[Archive.org]][317]</sup>

    -   IVPN: <https://www.ivpn.net/> <sup>[[Archive.org]][318]</sup> (check the top banner)

    -   ProtonVPN: Follow their instructions here <https://protonvpn.com/support/vpn-ip-change/> <sup>[[Archive.org]][319]</sup>
...
-   Create a Whonix Workstation Disposable VM (follow this tutorial <https://www.whonix.org/wiki/Qubes/DisposableVM> <sup>[[Archive.org]][320]</sup>)
...
Apply the instructions from <https://brave.com/linux/> <sup>[[Archive.org]][321]</sup> (Fedora 28+ section) and run the following commands:
...
Edit the Whonix Disposable VM template and follow instructions here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][290]</sup>
...
Now you will have to follow the instructions from here: <https://github.com/anbox/anbox-modules> <sup>[[Archive.org]][322]</sup>
...
Now we will follow their other tutorial from here: <https://github.com/anbox/anbox/blob/master/docs/install.md> <sup>[[Archive.org]][323]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^294] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^295] authentication with the authenticator feature.
...
Captcha[^296] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^297] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCaptcha service[^298]) and Cloudflare (hCaptcha[^299]). hCaptcha is used on 15% of the internet by their own metrics[^300].

They are designed to separate bots from humans but are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you will quickly encounter many captchas everywhere[^301]. Quite often when using Tor, even if you succeed in solving all the puzzles, you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor> <sup>[[Archive.org]][261]</sup>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^302]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^303].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it is "human".

-   They track your behavior before/during/after the tests to ensure you are "human"[^304].
...
You will often experience several in a row (sometimes endless[^305]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You will also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.

You will also have (in my experience) more Captchas (reCaptcha) in Google if you do not use Chrome. But this can be mitigated by using Chromium based browsers such as Brave or Ungoogled-Chromium. There is also a Browser extension called Buster that could help you those <https://github.com/dessant/buster> <sup>[[Archive.org]][325]</sup>.

As for Cloudflare (hCaptcha), you could also use their Accessibility solution here (<https://www.hcaptcha.com/accessibility> <sup>[[Archive.org]][326]</sup>) which would allow you to sign-up (with your anonymous identity created later) and set a cookie within your Browser that would allow you to bypass their captchas. Another solution to mitigate hCaptcha would be to use their own solution called "Privacy Pass"[^306] <https://privacypass.github.io/> <sup>[[Archive.org]][327]</sup> in the form of a Browser extension you could install in your VM Browser.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^307].
...
Browser and Device[^308] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^309]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Chromium based Browsers such as Brave Browser over Tor Browser within this VM.
...
While this might seem innocent, this is obviously an ID verification and de-anonymization method. This is just indirectly relying on third party financial KYC[^134] regulations.

This is for instance now the case on YouTube for some European Users[^310] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^311] and unfortunately sometimes required.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^312]. Unfortunately, this method is now also sometimes being used on Facebook[^313] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^314].

This verification is very hard to defeat but possible. A method to possibly defeat those would be to use "deep fake" technology software such as the open-source FaceSwap <https://github.com/deepfakes/faceswap> <sup>[[Archive.org]][331]</sup> to generate the required verification pictures using a randomly computer-generated face that would be swapped over the picture of a complicit model (or a stock photo).
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^315] (or alternatively "Never trust, always verify" if you are more hardcore about it and want to apply Zero-Trust Security[^316]) instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is the real term for this[^317]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience less issues with captchas and online platforms[^309] if you use Brave than if you use Tor Browser (feel free to try this yourself).

-   Every identity you should have a matching profile picture associated to it. For this purpose, I recommend you just go to <https://thispersondoesnotexist.com/> <sup>[[Archive.org]][332]</sup> and generate a computer-generated profile picture. You can also generate such pictures yourself from your computer if you prefer by using the open-source StyleGan project here <https://github.com/NVlabs/stylegan2> <sup>[[Archive.org]][333]</sup>. Just refresh the page until you find a picture that matches your identity in all aspects (age, sex, and ethnicity) and save that picture. It would be even better to have several pictures associated to that identity but I do not have an "easy way" of doing that yet.

    -   **Bonus**, you could also make it more real by using this service (with an anonymous identity) <https://www.myheritage.com/deep-nostalgia> <sup>[[Archive.org]][334]</sup> to make a picture more lifelike. Here is an example:
...
You could also achieve the same result without using MyHeritage and by doing it yourself using for example <https://github.com/AliaksandrSiarohin/first-order-model> <sup>[[Archive.org]][336]</sup> but this will require more manual operations (**and requires an NVIDIA GPU**).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^318] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^239]**.**

Here is also a good guide on this specific topic: <https://gendersec.tacticaltech.org/wiki/index.php/Complete_manual#.22Real.22_names> <sup>[[Archive.org]][337]</sup>
...
-   Whonix/Tails: Create/Edit a file ```/usr/local/etc/torrc.d/50_user.conf```[^319].

-   On Tor Browser: Edit the torrc file located at ```Browser/TorBrowser/Data/Tor```[^320].
...
**Please note that this is restricting Onion Routing could limit your Anonymity if you are too restrictive. You can see a visualized list of available Exit Nodes here: <https://www.bigdatacloud.com/insights/tor-exit-nodes>** <sup>[[Archive.org]][338]</sup>

Here is the list of possibilities (this is a general list and many of those countries might not have Exit nodes at all): <https://b3rn3d.herokuapp.com/blog/2014/03/05/tor-country-codes/> <sup>[[Archive.org]][339]</sup>
...
On my side, I strongly condemn this type of real-name policy. See for instance this Wikipedia article giving some examples: <https://en.wikipedia.org/wiki/Facebook_real-name_policy_controversy> <sup>[[Archive.org]][340]</sup>

Here are some more references about the German case for reference:

-   <https://slate.com/technology/2018/02/why-some-americans-are-cheering-germany-for-taking-on-facebooks-real-name-policy.html> <sup>[[Archive.org]][341]</sup>

-   <https://www.theverge.com/2018/2/12/17005746/facebook-real-name-policy-illegal-german-court-rules> <sup>[[Archive.org]][342]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][343]</sup>

-   <https://www.vzbv.de/sites/default/files/downloads/2018/02/14/18-02-12_vzbv_pm_facebook-urteil_en.pdf> <sup>[[Archive.org]][344]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][343]</sup>

-   <https://www.reuters.com/article/us-germany-facebook/german-court-rules-facebook-use-of-personal-data-illegal-idUSKBN1FW1FI> <sup>[[Archive.org]][345]</sup>
...
-   South Korea (see <https://en.wikipedia.org/wiki/Real-name_system#South_Korea> <sup>[[Archive.org]][346]</sup>)

-   If you know any other, please let me know with references in the GitHub issues.

Some platforms are by-passing this requirement all-together by requiring a valid payment method instead (see [Financial transactions:]). While this does not directly require a real-name through their ToS, this has the same results as they usually only accept mainstream (not Monero/Cash) payment methods (such as Visa/MasterCard/Maestro or PayPal) which do require a real-name legally as part of their KYC[^134] regulations. The result is the same and arguably even better than a simple real-name policy you could ignore in some countries such as Germany.

### Overview:

This section will show you an overview of the current various requirements on some platforms.

-   **Consider using the recommended tools on <https://privacytools.io/>** <sup>[[Archive.org]][347]</sup> **for your better privacy instead of the usual mainstream ones.**

-   **Consider using the recommended tools on <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][289]</sup> **as well instead of the usual mainstream ones such as E-mail providers: <https://www.whonix.org/wiki/E-Mail#Anonymity_Friendly_Email_Provider_List>** <sup>[[Archive.org]][348]</sup>

**The following overview does not mention the privacy practices of those platforms but only their requirements for registering an account. If you want to use privacy-aware tools and platforms, head on to <https://privacytools.io/>** <sup>[[Archive.org]][347].^
...
-   Is this against their ToS? No but yes <https://www.amazon.com/gp/help/customer/display.html?nodeId=202140280> <sup>[[Archive.org]][349]</sup>
...
-   Is this against their ToS? Yes <https://www.apple.com/legal/internet-services/icloud/en/terms.html> <sup>[[Archive.org]][350]</sup>
...
-   Is this against their ToS? No <https://briarproject.org/privacy-policy/> <sup>[[Archive.org]][351]</sup>
...
-   Is this against their ToS? No <https://discord.com/terms> <sup>[[Archive.org]][352]</sup>
...
-   Is this against their ToS? No <https://element.io/terms-of-service> <sup>[[Archive.org]][353]</sup>
...
-   Is this against their ToS? Yes <https://www.facebook.com/terms.php> <sup>[[Archive.org]][354]</sup>
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^321] will not help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
-   Is this against their ToS? No <https://docs.github.com/en/free-pro-team@latest/github/site-policy/github-terms-of-service> <sup>[[Archive.org]][355]</sup>
...
-   Is this against their ToS? No <https://about.gitlab.com/handbook/legal/subscription-agreement/> <sup>[[Archive.org]][356]</sup>
...
-   Is this against their ToS? No <https://policies.google.com/terms> <sup>[[Archive.org]][357]</sup>
...
If your account gets suspended [^324] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^325] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^326].

#### HackerNews:

-   Is this against their ToS? No <https://www.ycombinator.com/legal/#tou> <sup>[[Archive.org]][358]</sup>
...
-   Is this against their ToS? **Maybe?** I am not sure <https://help.instagram.com/581066165581870?ref=dp> <sup>[[Archive.org]][359]</sup>
...
-   Is this against their ToS? No <https://jami.net/privacy-policy/> <sup>[[Archive.org]][360]</sup>
...
-   Is this against their ToS? No <https://www.ivpn.net/tos/> <sup>[[Archive.org]][361]</sup>
...
-   Is this against their ToS? Yes <https://www.linkedin.com/legal/user-agreement> <sup>[[Archive.org]][362]</sup>
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^327] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
-   Is this against their ToS? No unless it is about crypto <https://policy.medium.com/medium-terms-of-service-9db0094a1e0f> <sup>[[Archive.org]][363]</sup>
...
-   Is this against their ToS? Yes <https://www.microsoft.com/en/servicesagreement/> <sup>[[Archive.org]][364]</sup>
...
-   Is this against their ToS? No <https://mullvad.net/en/help/terms-service/> <sup>[[Archive.org]][365]</sup>
...
-   Is this against their ToS? No <https://njal.la/tos/> <sup>[[Archive.org]][366]</sup>
...
-   Is this against their ToS? No <https://ProtonMail.com/terms-and-conditions> <sup>[[Archive.org]][367]</sup>
...
They are recommended by privacytools.io[^328]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^329]. If their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
-   Is this against their ToS? No <https://protonvpn.com/terms-and-conditions> <sup>[[Archive.org]][368]</sup>
...
-   Is this against their ToS? No <https://www.redditinc.com/policies> <sup>[[Archive.org]][369]</sup>
...
-   Is this against their ToS? Yes <https://slashdotmedia.com/terms-of-use/> <sup>[[Archive.org]][370]</sup>
...
-   Is this against their ToS? No <https://telegram.org/tos> <sup>[[Archive.org]][371]</sup>
...
Their appeal process is just sending an e-mail to <recover@telegram.org> <sup>[[Archive.org]][372]</sup> stating your phone number and issue and hope they answer.
...
-   Is this against their ToS? No <https://tutanota.com/terms/> <sup>[[Archive.org]][373]</sup>
...
This time you will need to submit an appeal using a form[^330], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it is possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
-   Is this against their ToS? No <https://www.twitch.tv/p/en/legal/terms-of-service/> <sup>[[Archive.org]][374]</sup>
...
-   Is this against their ToS? **Yes** <https://www.whatsapp.com/legal/updates/terms-of-service-eea> <sup>[[Archive.org]][375]</sup>
...
You are going to have to find a different way to post there using at least seven proxies[^331] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
End-to-end Encryption[^332] (aka e2ee) is a rather simple concept. It just means only you and your destination know each-others public encryption keys and no one in between that would be eavesdropping would be able to decrypt the communication.
...
More on this topic on this YouTube video: <https://www.youtube.com/watch?v=zSQtyW_ywZc> <sup>[[Invidious]][378]</sup>
...
Zero-Access Encryption[^334] at rest is used when you store data at some provider (let us say your chat history or chat backups) but this history or backup is encrypted on your side and cannot be read or decrypted by the provider hosting it.
...
Such feature would have prevented important hacks such as the Cambridge Analytica scandal[^335] if it was implemented.
...
-   The Sealed Sender option[^336].

-   The Private Contact Discovery[^337].

-   The Private Group System[^338].
...
-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_instant_messaging_protocols> <sup>[[Archive.org]][379]</sup>

-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_cross-platform_instant_messaging_clients> <sup>[[Archive.org]][380]</sup>

-   Secure Messaging Apps <https://www.securemessagingapps.com/> <sup>[[Archive.org]][381]</sup>

-   ProtonMail Blog, <https://protonmail.com/blog/whatsapp-alternatives/> <sup>[[Archive.org]][382]</sup>

-   Whonix Documentation, Instant Messenger Chat <https://www.whonix.org/wiki/Chat> <sup>[[Archive.org]][383]</sup>
...
I will recommend these option (as also recommend by privacytools.io[^339]'[^340]):

-   Briar (<https://briarproject.org/> <sup>[[Archive.org]][384]</sup>)*

-   OnionShare version >2.3 (<https://onionshare.org/> <sup>[[Archive.org]][385]</sup>)*

-   Jami (<https://jami.net/> <sup>[[Archive.org]][386]</sup>)

-   Element/Matrix.org (<https://element.io/> <sup>[[Archive.org]][387]</sup>)
...
If you must do so, you should take some steps because you cannot really trust any organization to protect your anonymity[^341]:
...
For this, I strongly recommend the use of SecureDrop[^342] (<https://securedrop.org/> <sup>[[Archive.org]][388]</sup>) which is an open-source project from the Freedom of the Press foundation.

Ideally you should use SecureDrop over Tor and you will find a curated list of those here <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][389]</sup>
...
-   Check regularly the eventual appearance of compromise of any of your identities using <https://haveibeenpwned.com/> <sup>[[Archive.org]][390]</sup> (obviously from a safe environment).
...
The process is fairly simple and all you will need is to follow Veracrypt tutorial here: <https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html> <sup>[[Archive.org]][391]</sup>
...
Qubes OS recommends using their own utility for backups as documented here <https://www.qubes-os.org/doc/backup-restore/> <sup>[[Archive.org]][392]</sup> . But I think it is just a hassle and provides limited added value unless you just want to back-up a single Qube. So instead, I am also recommending just making a full image with Clonezilla which will remove all the hassle and bring you back a working system in a few easy steps.
...
Self-hosting (using Nextcloud for instance) is also a possibility provided you do have an anonymous hosting. You could pick one from the list here that does accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][396]</sup>

**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][397]</sup>**) and it is Njalla <https://njal.la/>** <sup>[[Archive.org]][398]</sup>
...
To that the answer is very simple and a clear consensus for everyone: <https://syncthing.net/> <sup>[[Archive.org]][399]</sup>
...
So, the fact is that it is very unlikely[^349]'[^350] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^351]'[^352]'[^353] if they are fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^354] for 11$ and use it to erase any disk.
...
Trim should be sufficient in most cases and you could just use the blkdiscard command to force an entire device trim as explained here: <https://wiki.archlinux.org/index.php/Solid_state_drive#Trim_an_entire_device> <sup>[[Archive.org]][405]</sup>
...
-   Option A: Follow one of these tutorials:
    ...
    -   <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][408]</sup>

    -   <https://linoxide.com/linux-command/commands-wipe-disk-linux/> <sup>[[Archive.org]][409]</sup>

    -   <https://wiki.archlinux.org/index.php/Securely_wipe_disk> <sup>[[Archive.org]][410]</sup>

I recommend using dd or shred for this purpose.

-   Option B: Install and use BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][406]</sup> or follow this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][411]</sup>
...
-   Option B: Use external tools such as:
    ...
    -   Eraser (open-source): <https://eraser.heidi.ie/download/> <sup>[[Archive.org]][412]</sup>

    -   KillDisk Free: <http://killdisk.com/killdisk-freeware.htm> <sup>[[Archive.org]][413]</sup>
...
Unfortunately, the MacOS Recovery disk utility will not be able to perform a secure erase of your SSD drive as stated in Apple documentation <https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac> <sup>[[Archive.org]][414]</sup>.
...
-   <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][415]</sup>

-   <https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac> <sup>[[Archive.org]][416]</sup> or using Veracrypt full disk encryption.
...
Follow this tutorial: <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][415]</sup> and use the secure erase option from Disk Utility which should work fine on HDD and Thumb drives.
...
-   Right Click a file/folder:
    ...
    -   BleachBit: Shred with BleachBit (or see this tutorial from the EFF <https://ssd.eff.org/en/module/how-delete-your-data-securely-windows> <sup>[[Archive.org]][418]</sup>)
...
-   Decrypt and re-encrypt (using LUKS for instance following this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][419]</sup>) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.

-   Trim using ```fstrim --all``` and then fill up the entire free space of the disk using a utility such as:

    -   BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][406]</sup>
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][407]</sup>) run these commands on the drive you want to fill:
...
-   You can do this the graphical way with BleachBit following this tutorial from the EFF: <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][411]</sup>

-   Or you can do this from the command line following this tutorial: <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][408]</sup> (For this purpose I recommend wipe and shred).
...
-   Decrypt and re-encrypt (using LUKS using this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][419]</sup> or Veracrypt from the graphical interface for instance) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.
...
-   Fill the free space using one of those methods:
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][407]</sup>) run these commands:
...
Please follow this documentation to Trim within Qubes OS: <https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][420]</sup>
...
-   Decrypt and re-encrypt the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space. I didn't find a reliable tutorial on how to do this safely on Qubes OS but it's possible this Tutorial could work as well <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][419]</sup> (at your own risk, this has not been tested yet).

-   Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][420]</sup>) and then trim using "fstrim --all" and then fill up the entire free space of the disk using an utility such as:
...
If Trim is supported and enabled by your Linux Distribution for your external SSD drive. There should be no issue in securely deleting data normally and just issue an "fstrim --all" from terminal to trim the drive. Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][420]</sup>) to enable trim on a drive.
...
-   If your file system is APFS, you do not need to worry about Trim, it apparently happens asynchronously as the OS writes data[^360] according to their own documentation.
...
-   If your file system is HFS+, you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][421]</sup>)

![][422]

#### System/Internal, External HDD drive or a Thumb Drive:

Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^361]. So, you are left with using other tools:

-   Permanent Eraser <http://www.edenwaith.com/products/permanent%20eraser/> <sup>[[Archive.org]][423]</sup>

-   From the terminal you can use the "rm --P filename" command which should erase the file and overwrite it as explained in this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][424]</sup>.
...
Consider also reading this documentation if you're going with Whonix <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][291]</sup> as well as their general hardening tutorial for all platforms here <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][425]</sup>

### Removing Metadata from Files/Documents/Pictures:

#### Pictures and videos:

On Windows, MacOS and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][426]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][427]</sup>) that allows viewing and/or removing those properties.

**ExifTool is natively available on TAILS and Whonix Workstation.**

##### ExifCleaner:

Just install it from <https://exifcleaner.com/> <sup>[[Archive.org]][427]</sup>, run and drag and drop the files into the GUI.
...
Here is a tutorial to remove metadata from a Picture using OS provided tools: <https://www.purevpn.com/internet-privacy/how-to-remove-metadata-from-photos> <sup>[[Archive.org]][428]</sup>

##### Cloaking/Obfuscating to prevent picture recognition:

Consider the use of Fawkes <https://sandlab.cs.uchicago.edu/fawkes/> <sup>[[Archive.org]][429]</sup> (<https://github.com/Shawn-Shan/fawkes> <sup>[[Archive.org]][430]</sup>) to cloak the images from picture recognition tech on various platforms.

Or if you want on-line versions, consider:

-   <https://lowkey.umiacs.umd.edu/> <sup>[[Archive.org]][431]</sup>

-   <https://adversarial.io/> <sup>[[Archive.org]][432]</sup>

#### PDF Documents:

##### PDFParanoia (Linux/Windows/MacOS/QubesOS):

Consider using <https://github.com/kanzure/pdfparanoia> <sup>[[Archive.org]][433]</sup> which will remove metadata and watermarks on any PDF.
...
First, here is a tutorial to remove metadata from Office documents: <https://support.microsoft.com/en-us/office/remove-hidden-data-and-personal-information-by-inspecting-documents-presentations-or-workbooks-356b7b5d-77af-44fe-a07f-9aa4d085966f> <sup>[[Archive.org]][434]</sup>. Make sure however that you do use the latest version of Office with the latest security updates.

Alternatively, on Windows, MacOS, Qubes OS, and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][426]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][427]</sup>) that allows viewing and/or removing those properties
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^362] (<https://0xacab.org/jvoisin/mat2> <sup>[[Archive.org]][435]</sup>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^363] and available on TAILS by default[^364].
...
Note that it's possible to run Whonix in Live mode leaving no traces when you shut down the VMs, consider reading their documentation here <https://www.whonix.org/wiki/VM_Live_Mode> <sup>[[Archive.org]][437]</sup> and here <https://www.whonix.org/wiki/Warning#Whonix_.E2.84.A2_Persistence_vs_Live_vs_Amnesic> <sup>[[Archive.org]][186]</sup>.
...
Most of the info from this section can also be found at this nice guide <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][247]</sup>
...
Refer to this section of this guide for further information <https://github.com/drduh/macOS-Security-and-Privacy-Guide#gatekeeper-and-xprotect> <sup>[[Archive.org]][247]</sup>
...
See this section of this guide for guidance on where to find and how to delete such artifacts: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#metadata-and-artifacts> <sup>[[Archive.org]][247]</sup>

Many of those can be deleted using some various commercial third-party tools but I would personally recommend using the free and well-known Onyx which you can find here: <https://www.titanium-software.fr/en/onyx.html> <sup>[[Archive.org]][438]</sup>. Unfortunately, it is closed-source but it is notarized, signed and has been trusted for many years.
...
-   If your file system is HFS+ (or any other than APFS), you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][421]</sup>).

![][422]

### Linux (Qubes OS):

Please consider their guidelines <https://github.com/Qubes-Community/Contents/blob/master/docs/security/security-guidelines.md> <sup>[[Archive.org]][439]</sup>

If you are using Whonix on Qubes OS, please consider following some of their guides:

-   Whonix System Hardening guide <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][425]</sup>

-   Enabling App Armor on Qubes <https://www.whonix.org/wiki/Qubes/AppArmor> <sup>[[Archive.org]][440]</sup>

-   Also consider the use of Linux Kernel Guard <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][441]</sup>
...
Nevertheless, you might want to clean some logs. Just use this convenient tool: <https://github.com/sundowndev/go-covermyass> <sup>[[Archive.org]][442]</sup> (instructions on the page)
...
Consider the use of Linux Kernel Guard as an added measure <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][441]</sup>
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^267]), your network connections, app crash information and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   Launch Regedit using this tutorial: <https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11> <sup>[[Archive.org]][443]</sup>
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^260] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you should not do this on the Decoy OS.

-   Download Shellbag Analyzer & Cleaner from <https://privazer.com/en/download-shellbag-analyzer-shellbag-cleaner.php> <sup>[[Archive.org]][444]</sup>
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php> <sup>[[Archive.org]][445]</sup>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^271]). Secure Deletion with Trim[^268] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
-   Get and install the latest version from BleachBit here <https://www.bleachbit.org/download> <sup>[[Archive.org]][446]</sup>
...
-   <https://justdeleteme.xyz/> <sup>[[Archive.org]][447]</sup>

-   <https://justgetmydata.com/> <sup>[[Archive.org]][448]</sup>
...
Go to their "Remove outdated content from Google Search" page here: <https://search.google.com/search-console/remove-outdated-content> <sup>[[Archive.org]][449]</sup> and submit a request accordingly.
...
Go to their "Content Removal" page here: <https://www.bing.com/webmasters/tools/contentremoval> <sup>[[Archive.org]][450]</sup> and submit a request accordingly.
...
DuckDuckGo does not store cached version of pages[^365] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^366] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
Once have your Yandex account, head to the Yandex Webmaster tools <https://webmaster.yandex.com> <sup>[[Archive.org]][451]</sup> and then select Tools and Delete URL <https://webmaster.yandex.com/tools/del-url/> <sup>[[Archive.org]][452]</sup>
...
Yes, Yahoo Search still exists but as per their help page <https://help.yahoo.com/kb/SLN4530.html> <sup>[[Archive.org]][453]</sup> , there is no way to remove information or refresh information besides waiting. This could take 6 to 8 weeks.
...
As far as I know, there is no way to remove information from Wikipedia articles themselves but if you just want to remove traces of your username from it (as a user that contributed), you can do so by following these steps: <https://en.wikipedia.org/wiki/Wikipedia:Courtesy_vanishing> <sup>[[Archive.org]][454]</sup>
...
A commonly used technique which combines the idea of a Dead Drop[^367] and Secure Communication Obfuscation[^368] through Steganography[^369] and/or Kleptography[^370] and has many names such as Koalang[^371] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^239]**.**
...
-   Consider the use of Haven <https://guardianproject.github.io/haven/> <sup>[[Archive.org]][455]</sup> on some old android phone to keep watch on your home/room while you are away.

-   Doxx "yourself" and your identities from time to time by looking for them yourself online using various search engines to monitor your online identities. You can even automate the process somewhat using various tools such as Google Alerts <https://www.google.com/alerts> <sup>[[Archive.org]][456]</sup>.
...
-   Never ever leave your laptop unattended/on/unlocked anywhere when conducting sensitive activities. Remember the story of Ross Ulbricht and his arrest <https://en.wikipedia.org/wiki/Ross_Ulbricht#Silk_Road,_arrest_and_trial> <sup>[[Archive.org]][457]</sup>.

-   Check for tampering regularly (not only your devices but also your home/room).

-   If you can, do not talk to the police (at least if you are in the US) <https://www.youtube.com/watch?v=d-7o9xYp7eE> <sup>[[Invidious]][458]</sup> without a lawyer.
...
-   Read those tips here <https://www.whonix.org/wiki/DoNot> <sup>[[Archive.org]][288]</sup>

-   Finally, have common sense, do not be dumb, look and learn from others' mistakes:

    -   DEFCON 22, Adrian Crenshaw- Dropping Docs on Darknets: How People Got Caught, <https://www.youtube.com/watch?v=eQ2OZKitRwc> <sup>[[Invidious]][459]</sup> ([Slides][] <sup>[[Archive.org]][460]</sup>)

    -   Bad Opsec - How Tor Users Got Caught, <https://www.youtube.com/watch?v=GR_U0G-QGA0> <sup>[[Invidious]][461]</sup>
...
Keep in mind that many countries have specific laws to compel you to reveal your passwords that could override your "right to remain silent". See this Wikipedia article: <https://en.wikipedia.org/wiki/Key_disclosure_law> <sup>[[Archive.org]][462]</sup> and this other visual resource with law references <https://www.gp-digital.org/world-map-of-encryption/> <sup>[[Archive.org]][463]</sup>.
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> <sup>[[Archive.org]][464]</sup> [^372]

"In the end the Party would announce that two and two made five, and you would have to believe it." -- George Orwell, 1984.

# Donations:

This project has no funding and donations are welcome at <https://anonymousplanet.org/donations.html> <sup>[[Mirror]][465]</sup> <sup>[[Archive.org]][466]</sup> <sup>[[Tor Mirror]][467]</sup> or directly by sending Monero (XMR) to this address: ```4549BGJrEPBfpiPRL9CVGzGMgJnC1Dzf8EXLVfY8Ukrnj7LzkTV611dGf9tuQHiSQjbixsNWiffNiV5fPB3LkyF7UXi3vwQ```
...
-   **Special Thanks to Edward Snowden and who inspired me to write this guide (buy and read his book please <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>** <sup>[[Archive.org]][471]</sup>**)**
...
As written earlier in this guide and as noted by Privacytools.io[^373], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple of utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   Download and install W10Privacy from <https://www.w10privacy.de/english-home/> <sup>[[Archive.org]][472]</sup>
...
-   Download and run WindowsSpyBlocker from <https://crazymax.dev/WindowsSpyBlocker/download/> <sup>[[Archive.org]][473]</sup>
...
<https://www.microsoft.com/en-us/software-download/windows10> <sup>[[Archive.org]][474]</sup>
...
-   System Rescue:
    ...
    -   Create a System Rescue USB disk following these instructions <https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/> <sup>[[Archive.org]][475]</sup> (download the ISO and write to an USB stick with Rufus).
...
-   SATA SSD:
    ...
    -   ATA Secure Erase:
        ...
        -   Follow one of these tutorials
            ...
            -   <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing> <sup>[[Archive.org]][476]</sup>

            -   <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase> <sup>[[Archive.org]][477]</sup>

            -   <https://tinyapps.org/docs/wipe_drives_hdparm.html> <sup>[[Archive.org]][478]</sup>

    -   ATA Sanitize:

        -   Follow this tutorial <https://tinyapps.org/docs/ata_sanitize_hdparm.html> <sup>[[Archive.org]][479]</sup>
...
-   NVMe SSD:
    ...
    -   Follow one of these tutorials:
        ...
        -   <https://tinyapps.org/docs/nvme-secure-erase.html> <sup>[[Archive.org]][480]</sup>

        -   <https://tinyapps.org/docs/nvme-sanitize.html> <sup>[[Archive.org]][481]</sup>

# Appendix E: Clonezilla

-   Get Clonezilla by just following these instructions: <https://clonezilla.org/liveusb.php> <sup>[[Archive.org]][482]</sup> (I recommend the Alternative version AMD64 that should work with most recent laptops)

-   Boot from Clonezilla

-   Follow these steps to make a backup: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image> <sup>[[Archive.org]][483]</sup>
...
-   You are done, if you need to restore, follow these instructions: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image> <sup>[[Archive.org]][484]</sup>
...
-   Download and install Tor Browser according to the instructions from <https://www.torproject.org/download/> <sup>[[Archive.org]][485]</sup>

-   Open Tor Browser

-   Click the little shield Icon and select your Security level (see <https://tb-manual.torproject.org/security-settings/> <sup>[[Archive.org]][486]</sup> for details):
...
If you are experiencing issues connecting to Tor due to Censorship or Blocking, you might consider using Tor bridges as explained here: <https://bridges.torproject.org/> <sup>[[Archive.org]][487]</sup>
...
-   Native Tools:
    ...
    -   Windows 10 Disk Cleanup Utility: <https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68> <sup>[[Archive.org]][488]</sup>
...
-   Windows 10 Optimize Utility (Defrag on HDD Drives): <https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a> <sup>[[Archive.org]][489]</sup>
...
-   Third Party Tools:
    ...
    -   The open-source utility BleachBit <https://www.bleachbit.org/> <sup>[[Archive.org]][490]</sup>

    -   The closed-source utility PrivaZer <https://privazer.com/> <sup>[[Archive.org]][491]</sup>
...
-   The Windows USN journal which stores plenty of information[^375].

-   The Windows System Resource Usage Monitor (SRUM)[^376].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^377].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^378]) or System Rescue CD (<https://www.system-rescue.org/> <sup>[[Archive.org]][492]</sup>) for this but personally, I will recommend the use of ShredOS.

Feel free do go with DBAN instead if you want (using this tutorial: <https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148> <sup>[[Archive.org]][493]</sup>), the process is basically the same but will not work out of the box with UEFI laptops.

If you want to go with System-Rescue, just head to their website and follow the instructions.

## Windows:

-   Download ShredOS from <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][494]</sup>

-   Unzip the ISO file

-   Download Rufus from <https://rufus.ie/> <sup>[[Archive.org]][495]</sup>
...
-   Follow instructions on <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][494]</sup>
...
-   SanDisk DashBoard: <https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information> <sup>[[Archive.org]][496]</sup>

-   Seagate SeaTools: <https://www.seagate.com/support/downloads/seatools/> <sup>[[Archive.org]][497]</sup>

-   Samsung Magican: <https://www.samsung.com/semiconductor/minisite/ssd/download/tools/> <sup>[[Archive.org]][498]</sup>

-   Kingston SSD Manager: <https://www.kingston.com/unitedstates/en/support/technical/ssdmanager> <sup>[[Archive.org]][499]</sup>

-   Lenovo:

    -   Most likely native utility available within the BIOS/UEFI, please check

    -   Drive Erase Utility: <https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad> <sup>[[Archive.org]][500]</sup>

-   Crucial Storage Executive: <https://www.crucial.com/support/storage-executive> <sup>[[Archive.org]][501]</sup>

-   Western Digital Dashboard: <https://support.wdc.com/downloads.aspx?p=279> <sup>[[Archive.org]][502]</sup>

-   HP: Follow instructions on <https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd> <sup>[[Archive.org]][503]</sup>

-   Transcend SSD Scope: <https://www.transcend-info.com/Support/Software-10/> <sup>[[Archive.org]][504]</sup>

-   Dell:

    -   Most likely native utility available within the BIOS/UEFI, please check <https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt> <sup>[[Archive.org]][505]</sup>

## Tools that provide only support from running OS (for external drives).

-   Toshiba Storage Tools: <https://www.toshiba-storage.com/downloads/> <sup>[[Archive.org]][506]</sup>
...
Some might be very efficient[^379] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you if it is supported or not like CrystalDiskInfo[^380] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
Follow this good tutorial: <https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux> <sup>[[Archive.org]][507]</sup>

### ATA/NVMe Operations (Secure Erase/Sanitize):

**It is not "recommended". Please read the disclaimers here <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase>** <sup>[[Archive.org]][477]</sup> **and here <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing>** <sup>[[Archive.org]][476]</sup>
...
See also this tutorial <https://code.mendhak.com/securely-wipe-ssd/> <sup>[[Archive.org]][508]</sup>
...
According to Apple Documentation[^368], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
If your disk supports it but it is not enabled in MacOS. You could try issuing a "sudo trimforce enable" command from the Terminal and see if it enables Trim on your external SSD. And then again check the first aid command if it is not APFS (see this Tutorial for info <https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789> <sup>[[Archive.org]][509]</sup>)

If it does not work, I am not aware of any reliable method to enable TRIM besides the commercial utility Trim Enabler here <https://cindori.org/trimenabler/> <sup>[[Archive.org]][510]</sup> which claims support for external drives.
...
Download the latest Debian testing amd64 netinst ISO from <https://www.debian.org/CD/netinst/> <sup>[[Archive.org]][511]</sup>
...
-   Lenovo ThinkPads: <https://support.lenovo.com/be/en/solutions/migr-68369> <sup>[[Archive.org]][512]</sup>

-   HP (all): <https://support.hp.com/gb-en/document/c06204100> <sup>[[Archive.org]][513]</sup>

-   Dell (all): <https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe> <sup>[[Archive.org]][514]</sup>

-   Acer (Travelmate only): <https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks> <sup>[[Archive.org]][515]</sup>
...
-   If you really must take them with you, you could consider the use of a faraday cage[^381] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^382] for their effectiveness. If you cannot afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
-   **Encrochat: <https://en.wikipedia.org/wiki/EncroChat>** <sup>[[Archive.org]][516]</sup>

-   **Sky ECC: <https://en.wikipedia.org/wiki/Sky_ECC>** <sup>[[Archive.org]][517]</sup>
...
There are three VPN companies recommended by privacytools.io (<https://privacytools.io/providers/vpn/> <sup>[[Archive.org]][518]</sup>) that accept cash payments: Mullvad, iVPN and ProtonVPN.
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^383] and touch DNA[^384].
...
-   Do not lick the envelope or the stamps[^385] if you use them to avoid leaving DNA traces.
...
This does offer some disadvantage as Monero is not perfect as explained earlier in this guide and some global adversaries could maybe still track you. You will need to get Monero from an Exchange using the normal financial system and then pick a hosting (list here <https://www.getmonero.org/community/merchants/#exchanges> <sup>[[Archive.org]][396]</sup>)

You could use <https://evolution-host.com/vps-hosting.php> <sup>[[Archive.org]][519]</sup> (who supports Whonix) which does accept Monero payments. Or pick one from the list here that do accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][396]</sup>

**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][397]</sup>**) and it is Njalla <https://njal.la/>** <sup>[[Archive.org]][398]</sup> **and I would recommend them as well.**
...
There are plenty of tutorials on how to do this like this one <https://proprivacy.com/vpn/guides/create-your-own-vpn-server> <sup>[[Archive.org]][520]</sup>
...
-   (Windows/Linux/MacOS) <https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/> <sup>[[Archive.org]][521]</sup>

-   (Windows/Linux/MacOS) <https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel> <sup>[[Archive.org]][522]</sup>

-   (Windows) <https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/> <sup>[[Archive.org]][523]</sup>

-   (Linux/MacOS) <https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/> <sup>[[Archive.org]][524]</sup>
...
-   Download and install Putty from <https://www.putty.org/> <sup>[[Archive.org]][525]</sup>
...
In that case my last resort recommendation is to connect safely **from a distance** to a Public Wi-Fi (See [Find some safe places with decent public Wi-Fi:]) using your laptop and TAILS "unsafe browser". See <https://tails.boum.org/contribute/design/Unsafe_Browser/> <sup>[[Archive.org]][527]</sup>.
...
Consider reading this older but still relevant guide <https://archive.flossmanuals.net/bypassing-censorship/index.html> <sup>[[Archive.org]][528]</sup>
...
**Personally, I would recommend the AWUS036 series in the Alfa brand of adapters (see <https://www.alfa.com.tw/>** <sup>[[Archive.org]][530]</sup>**).** But you could also go with some other brands if you want such as the TP-Link TL-WN722 (see <https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/> <sup>[[Archive.org]][531]</sup>).

See this post for a comparison of various adapters: <https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html> <sup>[[Archive.org]][532]</sup> (Usually those antennas are used by Penetration Testers to probe Wi-Fis from a distance and are often discussed within the scope of the Kali Linux distribution).
...
**Do not forget tho that this will only delay a motivated adversary. Your signal can be triangulated easily by a motivated adversary in a matter of minutes once they reach the physical location of the Wi-Fi you're connecting to (for instance using a device such as AirCheck <https://www.youtube.com/watch?v=8FV2QZ1BPnw>** <sup>[[Invidious]][533]</sup>**, also see their other products here <https://www.netally.com/products/#wifi_s>** <sup>[[Archive.org]][534]</sup>**). These products can easily be deployed on mobile units (in a Car for instance) and pinpoint your location in a matter of minutes.**
...
-   Windows Tutorials:
    ...
    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-windows/> <sup>[[Archive.org]][535]</sup>

    -   iVPN: <https://www.ivpn.net/apps-windows> <sup>[[Archive.org]][536]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-windows-vpn-application/> <sup>[[Archive.org]][537]</sup>

-   MacOS:

    -   Mullvad: <https://mullvad.net/en/help/install-and-use-mullvad-app-macos/> <sup>[[Archive.org]][538]</sup>

    -   IVPN: <https://www.ivpn.net/apps-macos/> <sup>[[Archive.org]][539]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-mac-vpn-application/> <sup>[[Archive.org]][540]</sup>

-   Linux:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-linux/> <sup>[[Archive.org]][541]</sup>

    -   iVPN: <https://www.ivpn.net/apps-linux/> <sup>[[Archive.org]][542]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/linux-vpn-setup/> <sup>[[Archive.org]][543]</sup>
...
-   Windows:
    ...
    -   iVPN: <https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/> <sup>[[Archive.org]][544]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/what-is-kill-switch/> <sup>[[Archive.org]][545]</sup>

    -   Mullvad: <https://mullvad.net/en/help/using-mullvad-vpn-app/#killswitch> <sup>[[Archive.org]][546]</sup>
...
-   MacOS:
    ...
    -   ProtonVPN same as Windows with the client, the option should be in the provided VPN client <https://protonvpn.com/blog/macos-vpn-kill-switch/> <sup>[[Archive.org]][547]</sup>

-   Linux:

    -   Mullvad:

        -   <https://mullvad.net/en/help/wireguard-and-mullvad-vpn/> <sup>[[Archive.org]][548]</sup>

        -   <https://mullvad.net/en/help/linux-openvpn-installation/> <sup>[[Archive.org]][549]</sup>

    -   ProtonVPN: <https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md#kill-switch> <sup>[[Archive.org]][550]</sup>

    -   iVPN:

        -   <https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/> <sup>[[Archive.org]][551]</sup>

        -   <https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/> <sup>[[Archive.org]][552]</sup>

# Appendix S: Check your network for surveillance/censorship using OONI

So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^230].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^386].
...
The probe can be found here: <https://ooni.org/install/> <sup>[[Archive.org]][553]</sup> for various platforms (iOS, Android, Windows, MacOS, and Linux).

# Appendix T: Checking files for malware

## Integrity (if available):

Usually, integrity checks[^387] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^388], more recently MD5[^389] but those present several weaknesses (CRC, MD5[^390]) that makes them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^391] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it is usually recommended to use SHA[^392] based hashes and the most used is probably the SHA-2[^393] based SHA256 for verifying file integrity. SHA is much more resistant to collisions[^394] than CRC and MD5. And collisions with SHA256 or SHA512 are rare and hard to compute for an adversary.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^397] for instance) using public key cryptography[^398].
...
-   Install GPG for your OS:
    ...
    -   Windows: gpg4win (<https://www.gpg4win.org/> <sup>[[Archive.org]][554]</sup>)

    -   MacOS: GPGTools (<https://gpgtools.org/> <sup>[[Archive.org]][555]</sup>)
...
-   <https://support.torproject.org/tbb/how-to-verify-signature/> <sup>[[Archive.org]][556]</sup>

-   <https://tails.boum.org/install/vm-download/index.en.html> <sup>[[Archive.org]][557]</sup> (See Basic OpenPGP verification).

-   <https://www.whonix.org/wiki/Verify_the_Whonix_images> <sup>[[Archive.org]][558]</sup>
...
You might be asking yourself, what about Anti-Virus solutions? Well, no ... these are not perfect solutions against many modern malware and viruses using polymorphic code[^399]. But it does not mean they cannot help against less sophisticated and known attacks. It depends how to use them as AV software can become an attack vector in itself.
...
Some will just argue against them broadly like Whonix[^400] but this topic is being discussed and disputed even at Whonix[^401] by other members of their community.
...
-   Yes, there are viruses and malware for Linux[^402]'[^403]'[^404]'[^405]'[^406]

-   Yes, there are viruses and malware for MacOS[^410]'[^407][^408]'[^409][^410]
...
-   Do use Open-Source non-real time offline Anti-Virus/Anti-Malware tools as an added measure to scan some files such as:
    ...
    -   Windows/Linux/MacOS/Qubes OS: ClamAV (<https://www.clamav.net/> <sup>[[Archive.org]][559]</sup>)

    -   Linux/Qubes OS: RFXN Linux Malware Detect (<https://github.com/rfxn/linux-malware-detect> <sup>[[Archive.org]][560]</sup>)

    -   Linux/Qubes OS: Chkrootkit (<http://www.chkrootkit.org/> <sup>[[Archive.org]][561]</sup>)
...
-   You could also use online services for **non-sensitive files*** such as VirusTotal (<https://www.virustotal.com/gui>) or Hybrid-analysis (<https://hybrid-analysis.com/>).
    ...
    -   You could also just check the VirusTotal database for the hash of your file if you don't want to send it over (see <https://developers.virustotal.com/v3.0/docs/search-by-hash> <sup>[[Archive.org]][562]</sup> (See the [Integrity (if available):] section again for guidance on how to generate hashes).

    -   Other tools are also available for non-sensitive files and a convenient list is right here: <https://github.com/rshipp/awesome-malware-analysis#online-scanners-and-sandboxes> <sup>[[Archive.org]][563]</sup>

* **Please be aware that while VirusTotal might seem very practical for scanning various files, their "privacy policy" is problematic (see <https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy>** <sup>[[Archive.org]][564]</sup>**) and states:**
...
Again, regarding the PDFs of this guide and as explained in the README of my repository, you could check for anomalies using PDFID which you can download at <https://blog.didierstevens.com/programs/pdf-tools/> <sup>[[Archive.org]][565]</sup>
...
-   **Qubes OS:** Consider using <https://github.com/QubesOS/qubes-app-linux-pdf-converter> <sup>[[Archive.org]][566]</sup> which will convert your PDF into a flattened image file. This should theoretically remove any malicious code in it. Note that this will also render the PDF formatting useless (such as links, headings, bookmarks, and references).

-   **(Deprecated) Linux/Qubes OS** (or possibly MacOS through Homebrew or Windows through Cygwin): Consider not using <https://github.com/firstlookmedia/pdf-redact-tools> <sup>[[Archive.org]][567]</sup> which will also turn your PDF into a flattened image file. Again, this should theoretically remove any malicious code in it. Again, this will also render the PDF formatting useless (such as links, headings, bookmarks, and references). **Note that this tool is deprecated and relies on a library called "ImageMagick" which is known for several security issues**[^411]**. You should not use this tool even if it is recommended in some other guides.**

-   **Windows/Linux/Qubes/OS/MacOS:** Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][568]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^412]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

#### Other type of files:

Here are some various resources for this purpose where you will find what tool to use for what type:

-   For Documents/Pictures: Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][568]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^413]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

```{=html}
<!-- -->
```
-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> <sup>[[Archive.org]][569]</sup> (warning, many of those tools might be harder to use on Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^414] that might be of use).

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis> <sup>[[Archive.org]][563]</sup>

-   This interesting PDF detailing which tool to use for which file type <https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf> <sup>[[Archive.org]][570]</sup>
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^415]. Those apps do not require installation your system and can be run from an USB key or anywhere else.
...
Another alternative if you do not trust Brave would be to use Ungoogled-Chromium (<https://github.com/Eloston/ungoogled-chromium> <sup>[[Archive.org]][53]</sup>).

# Appendix W: Virtualization

So, you might ask yourself, what is Virtualization[^416]?
...
In those cases, it might be necessary to use Tor bridges to connect to the Tor network (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][203]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][286]</sup>).

Bridges are special Tor entry nodes that are not listed on the Tor public directory. Some of those are running on people running the Snowflake Browser extension[^417] while others are running on various servers around the world. Most of those bridges are running some type of obfuscation method called obfs4[^418].

Here is the definition from the Tor Browser Manual[^419]: "obfs4 makes Tor traffic look random, and also prevents censors from finding bridges by Internet scanning. obfs4 bridges are less likely to be blocked than its predecessor, obfs3 bridges".
...
As per their definition from their manual[^427]: "meek transports make it look like you are browsing a major web site instead of using Tor. meek-azure makes it look like you are using a Microsoft web site".

First you should, proceed with the following checklist to make sure you cannot circumvent Tor Blocking (double-check) and try to use Tor Bridges (<https://bridges.torproject.org/> <sup>[[Archive.org]][487]</sup>).
...
[^1]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf> <sup>[[Archive.org]][573]</sup>. Section 13, Article 6, "The service provider must enable the use of Telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^2]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany> <sup>[[Archive.org]][346]</sup>

[^3]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil> <sup>[[Archive.org]][574]</sup>

[^4]: YouTube, <https://www.youtube.com/watch?v=6DGNZnfKYnU> <sup>[[Invidious]][575]</sup>

[^5]: Wikipedia, OSINT <https://en.wikipedia.org/wiki/Open-source_intelligence> <sup>[[Archive.org]][576]</sup>

[^6]: YouTube Internet Historian Playlist, HWNDU <https://www.youtube.com/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY> <sup>[[Invidious]][577]</sup>

[^7]: Wikipedia, 4chan <https://en.wikipedia.org/wiki/4chan> <sup>[[Archive.org]][578]</sup>

[^8]: PIA, See this good article on the matter <https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/> <sup>[[Archive.org]][579]</sup> (disclaimer: this is not an endorsement or recommendation for this commercial service).
...
[^10]: This World of Ours, James Mickens <https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf> <sup>[[Archive.org]][580]</sup>

[^11]: XKCD, Security <https://xkcd.com/538/> <sup>[[Archive.org]][581]</sup>

[^12]: Wikipedia, Threat Model <https://en.wikipedia.org/wiki/Threat_model> <sup>[[Archive.org]][582]</sup>

[^13]: Bellingcat <https://www.bellingcat.com/> <sup>[[Archive.org]][583]</sup>

[^14]: Wikipedia, Doxing <https://en.wikipedia.org/wiki/Doxing> <sup>[[Archive.org]][584]</sup>

[^15]: YouTube, Internet Historian, The Bikelock Fugitive of Berkeley <https://www.youtube.com/watch?v=muoR8Td44UE> <sup>[[Invidious]][585]</sup>

[^16]: BBC News, Tor Mirror <https://www.bbc.com/news/technology-50150981> <sup>[[Archive.org]][586]</sup>

[^17]: GitHub, Real World Onion websites <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][389]</sup>

[^18]: Tor Project, Who Uses Tor <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][587]</sup>

[^19]: Whonix Documentation, The importance of Anonymity <https://www.whonix.org/wiki/Anonymity> <sup>[[Archive.org]][588]</sup>

[^20]: Geek Feminism, <https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F> <sup>[[Archive.org]][589]</sup>

[^21]: Tor Project, Tor Users <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][587]</sup>

[^22]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL> <sup>[[Archive.org]][590]</sup>

[^23]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][591]</sup>

[^24]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address> <sup>[[Archive.org]][592]</sup>

[^25]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention> <sup>[[Archive.org]][593]</sup>

[^26]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)> <sup>[[Archive.org]][594]</sup>

[^27]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network> <sup>[[Archive.org]][595]</sup>

[^28]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System> <sup>[[Archive.org]][596]</sup>

[^29]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking> <sup>[[Archive.org]][597]</sup>

[^30]: CensoredPlanet <https://censoredplanet.org/> <sup>[[Archive.org]][191]</sup>

[^31]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf> <sup>[[Archive.org]][598]</sup>

[^32]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole> <sup>[[Archive.org]][599]</sup>

[^33]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS> <sup>[[Archive.org]][600]</sup>

[^34]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS> <sup>[[Archive.org]][601]</sup>

[^35]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole> <sup>[[Archive.org]][602]</sup>

[^36]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication> <sup>[[Archive.org]][603]</sup>

[^37]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_H
: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix> <sup>[[Archive.org]][822]</sup>

[^283]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html> <sup>[[Archive.org]][823]</sup>

[^284]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf> <sup>[[Archive.org]][824]</sup>

[^285]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)> <sup>[[Archive.org]][825]</sup>

[^286]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)> <sup>[[Archive.org]][826]</sup>

[^287]: Whonix Documentation, Stream Isolation, By Settings <https://www.whonix.org/wiki/Stream_Isolation#By_Settings> <sup>[[Archive.org]][827]</sup>

[^288]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Archive.org]][828]</sup>

[^289]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Archive.org]][829]</sup>

[^290]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/> <sup>[[Archive.org]][830]</sup>

[^291]: Whonix Documentation, Stream Isolation <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][265]</sup>

[^292]: Whonix Documentation, Tunnels Comparison Table, <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][267]</sup>

[^293]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk #2414 <https://github.com/QubesOS/qubes-issues/issues/2414> <sup>[[Archive.org]][831]</sup>

[^294]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Archive.org]][828]</sup>

[^295]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Archive.org]][829]</sup>

[^296]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA> <sup>[[Archive.org]][832]</sup>

[^297]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test> <sup>[[Archive.org]][833]</sup>

[^298]: Google reCaptcha <https://www.google.com/recaptcha/about/> <sup>[[Archive.org]][834]</sup>

[^299]: hCaptcha <https://www.hcaptcha.com/> <sup>[[Archive.org]][835]</sup>

[^300]: hCaptcha, hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service> <sup>[[Archive.org]][836]</sup>

[^301]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/> <sup>[[Archive.org]][837]</sup>

[^302]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/> <sup>[[Archive.org]][838]</sup>

[^303]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf> <sup>[[Archive.org]][839]</sup>

[^304]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html> <sup>[[Archive.org]][840]</sup>

[^305]: Tor Project Community, Cloudflare Captcha Monitoring <https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/> <sup>[[Archive.org]][841]</sup>

[^306]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/> <sup>[[Archive.org]][842]</sup>

[^307]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][783]</sup>

[^308]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint> <sup>[[Archive.org]][843]</sup>

[^309]: Developers Google Blog,

    Guidance to developers affected by our effort to block less secure browsers and applications <https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html> <sup>[[Archive.org]][844]</sup>

[^310]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085> <sup>[[Archive.org]][845]</sup>

[^311]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern> <sup>[[Archive.org]][846]</sup>

[^312]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight> <sup>[[Archive.org]][847]</sup>

[^313]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html> <sup>[[Archive.org]][848]</sup>

[^314]: Vice.com, PornHub Announces 'Biometric Technology' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id> <sup>[[Archive.org]][849]</sup>

[^315]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][591]</sup>

[^316]: Wikipedia, Zero-trust Security Model <https://en.wikipedia.org/wiki/Zero_trust_security_model> <sup>[[Archive.org]][850]</sup>

[^317]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization> <sup>[[Archive.org]][851]</sup>

[^318]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam> <sup>[[Archive.org]][852]</sup>

[^319]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration> <sup>[[Archive.org]][853]</sup>

[^320]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/> <sup>[[Archive.org]][854]</sup>

[^321]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^322]: Google Help <https://support.google.com/accounts/answer/114129?hl=en> <sup>[[Archive.org]][855]</sup>

[^323]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en> <sup>[[Archive.org]][856]</sup>

[^324]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695> <sup>[[Archive.org]][857]</sup>

[^325]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2> <sup>[[Archive.org]][858]</sup>

[^326]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en> <sup>[[Archive.org]][859]</sup>

[^327]: Jumio, ID verification features <https://www.jumio.com/features/> <sup>[[Archive.org]][860]</sup>

[^328]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/> <sup>[[Archive.org]][861]</sup>

[^329]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/][] <sup>[[Archive.org]][862]</sup>

[^330]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^331]: KnowYourMeme, Good Luck, I'm Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies> <sup>[[Archive.org]][863]</sup>

[^332]: Wikipedia, end-to-end encryption, <https://en.wikipedia.org/wiki/End-to-end_encryption> <sup>[[Archive.org]][864]</sup>

[^333]: Wikipedia, Forward Secrecy, <https://en.wikipedia.org/wiki/Forward_secrecy> <sup>[[Archive.org]][865]</sup>

[^334]: Protonblog, What is zero-access encryption and why it is important for security <https://protonmail.com/blog/zero-access-encryption/> <sup>[[Archive.org]][866]</sup>

[^335]: Wikipedia, Cambridge Analytica Scandal, <https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal> <sup>[[Archive.org]][867]</sup>

[^336]: Signal Blog, Technology preview: Sealed sender for Signal <https://signal.org/blog/sealed-sender/> <sup>[[Archive.org]][868]</sup>

[^337]: Signal Blog, Private Contact Discovery, <https://signal.org/blog/private-contact-discovery/> <sup>[[Archive.org]][869]</sup>

[^338]: Signal Blog, Private Group System, <https://signal.org/blog/signal-private-group-system/> <sup>[[Archive.org]][870]</sup>

[^339]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/> <sup>[[Archive.org]][871]</sup>

[^340]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/> <sup>[[Archive.org]][872]</sup>

[^341]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/> <sup>[[Archive.org]][873]</sup>

[^342]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop> <sup>[[Archive.org]][874]</sup>

[^343]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module> <sup>[[Archive.org]][875]</sup>

[^344]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling> <sup>[[Archive.org]][876]</sup>

[^345]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM> <sup>[[Archive.org]][877]</sup>

[^346]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification> <sup>[[Archive.org]][877]</sup>

[^347]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages> <sup>[[Archive.org]][403]</sup>

[^348]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection> <sup>[[Archive.org]][877]</sup>

[^349]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/> <sup>[[Archive.org]][878]</sup>

[^350]: ResearchGate, Live forensics method for acquisition on the Solid-State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function> <sup>[[Archive.org]][879]</sup>

[^351]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/> <sup>[[Archive.org]][880]</sup>

[^352]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/> <sup>[[Archive.org]][881]</sup>

[^353]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand> <sup>[[Archive.org]][882]</sup>

[^354]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic> <sup>[[Archive.org]][883]</sup>

[^355]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm> <sup>[[Archive.org]][884]</sup>

[^356]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli> <sup>[[Archive.org]][885]</sup>

[^357]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/> <sup>[[Archive.org]][886]</sup>

[^358]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/> <sup>[[Archive.org]][887]</sup>

[^359]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php> <sup>[[Archive.org]][888]</sup>

[^360]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html> <sup>[[Archive.org]][889]</sup>

[^361]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][424]</sup>

[^362]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/> <sup>[[Archive.org]][890]</sup>

[^363]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata> <sup>[[Archive.org]][891]</sup>

[^364]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/> <sup>[[Archive.org]][892]</sup>

[^365]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/> <sup>[[Archive.org]][893]</sup>

[^366]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/> <sup>[[Archive.org]][894]</sup>

[^367]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop> <sup>[[Archive.org]][895]</sup>

[^368]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication> <sup>[[Archive.org]][896]</sup>

[^369]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Archive.org]][679]</sup>

[^370]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography> <sup>[[Archive.org]][897]</sup>

[^371]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang> <sup>[[Archive.org]][898]</sup>

[^372]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/> <sup>[[Archive.org]][899]</sup>

[^373]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/> <sup>[[Archive.org]][796]</sup>

[^374]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-> <sup>[[Archive.org]][900]</sup>

[^375]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e> <sup>[[Archive.org]][901]</sup>

[^376]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375> <sup>[[Archive.org]][902]</sup>

[^377]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/> <sup>[[Archive.org]][903]</sup>

[^378]: DBAN, <https://dban.org/> <sup>[[Archive.org]][904]</sup>

[^379]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html> <sup>[[Archive.org]][784]</sup>

[^380]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/> <sup>[[Archive.org]][905]</sup>

[^381]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage> <sup>[[Archive.org]][906]</sup>

[^382]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf> <sup>[[Archive.org]][907]</sup>

[^383]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M> <sup>[[Invidious]][908]</sup>

[^384]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA> <sup>[[Archive.org]][909]</sup>

[^385]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps> <sup>[[Archive.org]][910]</sup>

[^386]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection> <sup>[[Archive.org]][911]</sup>

[^387]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification> <sup>[[Archive.org]][912]</sup>

[^388]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check> <sup>[[Archive.org]][913]</sup>

[^389]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5> <sup>[[Archive.org]][914]</sup>

[^390]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security> <sup>[[Archive.org]][914]</sup>

[^391]: Wikipedia, Collisions <https://en.wikipedia.org/wiki/Collision_(computer_science)> <sup>[[Archive.org]][915]</sup>

[^392]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms> <sup>[[Archive.org]][916]</sup>

[^393]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2> <sup>[[Archive.org]][917]</sup>

[^394]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance> <sup>[[Archive.org]][918]</sup>

[^395]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity> <sup>[[Archive.org]][919]</sup>

[^396]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f> <sup>[[Archive.org]][920]</sup>

[^397]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard> <sup>[[Archive.org]][921]</sup>

[^398]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography> <sup>[[Archive.org]][922]</sup>

[^399]: Wikipedia, Polymorphic Code <https://en.wikipedia.org/wiki/Polymorphic_code> <sup>[[Archive.org]][923]</sup>

[^400]: Whonix Documentation, Use of AV, <https://www.whonix.org/wiki/Malware_and_Firmware_Trojans#The_Utility_of_Antivirus_Tools> <sup>[[Archive.org]][924]</sup>

[^401]: Whonix Forums, <https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8> <sup>[[Archive.org]][925]</sup>

[^402]: AV-Test Security Report 2018-2019, <https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf> <sup>[[Archive.org]][926]</sup>

[^403]: ZDNet, ESET discovers 21 new Linux malware families <https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/> <sup>[[Archive.org]][927]</sup>

[^404]: NakeSecurity, EvilGnome -- Linux malware aimed at your desktop, not your servers <https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/> <sup>[[Archive.org]][928]</sup>

[^405]: Immunify, HiddenWasp: How to detect malware hidden on Linux & IoT <https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot> <sup>[[Archive.org]][929]</sup>

[^406]: Wikipedia, Linux Malware <https://en.wikipedia.org/wiki/Linux_malware> <sup>[[Archive.org]][930]</sup>

[^407]: Wikipedia, MacOS Malware <https://en.wikipedia.org/wiki/MacOS_malware> <sup>[[Archive.org]][931]</sup>

[^408]: MacWorld, List of Mac viruses, malware and security flaws <https://www.macworld.co.uk/feature/mac-viruses-list-3668354/> <sup>[[Archive.org]][932]</sup>

[^409]: JAMF, The Mac Malware of 2020 <https://resources.jamf.com/documents/macmalware-2020.pdf> <sup>[[Archive.org]][933]</sup>

[^410]: MacOS Security and Privacy Guide, <https://github.com/drduh/macOS-Security-and-Privacy-Guide#viruses-and-malware> <sup>[[Archive.org]][247]</sup>

[^411]: ImageTragick.com, <https://imagetragick.com/> <sup>[[Archive.org]][934]</sup>

[^412]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][935]</sup>

[^413]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][935]</sup>

[^414]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/> <sup>[[Archive.org]][936]</sup>

[^415]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application> <sup>[[Archive.org]][937]</sup>

[^416]: Wikipedia, Virtualization <https://en.wikipedia.org/wiki/Virtualization> <sup>[[Archive.org]][938]</sup>

[^417]: Tor Project, Project Snowflake <https://snowflake.torproject.org/> <sup>[[Archive.org]][939]</sup>

[^418]: GitHub, Obfs4 Repository <https://github.com/Yawning/obfs4/> <sup>[[Archive.org]][940]</sup>

[^419]: Tor Browser Manual, Pluggable Transport <https://tb-manual.torproject.org/circumvention/> <sup>[[Archive.org]][941]</sup>
  ...
  [22]: https://web.archive.org/web/https://creativecommons.org/licenses/by/4.0/
  ...
  [25]: https://web.archive.org/web/https://mirror.anonymousplanet.org/
  [26]: https://archive.fo/mirror.anonymousplanet.org/guide.html
  [27]: https://mirror.anonymousplanet.org/guide.pdf
  [28]: https://web.archive.org/web/https://anonymousplanet.org/guide.pdf
  [29]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/guide.pdf
  [30]: https://mirror.anonymousplanet.org/guide-dark.pdf
  [31]: https://web.archive.org/web/https://anonymousplanet.org/guide-dark.pdf
://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm
  [208]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis
  [209]: https://web.archive.org/web/https://github.com/aforensics/HiddenVM/releases
  [210]: https://web.archive.org/web/https://www.whonix.org/wiki/VirtualBox/XFCE
  [211]: https://web.archive.org/web/https://puri.sm/
  [212]: https://web.archive.org/web/https://system76.com/
  [213]: https://web.archive.org/web/https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/
  [214]: https://web.archive.org/web/https://libreboot.org/docs/hardware/
  [215]: https://web.archive.org/web/https://coreboot.org/status/board-status.html
  [216]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs
  [217]: https://yewtu.be/watch?v=QDSlWa9xQuA
  [218]: https://yewtu.be/watch?v=0fZdL3ufVOI
  [219]: https://web.archive.org/web/https://support.apple.com/en-au/HT204455
  [220]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/security/sec28382c9ca/web
  [221]: https://web.archive.org/web/https://mullvad.net/en/help/how-tamper-protect-laptop/
  [222]: media/image18.jpeg
  [223]: media/image19.jpeg
  [224]: https://web.archive.org/web/https://www.whonix.org/wiki/Cold_Boot_Attack_Defense
  [225]: https://web.archive.org/web/https://www.whonix.org/wiki/Protection_Against_Physical_Attacks
  [226]: https://web.archive.org/web/https://github.com/0xPoly/Centry
  [227]: https://web.archive.org/web/https://github.com/AnonymousPlanet/Centry
  [228]: https://web.archive.org/web/https://github.com/hephaest0s/usbkill
  [229]: https://web.archive.org/web/https://github.com/Lvl4Sword/Killer
  [230]: https://web.archive.org/web/https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks
  [231]: https://web.archive.org/web/https://github.com/QubesOS/qubes-antievilmaid
  [232]: https://web.archive.org/web/https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download
  [233]: https://web.archive.org/web/https://www.whonix.org/wiki/Full_Disk_Encryption
  [234]: https://web.archive.org/web/https://madaidans-insecurities.github.io/linux.html
  [235]: https://web.archive.org/web/https://ubuntu.com/tutorials/install-ubuntu-desktop
  [236]: https://web.archive.org/web/https://help.ubuntu.com/community/ManualFullSystemEncryption
  [237]: https://web.archive.org/web/https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/
  [238]: https://web.archive.org/web/https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/
  [239]: https://web.archive.org/web/https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html
  [240]: https://web.archive.org/web/https://help.ubuntu.com/16.04/ubuntu-help/power-hibernate.html
  [241]: https://web.archive.org/web/http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/
  [242]: https://web.archive.org/web/https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/
  [243]: https://web.archive.org/web/https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap
  [244]: https://web.archive.org/web/https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses
  [245]: https://web.archive.org/web/https://josh.works/shell-script-basics-change-mac-address
  [246]: https://web.archive.org/web/https://madaidans-insecurities.github.io/guides/linux-hardening.html
  [247]: https://web.archive.org/web/https://github.com/drduh/macOS-Security-and-Privacy-Guide
  [248]: https://web.archive.org/web/https://support.apple.com/en-us/HT204455
  [249]: https://web.archive.org/web/https://sneak.berlin/20201112/your-computer-isnt-yours/
  [250]: https://web.archive.org/web/https://blog.jacopo.io/en/post/apple-ocsp/
  [251]: https://web.archive.org/web/https://technitium.com/tmac/
  [252]: https://web.archive.org/web/https://www.veracrypt.fr/en/Downloads.html
  [253]: https://web.archive.org/web/https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation
  [254]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html
  [255]: https://web.archive.org/web/https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html
  [256]: media/image20.jpeg
  [257]: https://web.archive.org/web/https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html
  [258]: https://web.archive.org/web/https://www.whonix.org/wiki/KVM
  [259]: media/image21.jpg 
  [260]: media/image22.jpg 
  [261]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor
  [262]: media/image23.jpg 
  [263]: media/image24.jpg 
  [264]: Anonymized#_Your_
  [265]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation
  [266]: https://web.archive.org/web/https://tails.boum.org/contribute/design/stream_isolation/
  [267]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table
  ...
  [279]: media/image25.jpg 
  [280]: media/image26.jpg 
  ...
  [286]: https://web.archive.org/web/https://www.whonix.org/wiki/Bridges
  [287]: https://web.archive.org/web/https://www.whonix.org/wiki/Operating_System_Software_and_Updates
  [288]: https://web.archive.org/web/https://www.whonix.org/wiki/DoNot
  [289]: https://web.archive.org/web/https://www.whonix.org/wiki/Documentation
  [290]: https://web.archive.org/web/https://www.whonix.org/wiki/Install_Software
  [291]: https://web.archive.org/web/https://www.whonix.org/wiki/Anti-Forensics_Precautions
  [292]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Fingerprinting
  [293]: https://web.archive.org/web/https://www.whonix.org/wiki/Other_Operating_Systems
  [294]: https://web.archive.org/web/https://brave.com/download/
  [295]: https://web.archive.org/web/https://www.android-x86.org/documentation/virtualbox.html
  [296]: https://web.archive.org/web/https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/
  [297]: https://web.archive.org/web/https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/
  [298]: https://web.archive.org/web/https://github.com/myspaghetti/macos-virtualbox
  [https://www.whonix.org/wiki/KeePassXC]: https://www.whonix.org/wiki/Keepassxc
  [299]: https://web.archive.org/web/https://www.whonix.org/wiki/Keepassxc
  [300]: https://web.archive.org/web/https://keepassxc.org/download/
  [301]: https://web.archive.org/web/https://keepassxc.org/docs/KeePassXC_GettingStarted.html
  ...
  [302]: https://web.archive.org/web/https://www.qubes-os.org/intro/
  [303]: https://web.archive.org/web/https://www.qubes-os.org/video-tours/
: https://web.archive.org/web/https://support.apple.com/en-ie/guide/disk-utility/dskutl1010/mac
  [396]: https://web.archive.org/web/https://www.getmonero.org/community/merchants/
  [397]: https://web.archive.org/web/https://privacytools.io/providers/hosting/
  [398]: https://web.archive.org/web/https://njal.la/
  [399]: https://web.archive.org/web/https://syncthing.net/
  [400]: media/image35.jpeg
  [401]: media/image36.jpeg
  [402]: media/image37.jpeg
  [403]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trim_(computing)
  [404]: media/image38.jpeg
  [405]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive
  [406]: https://web.archive.org/web/https://www.bleachbit.org/download/linux
  [407]: https://web.archive.org/web/https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux
  [408]: https://web.archive.org/web/https://linuxhint.com/completely_wipe_hard_drive_ubuntu/
  [409]: https://web.archive.org/web/https://linoxide.com/linux-command/commands-wipe-disk-linux/
  [410]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Securely_wipe_disk
  [411]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-linux
  [Extra Tools Cleaning:]: #extra-tools-cleaning
  [412]: https://web.archive.org/web/https://eraser.heidi.ie/download/
  [413]: https://web.archive.org/web/http://killdisk.com/killdisk-freeware.htm
  [414]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac
  [415]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac
  [416]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac
  [417]: media/image39.jpeg
  [418]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-windows
  [419]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption
  [420]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md
  [421]: https://web.archive.org/web/https://support.apple.com/en-us/HT210898
  [422]: media/image40.jpeg
  [423]: https://web.archive.org/web/http://www.edenwaith.com/products/permanent%20eraser/
  [424]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-macos
  [425]: https://web.archive.org/web/https://www.whonix.org/wiki/System_Hardening_Checklist
  [426]: https://web.archive.org/web/https://exiftool.org/
  [427]: https://web.archive.org/web/https://exifcleaner.com/
  [428]: https://web.archive.org/web/https://www.purevpn.com/internet-privacy/how-to-remove-metadata-from-photos
  [429]: https://web.archive.org/web/https://sandlab.cs.uchicago.edu/fawkes/
  [430]: https://web.archive.org/web/https://github.com/Shawn-Shan/fawkes
  [431]: https://web.archive.org/web/https://lowkey.umiacs.umd.edu/
  [432]: https://web.archive.org/web/https://adversarial.io/
  [433]: https://web.archive.org/web/https://github.com/kanzure/pdfparanoia
  [434]: https://web.archive.org/web/https://support.microsoft.com/en-us/office/remove-hidden-data-and-personal-information-by-inspecting-documents-presentations-or-workbooks-356b7b5d-77af-44fe-a07f-9aa4d085966f
  [435]: https://web.archive.org/web/https://0xacab.org/jvoisin/mat2
  [436]: media/image41.JPG 
  [437]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Live_Mode
  [438]: https://web.archive.org/web/https://www.titanium-software.fr/en/onyx.html
  [439]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/security/security-guidelines.md
  [440]: https://web.archive.org/web/https://www.whonix.org/wiki/Qubes/AppArmor
  [441]: https://web.archive.org/web/https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG
  [442]: https://web.archive.org/web/https://github.com/sundowndev/go-covermyass
  [443]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11
  [444]: https://web.archive.org/web/https://privazer.com/en/download-shellbag-analyzer-shellbag-cleaner.php
  [445]: https://web.archive.org/web/https://privazer.com/en/download.php
  [446]: https://web.archive.org/web/https://www.bleachbit.org/download
  [447]: https://web.archive.org/web/https://justdeleteme.xyz/
  [448]: https://web.archive.org/web/https://justgetmydata.com/
  [449]: https://web.archive.org/web/https://search.google.com/search-console/remove-outdated-content
  [450]: https://web.archive.org/web/https://www.bing.com/webmasters/tools/contentremoval
  [451]: https://web.archive.org/web/https://webmaster.yandex.com/
  [452]: https://web.archive.org/web/https://webmaster.yandex.com/tools/del-url/
  [453]: https://web.archive.org/web/https://help.yahoo.com/kb/SLN4530.html
  [454]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wikipedia:Courtesy_vanishing
  [455]: https://web.archive.org/web/https://guardianproject.github.io/haven/
  [456]: https://web.archive.org/web/https://www.google.com/alerts
  [457]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Ross_Ulbricht
  [458]: https://yewtu.be/watch?v=d-7o9xYp7eE
  [459]: https://yewtu.be/watch?v=eQ2OZKitRwc
  ...
  [460]: https://web.archive.org/web/https://www.defcon.org/images/defcon-22/dc-22-presentations/Crenshaw/DEFCON-22-Adrian-Crenshaw-Dropping-Docs-on-Darknets-How-People-Got-Caught-UPDATED.pdf
  [461]: https://yewtu.be/watch?v=GR_U0G-QGA0
  [462]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Key_disclosure_law
  [463]: https://web.archive.org/web/https://www.gp-digital.org/world-map-of-encryption/
  [464]: https://web.archive.org/web/https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/
  [465]: https://mirror.anonymousplanet.org/donations.html
  [466]: https://web.archive.org/web/https://anonymousplanet.org/donations.html
  [467]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/donations.html
  [468]: media/image42.jpg 
  [469]: media/image43.jpeg 
  [470]: media/image44.jpeg 
  [471]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)
  [472]: https://web.archive.org/web/https://www.w10privacy.de/english-home/
  [473]: https://web.archive.org/web/https://crazymax.dev/WindowsSpyBlocker/download/
  [474]: https://web.archive.org/web/https://www.microsoft.com/en-us/software-download/windows10
  [475]: https://web.archive.org/web/https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/
  [476]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing
  [477]: https://web.archive.org/web/https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase
  [478]: https://web.archive.org/web/https://tinyapps.org/docs/wipe_drives_hdparm.html
  [479]: https://web.archive.org/web/https://tinyapps.org/docs/ata_sanitize_hdparm.html
  [480]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-secure-erase.html
  [481]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-sanitize.html
  [482]: https://web.archive.org/web/https://clonezilla.org/liveusb.php
  [483]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image
  [484]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image
  [485]: https://web.archive.org/web/https://www.torproject.org/download/
  [486]: https://web.archive.org/web/https://tb-manual.torproject.org/security-settings/
  [487]: https://web.archive.org/web/https://bridges.torproject.org/
  [488]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68
  [489]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a
  [490]: https://web.archive.org/web/https://www.bleachbit.org/
  [491]: https://web.archive.org/web/https://privazer.com/
  [492]: https://web.archive.org/web/https://www.system-rescue.org/
  [493]: https://web.archive.org/web/https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148
  [494]: https://web.archive.org/web/https://github.com/PartialVolume/shredos.2020.02
  [495]: https://web.archive.org/web/https://rufus.ie/
  [496]: https://web.archive.org/web/https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information
  [497]: https://web.archive.org/web/https://www.seagate.com/support/downloads/seatools/
  [498]: https://web.archive.org/web/https://www.samsung.com/semiconductor/minisite/ssd/download/tools/
  [499]: https://web.archive.org/web/https://www.kingston.com/unitedstates/en/support/technical/ssdmanager
  [500]: https://web.archive.org/web/https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad
  [501]: https://web.archive.org/web/https://www.crucial.com/support/storage-executive
  [502]: https://web.archive.org/web/https://support.wdc.com/downloads.aspx?p=279
  [503]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd
  [504]: https://web.archive.org/web/https://www.transcend-info.com/Support/Software-10/
  [505]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt
  [506]: https://web.archive.org/web/https://www.toshiba-storage.com/downloads/
  [507]: https://web.archive.org/web/https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux
  [508]: https://web.archive.org/web/https://code.mendhak.com/securely-wipe-ssd/
  [509]: https://web.archive.org/web/https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789
  [510]: https://web.archive.org/web/https://cindori.org/trimenabler/
  [511]: https://web.archive.org/web/https://www.debian.org/CD/netinst/
  [512]: https://web.archive.org/web/https://support.lenovo.com/be/en/solutions/migr-68369
  [513]: https://web.archive.org/web/https://support.hp.com/gb-en/document/c06204100
  [514]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe
  [515]: https://web.archive.org/web/https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks
  [516]: https://web.archive.org/web/https://en.wikipedia.org/wiki/EncroChat
  [517]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sky_ECC
  [518]: https://web.archive.org/web/https://privacytools.io/providers/vpn/
  [519]: https://web.archive.org/web/https://evolution-host.com/vps-hosting.php
  [520]: https://web.archive.org/web/https://proprivacy.com/vpn/guides/create-your-own-vpn-server
  [521]: https://web.archive.org/web/https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/
  [522]: https://web.archive.org/web/https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel
  [523]: https://web.archive.org/web/https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/
  [524]: https://web.archive.org/web/https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/
  [525]: https://web.archive.org/web/https://www.putty.org/
  [526]: media/image45.jpeg 
  [527]: https://web.archive.org/web/https://tails.boum.org/contribute/design/Unsafe_Browser/
  [528]: https://web.archive.org/web/https://archive.flossmanuals.net/bypassing-censorship/index.html
  [529]: media/image46.jpg 
  [530]: https://web.archive.org/web/https://www.alfa.com.tw/
  [531]: https://web.archive.org/web/https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/
  [532]: https://web.archive.org/web/https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html
  [533]: https://yewtu.be/watch?v=8FV2QZ1BPnw
  [534]: https://web.archive.org/web/https://www.netally.com/products/
  [535]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-windows/
  [536]: https://web.archive.org/web/https://www.ivpn.net/apps-windows
  [537]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-windows-vpn-application/
  [538]: https://web.archive.org/web/https://mullvad.net/en/help/install-and-use-mullvad-app-macos/
  [539]: https://web.archive.org/web/https://www.ivpn.net/apps-macos/
  [540]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-mac-vpn-application/
  [541]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-linux/
  [542]: https://web.archive.org/web/https://www.ivpn.net/apps-linux/
  [543]: https://web.archive.org/web/https://protonvpn.com/support/linux-vpn-setup/
  [544]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/
  [545]: https://web.archive.org/web/https://protonvpn.com/support/what-is-kill-switch/
  [546]: https://web.archive.org/web/https://mullvad.net/en/help/using-mullvad-vpn-app/
  [547]: https://web.archive.org/web/https://protonvpn.com/blog/macos-vpn-kill-switch/
  [548]: https://web.archive.org/web/https://mullvad.net/en/help/wireguard-and-mullvad-vpn/
  [549]: https://web.archive.org/web/https://mullvad.net/en/help/linux-openvpn-installation/
  [550]: https://web.archive.org/web/https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md
  [551]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/
  [552]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/
  [553]: https://web.archive.org/web/https://ooni.org/install/
  [554]: https://web.archive.org/web/https://www.gpg4win.org/
  [555]: https://web.archive.org/web/https://gpgtools.org/
  [556]: https://web.archive.org/web/https://support.torproject.org/tbb/how-to-verify-signature/
  [557]: https://web.archive.org/web/https://tails.boum.org/install/vm-download/index.en.html
  [558]: https://web.archive.org/web/https://www.whonix.org/wiki/Verify_the_Whonix_images
  [559]: https://web.archive.org/web/https://www.clamav.net/
  [560]: https://web.archive.org/web/https://github.com/rfxn/linux-malware-detect
  [561]: https://web.archive.org/web/http://www.chkrootkit.org/
  [562]: https://web.archive.org/web/https://developers.virustotal.com/v3.0/docs/search-by-hash
  [563]: https://web.archive.org/web/https://github.com/rshipp/awesome-malware-analysis
  [564]: https://web.archive.org/web/https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy
  [565]: https://web.archive.org/web/https://blog.didierstevens.com/programs/pdf-tools/
  [566]: https://web.archive.org/web/https://github.com/QubesOS/qubes-app-linux-pdf-converter
  [567]: https://web.archive.org/web/https://github.com/firstlookmedia/pdf-redact-tools
  [568]: https://web.archive.org/web/https://github.com/firstlookmedia/dangerzone
  [569]: https://web.archive.org/web/https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf
  [570]: https://web.archive.org/web/https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf
  [571]: media/image47.jpg 
  [572]: media/image48.jpeg 
  [573]: https://web.archive.org/web/https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf
  [574]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Don%27t_be_evil
  [575]: https://yewtu.be/watch?v=6DGNZnfKYnU
  [576]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Open-source_intelligence
  [577]: https://yewtu.be/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY
  [578]: https://web.archive.org/web/https://en.wikipedia.org/wiki/4chan
  [579]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/
  [580]: https://web.archive.org/web/https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf
  [581]: https://web.archive.org/web/https://xkcd.com/538/
  [582]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Threat_model
  [583]: https://web.archive.org/web/https://www.bellingcat.com/
  [584]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Doxing
  [585]: https://yewtu.be/watch?v=muoR8Td44UE
  [586]: https://web.archive.org/web/https://www.bbc.com/news/technology-50150981
  [587]: https://web.archive.org/web/https://2019.www.torproject.org/about/torusers.html.en
  [588]: https://web.archive.org/web/https://www.whonix.org/wiki/Anonymity
  [589]: https://web.archive.org/web/https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F
  [590]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IANAL
  [591]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trust,_but_verify
  [592]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IP_address
  [593]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Data_retention
  [594]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tor_(anonymity_network)
  [595]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_private_network
  [596]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Domain_Name_System
  [597]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_blocking
  [598]: https://web.archive.org/web/https://arxiv.org/pdf/2001.08288.pdf
  [599]: https://web.archive.org/web/https://labzilla.io/blog/force-dns-pihole
  [600]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_HTTPS
  [601]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_TLS
  [602]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pi-hole
  [603]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Server_Name_Indication
  [604]: https://web.archive.org/web/https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/
  [605]: https://web.archive.org/web/https://www.usenix.org/system/files/foci19-paper_chai_0.pdf
  [606]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Content_delivery_network
  [607]: https://web.archive.org/web/https://blog.cloudflare.com/encrypted-client-hello/
  [608]: https://web.archive.org/web/https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/
  [609]: https://web.archive.org/web/https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/
  [610]: https://web.archive.org/web/https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf
  [611]: https://web.archive.org/web/https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries
  [612]: https://web.archive.org/web/https://nymity.ch/tor-dns/
  [613]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity
  [614]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_mobile_subscriber_identity
  [615]: https://web.archive.org/web/https://source.android.com/devices/tech/config/device-identifiers
  [616]: https://web.archive.org/web/https://policies.google.com/privacy/embedded?hl=en-US
  [617]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/
  [618]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/
  [619]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Closed-circuit_television
  [620]: https://web.archive.org/web/https://www.apple.com/legal/transparency/device-requests.html
  [621]: https://web.archive.org/web/https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/
  [622]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IMSI-catcher
  [623]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stingray_phone_tracker
  [624]: https://web.archive.org/web/https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778
  [625]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Man-in-the-middle_attack
  [626]: https://web.archive.org/web/https://shop.puri.sm/shop/librem-5/
  [627]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MAC_address
  [628]: https://web.archive.org/web/https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf
  [629]: https://web.archive.org/web/https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf
  [630]: https://web.archive.org/web/https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf
  [631]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Differential_privacy
  [632]: https://web.archive.org/web/https://support.google.com/accounts/answer/3467281?hl=en
  [633]: https://web.archive.org/web/https://support.apple.com/en-us/HT207056
  [634]: https://web.archive.org/web/https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf
  [635]: https://web.archive.org/web/https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/
  [636]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sensorvault
  [637]: https://web.archive.org/web/https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/
  [638]: https://web.archive.org/web/https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html
  [639]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/
  [640]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Geo-fence_warrant
  [641]: https://web.archive.org/web/https://www.vice.com/en/article/y3g97x/location-data-apps-drone-strikes-iowa-national-guard
  [642]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Room_641A
  [643]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Edward_Snowden
  [644]: https://web.archive.org/web/https://en.wikipedia.org/wiki/XKeyscore
  [645]: https://web.archive.org/web/https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html
  [646]: https://web.archive.org/web/https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)
  [647]: https://web.archive.org/web/https://en.wikipedia.org/wiki/PRISM_(surveillance_program)
  [648]: https://web.archive.org/web/https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/
  [649]: https://web.archive.org/web/https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT
  [650]: https://web.archive.org/web/https://www.zdnet.com/article/apple-data-collection-stored-request/
  [651]: https://web.archive.org/web/https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27
  [652]: https://web.archive.org/web/https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/
  [653]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/
  [654]: https://web.archive.org/web/https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases
  [655]: https://web.archive.org/web/https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/
  [656]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59
  [657]: https://web.archive.org/web/https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/
  [658]: https://web.archive.org/web/https://support.apple.com/en-us/HT210515
  [659]: https://web.archive.org/web/https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/
  [660]: https://web.archive.org/web/https://support.apple.com/en-us/HT204756
  [661]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Bluetooth_Low_Energy
  [662]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Near-field_communication
  [663]: https://web.archive.org/web/https://shop.samsonite.com/accessories/rfid-accessories/
  [664]: https://web.archive.org/web/https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/
  [665]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Exploit_(computer_security)
  [666]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Freedom_Hosting
  [667]: https://web.archive.org/web/https://www.wired.com/2013/09/freedom-hosting-fbi/
  [668]: https://web.archive.org/web/https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach
  [669]: https://web.archive.org/web/https://www.bbc.com/news/blogs-china-blog-48552907
  [670]: https://web.archive.org/web/https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/
  [671]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sandbox_(computer_security)
  [672]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark
  [673]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark
  [674]: https://web.archive.org/web/https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html
  [675]: https://web.archive.org/web/https://dtv.nagra.com/nexguard-forensic-watermarking
  [676]: https://web.archive.org/web/https://www.vobilegroup.com/solutions
  [677]: https://web.archive.org/web/https://www.cinavia.com/languages/english/pages/technology.html
  [678]: https://web.archive.org/web/https://www.imatag.com/
  [679]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Steganography
  [680]: https://web.archive.org/web/https://ieeexplore.ieee.org/document/4428921
  [681]: https://web.archive.org/web/https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140
  [682]: https://web.archive.org/web/https://ieeexplore.ieee.org/abstract/document/1188746
  [683]: https://web.archive.org/web/https://scholar.google.com/scholar?q=source+camera+identification
  [684]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Machine_Identification_Code
  [685]: https://web.archive.org/web/http://seeingyellow.com/
  [686]: https://web.archive.org/web/https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html
  [687]: https://web.archive.org/web/https://distrinet.cs.kuleuven.be/software/tor-wf-dl/
  [688]: https://web.archive.org/web/https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/
  [689]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/
  [690]: https://web.archive.org/web/https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/
  [691]: https://web.archive.org/web/https://www.pnas.org/content/early/2020/11/24/2011893117
  [692]: https://web.archive.org/web/https://arxiv.org/abs/1107.4524
  [693]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/
  [694]: https://web.archive.org/web/https://en.wikipedia.org/wiki/
: https://web.archive.org/web/https://en.wikipedia.org/wiki/Social_engineering_(security)
  [759]: https://web.archive.org/web/https://www.grayshift.com/
  [760]: https://web.archive.org/web/https://securephones.io/main.pdf
  [761]: https://web.archive.org/web/https://loup-vaillant.fr/articles/rolling-your-own-crypto
  [762]: https://web.archive.org/web/https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/
  [763]: https://web.archive.org/web/https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto
  [764]: https://yewtu.be/watch?v=loy84K3AJ5Q
  [765]: https://web.archive.org/web/https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/
  [766]: https://web.archive.org/web/https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369
  [767]: https://web.archive.org/web/https://blog.congruentlabs.co/military-grade-encryption/
  [768]: https://web.archive.org/web/https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588
  [769]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gag_order
  [770]: https://web.archive.org/web/https://en.wikipedia.org/wiki/National_security_letter
  [771]: https://web.archive.org/web/https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html
  [772]: https://web.archive.org/web/https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information
  [773]: https://web.archive.org/web/https://archive.is/XNuVw
  [774]: https://web.archive.org/web/https://archive.is/ag9w4
  [775]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Lavabit
  [776]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Warrant_canary
  [777]: https://web.archive.org/web/https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/
  [778]: https://web.archive.org/web/https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432
  [779]: https://web.archive.org/web/https://en.wikipedia.org/wiki/The_Lives_of_Others
  [780]: https://web.archive.org/web/https://www.wired.com/story/air-gap-researcher-mordechai-guri/
  [781]: https://web.archive.org/web/https://www.nassiben.com/lamphone
  [782]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OONI
  [783]: https://web.archive.org/web/https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws
  [784]: https://web.archive.org/web/https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html
  [785]: https://web.archive.org/web/https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf
  [786]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tails_(operating_system)
  [787]: https://web.archive.org/web/https://www.veracrypt.fr/en/Trim%20Operation.html
  [788]: https://web.archive.org/web/https://www.coreboot.org/
  [789]: https://web.archive.org/web/https://xkcd.com/936/
  [790]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface
  [791]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Booting
  [792]: https://web.archive.org/web/https://www.wired.com/2013/12/better-data-security-nail-polish/
  [793]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_machine
  [794]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Plausible_deniability
  [795]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deniable_encryption
  [796]: https://web.archive.org/web/https://privacytools.io/operating-systems/
  [797]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BitLocker
  [798]: https://web.archive.org/web/https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop
  [799]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Evil_maid_attack
  [800]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cold_boot_attack
  [801]: https://yewtu.be/watch?v=JDaicPIgn9U
  [802]: https://web.archive.org/web/https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems
  [803]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500
  [804]: https://web.archive.org/web/https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/
  [805]: https://web.archive.org/web/https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures
  [806]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545
  [807]: https://web.archive.org/web/https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf
  [808]: https://web.archive.org/web/https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf
  [809]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gatekeeper_(macOS)
  [810]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VeraCrypt
  [811]: https://web.archive.org/web/https://ostif.org/the-veracrypt-audit-results/
  [812]: https://web.archive.org/web/https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html
  [813]: https://web.archive.org/web/https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html
  [814]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html
  [815]: https://web.archive.org/web/https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds
  [816]: https://web.archive.org/web/https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance
  [817]: https://web.archive.org/web/https://veracrypt.eu/en/docs/trim-operation/
  [818]: https://web.archive.org/web/https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf
  [819]: https://web.archive.org/web/http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html
  [820]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VirtualBox
  [821]: https://web.archive.org/web/https://www.virtualbox.org/ticket/17987
  [822]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Whonix
  [823]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html
  [824]: https://web.archive.org/web/https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf
  [825]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)
  [826]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)
  [827]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation#By_Settings
  [828]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm
  [829]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Multi-factor_authentication
  [830]: https://web.archive.org/web/https://www.qubes-os.org/doc/system-requirements/
  [831]: https://web.archive.org/web/https://github.com/QubesOS/qubes-issues/issues/2414
  [832]: https://web.archive.org/web/https://en.wikipedia.org/wiki/CAPTCHA
  [833]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Turing_test
  [834]: https://web.archive.org/web/https://www.google.com/recaptcha/about/
  [835]: https://web.archive.org/web/https://www.hcaptcha.com/
  [836]: https://web.archive.org/web/https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service
  [837]: https://web.archive.org/web/https://nearcyan.com/you-probably-dont-need-recaptcha/
  [838]: https://web.archive.org/web/https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/
  [839]: https://web.archive.org/web/https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf
  [840]: https://web.archive.org/web/https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html
  [841]: https://web.archive.org/web/https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/
  [842]: https://web.archive.org/web/https://blog.cloudflare.com/cloudflare-supports-privacy-pass/
  [843]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Device_fingerprint
  [844]: https://web.archive.org/web/https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html
  [845]: https://web.archive.org/web/https://support.google.com/accounts/answer/10071085
  [846]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dark_pattern
  [847]: https://web.archive.org/web/https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight
  [848]: https://web.archive.org/web/https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html
  [849]: https://web.archive.org/web/https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id
  [850]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Zero_trust_security_model
  [851]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Espionage
  [852]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SIM_swap_scam
  [853]: https://web.archive.org/web/https://www.whonix.org/wiki/Tor
  [854]: https://web.archive.org/web/https://support.torproject.org/tbb/tbb-editing-torrc/
  [855]: https://web.archive.org/web/https://support.google.com/accounts/answer/114129?hl=en
  [856]: https://web.archive.org/web/https://support.google.com/google-ads/answer/7474263?hl=en
  [857]: https://web.archive.org/web/https://support.google.com/accounts/answer/40695
  [858]: https://web.archive.org/web/https://support.google.com/accounts/contact/disabled2
  [859]: https://web.archive.org/web/https://support.google.com/accounts/answer/1333913?hl=en
  [860]: https://web.archive.org/web/https://www.jumio.com/features/
  [861]: https://web.archive.org/web/https://privacytools.io/providers/email/
  ...
  [862]: https://web.archive.org/web/https://protonmail.com/support/knowledge-base/human-verification/
  [863]: https://web.archive.org/web/https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies
  [864]: https://web.archive.org/web/https://en.wikipedia.org/wiki/End-to-end_encryption
  [865]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Forward_secrecy
  [866]: https://web.archive.org/web/https://protonmail.com/blog/zero-access-encryption/
  [867]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
  [868]: https://web.archive.org/web/https://signal.org/blog/sealed-sender/
  [869]: https://web.archive.org/web/https://signal.org/blog/private-contact-discovery/
  [870]: https://web.archive.org/web/https://signal.org/blog/signal-private-group-system/
  [871]: https://web.archive.org/web/https://privacytools.io/software/file-sharing/
  [872]: https://web.archive.org/web/https://privacytools.io/software/real-time-communication/
  [873]: https://web.archive.org/web/https://www.praxisfilms.org/open-letter-from-laura-poitras/
  [874]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SecureDrop
  [875]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trusted_Platform_Module
  [876]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wear_leveling
  [877]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Write_amplification
  [878]: https://web.archive.org/web/https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/
  [879]: https://web.archive.org/web/https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function
  [880]: https://web.archive.org/web/https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/
  [881]: https://web.archive.org/web/https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/
  [882]: https://web.archive.org/web/https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand
  [883]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Parted_Magic
  [884]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Hdparm
  [885]: https://web.archive.org/web/https://github.com/linux-nvme/nvme-cli
  [886]: https://web.archive.org/web/https://partedmagic.com/secure-erase/
  [887]: https://web.archive.org/web/https://partedmagic.com/nvme-secure-erase/
  [888]: https://web.archive.org/web/https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php
  [889]: https://web.archive.org/web/https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html
  [890]: https://web.archive.org/web/https://www.privacytools.io/software/productivity/
  [891]: https://web.archive.org/web/https://www.whonix.org/wiki/Metadata
  [892]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/
  [893]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/
  [894]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/
  [895]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dead_drop
  [896]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Obfuscation
  [897]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Kleptography
  [898]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Koalang
  [899]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/
  [900]: https://web.archive.org/web/https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-
  [901]: https://web.archive.org/web/https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e
  [902]: https://web.archive.org/web/https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375
  [903]: https://web.archive.org/web/https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/
  [904]: https://web.archive.org/web/https://dban.org/
  [905]: https://web.archive.org/web/https://crystalmark.info/en/software/crystaldiskinfo/
  [906]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Faraday_cage
  [907]: https://web.archive.org/web/https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf
  [908]: https://yewtu.be/watch?v=sO98kDLkh-M
  [909]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Touch_DNA
  [910]: https://web.archive.org/web/https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps
  [911]: https://web.archive.org/web/https://github.com/mhinkie/ooni-detection
  [912]: https://web.archive.org/web/https://en.wikipedia.org/wiki/File_verification
  [913]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cyclic_redundancy_check
  [914]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MD5
  [915]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_(computer_science)
  [916]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Secure_Hash_Algorithms
  [917]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SHA-2
  [918]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_resistance
  [919]: https://web.archive.org/web/https://wiki.gnupg.org/Gpg4win/CheckIntegrity
  [920]: https://web.archive.org/web/https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f
  [921]: https://web.archive.org/web/https://en.wikipedia.org/wiki/GNU_Privacy_Guard
  [922]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Public-key_cryptography
  [923]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Polymorphic_code
  [924]: https://web.archive.org/web/https://www.whonix.org/wiki/Malware_and_Firmware_Trojans
  [925]: https://web.archive.org/web/https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8
  [926]: https://web.archive.org/web/https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf
  [927]: https://web.archive.org/web/https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/
  [928]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/
  [929]: https://web.archive.org/web/https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot
  [930]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Linux_malware
  [931]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MacOS_malware
  [932]: https://web.archive.org/web/https://www.macworld.co.uk/feature/mac-viruses-list-3668354/
  [933]: https://web.archive.org/web/https://resources.jamf.com/documents/macmalware-2020.pdf
  [934]: https://web.archive.org/web/https://imagetragick.com/
  [935]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html
  [936]: https://web.archive.org/web/https://zeltser.com/analyzing-malicious-documents/
  [937]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Portable_application
  [938]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtualization
  [939]: https://web.archive.org/web/https://snowflake.torproject.org/
  [940]: https://web.archive.org/web/https://github.com/Yawning/obfs4/
  [941]: https://web.archive.org/web/https://tb-manual.torproject.org/circumvention/
