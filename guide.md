...
Version 0.8.0 (draft), March 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^63].
...
-   IOS/Apple:
    ...
    -   Apple does claim[^64] that they anonymize this data using differential privacy[^65] but you will have to trust them on that.
...
But that is not all those Wi-Fis access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it is possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^68] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE>
...
Your metadata is all the information about your activities without the actual content of those activities. For instance, it is like knowing you had a call from an oncologist before then calling your family and friends successively. You do not know what was said during the conversation but you can guess what it was just from the "metadata"[^69].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^70]/IOS), Browsers, Apps, Websites. Odds are there are several companies knowing exactly where you are at any time[^71] because of your smartphone[^72].

This location data has been used in many judicial cases[^73] already as part of "geofence warrants" [^74] that allows law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time.
...
The question is: Is there someone somewhere that would possibly have both pieces of information available[^75] for correlation in a convenient database?

Have you heard of Edward Snowden[^76]? Now is the time to google him and read his book[^77]. I recommend reading about XKEYSCORE[^78]'[^79], MUSCULAR[^80] and PRISM[^81].

"We kill people based on Metadata"[^82]
...
All this data is very likely being transmitted, processed, and stored indefinitely (unencrypted[^83]) by various third parties[^84].
...
-   Your Fitness Devices and Apps? (Strava[^85]'[^86], Fitbit[^87], Garmin, Polar[^88], ...)[^89]

-   Your Smart Speaker? (Amazon Alexa[^90], Google Echo, Apple Homepod ...).
...
Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^95]. They do not have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^96].
...
RFID stands for Radio-frequency identification[^97], it is the technology used for instance for contactless payments and various identification systems. Of course, your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^98]. As with everything else, such capabilities can be used for tracking by various actors.
...
A real use case of this technique was the Freedom Hosting[^102] case in 2013 where the FBI inserted malware[^103] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^104] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^105] which can then be used in combination with other data for state surveillance[^106].
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^107] without being able to access identifying information or compromise your system.
...
Such watermarks can be inserted by various products[^111]'[^112]'[^113]'[^114] using Steganography[^115] and can resist compression[^116] and re-encoding[^117]'[^118].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^119] which could lead to de-anonymization.
...
-   Do not ever print in Color, usually watermarkings are not present without color toners/cartdriges[^121].
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^122]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^123] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It can do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][32]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^124] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^125] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor can already be considered a suspicious activity[^126] and its use could be considered malicious by some[^127].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^128]. Most crypto currencies can be tracked accurately through various methods[^129].
...
The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You will have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^130] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy/anonymity in mind like Monero but even those have some and warnings to consider[^131]'[^132].

Even if you use Mixers or Tumblers[^133] (services that specialize in "anonymizing" crypto currencies by "mixing them"), keep in mind this is only obfuscation[^134] and not actual anonymity[^135]. Not only are they only obfuscation but they could also put you in trouble as you might end up exchanging your crypto against "dirty" crypto that was used in various questionable contexts[^136].
...
All modern CPUs[^137] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^138] and the AMD Platform Security Processor[^139].
...
These have already been affected by several security vulnerabilities in the past[^140] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^141].

There are some not so easy ways[^142] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you are feeling a bit more adventurous, you could install your own BIOS using Libreboot[^143] or Coreboot[^224] if your laptop supports it.
...
These have been used since at least since 2008 using an attack called "Jasager"[^144] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^145].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance, a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^146]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^147] with the exact same layout as the Wi-Fi you are trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they will themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^148]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^149] in 2005.

While you could inspect an USB key physically, scan it with various utilities, check the various components to see if they are genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^150].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^151], Cellebrite Cloud[^152]) that will help an adversary analyze your cloud data with ease.
...
This is the part where you should watch the documentary "The Social Dilemma"[^153] on Netflix as they cover this topic much better than anyone else IMHO.

This includes is the way you write[^154]'[^155] , the way you behave[^156]'[^157]. The way you click. The way you browse. The fonts you use on your browser[^158]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
-   For example, as a basis of authentication, a user\'s typing speed, keystroke depressions, patterns of error (say accidentally hitting an "l" instead of a "k" on three out of every seven transactions) and mouse movements establishes that person's unique pattern of behavior[^159].

-   This technology is also widely used in CAPTCHAS[^279] services to verify that you are "human" and can be used to fingerprint a user.
...
-   2017, Behavior Analysis in Social Networks, <https://link.springer.com/10.1007/978-1-4614-7163-9_110198-1>

-   2017, Social Networks and Positive and Negative Affect <https://www.sciencedirect.com/science/article/pii/S1877042811013747/pdf?md5=253d8f1bb615d5dee195d353dc077d46&pid=1-s2.0-S1877042811013747-main.pdf>

-   2015, Using Social Networks Data for Behavior and Sentiment Analysis <https://www.researchgate.net/publication/300562034_Using_Social_Networks_Data_for_Behavior_and_Sentiment_Analysis>

-   2016, A Survey on User Behavior Analysis in Social Networks <https://www.academia.edu/30936118/A_Survey_on_User_Behaviour_Analysis_in_Social_Networks>

-   2019, In­fluence and Behavior Analysis in Social Networks and Social Media <https://sci-hub.do/10.1007/978-3-030-02592-2>
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^160] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^161].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>
...
Even if you are not looking at the camera, they can still figure out who you are[^168], make out your emotions[^169], analyze your gait[^170] and probably guess your political affiliation[^171]'[^172].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^173]'[^174]'[^175]'[^176]'[^177].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^178]) in their CCTV networks to look for "persons of interest"[^179]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^180] including possibly identifying ethnic minorities[^181]. A simple face recognition error by some algorithm can ruin your life[^182].
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^183].
...
-   Wear a facemask as they have been proven to defeat some face recognition technologies[^192].
...
Your Browser and Device Fingerprints[^291] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance, websites will be able to provide a "mobile experience" if you are using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^193] based browsers (such as Chrome) or Firefox[^194] unless taking special measures.
...
Phishing[^195] is a social engineering[^196] type of attack where an adversary could try to extract information from you by pretending to be something/someone else.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they are encrypted[^197]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And finally, here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^198]
...
And there are reasons[^199]'[^200] for that:

Personally, I would not want people discouraged from studying and innovating in the crypto field because of that adage. So Instead, I would recommend people to be cautious with "Roll your own crypto" because it's not necessarily good crypto.
...
-   There is no such thing as "military grade crypto"[^201]'[^202]'[^203]
...
-   Telegram: <https://buttondown.email/cryptography-dispatches/archive/cryptography-dispatches-the-most-backdoor-looking/>
...
-   Many other examples can be found here: <https://www.cryptofails.com/>
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^204] or a national security letter[^205]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^206].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^207].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^208].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^209].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^210].

Some providers have implemented the use of a Warrant Canary[^211] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^212] and Omnisec story[^213]).
...
There are many advanced techniques that can be used by skilled adversaries[^214] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^215] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw>
...
Have a look at the Whonix Documentation concerning Data Collection techniques here: <https://www.whonix.org/wiki/Data_Collection_Techniques>
...
-   If you absolutely want system wide plausible deniability[^235]'[^216] despite the risks[^217]'[^237].
...
-   Check some data yourself here on the Tor Project OONI[^218] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/>
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^226]**.**
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^219].
...
Some might be very efficient[^220] but many are gimmicky gadgets that offer no real protection[^221].
...
TAILS[^222] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^223]**.**
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^224] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However, if you are going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^225].
...
-   Enable BIOS/UEFI password and use a long passphrase[^226] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^227]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^228]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^229]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^230] (with pictures).
...
This route will make extensive use of Virtual Machines[^231], they will require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^235] easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^236] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It is the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^237].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^238]'[^239] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
Cold Boot attacks[^242] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^243], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^244] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^245] and by the maintainer of Veracrypt[^246] so I would not worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^248].

-   Windows Indexing keeping traces of the files present in your User folder by default[^249].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^250].
...
-   Gatekeeper[^251] and XProtect keeping track of your download history in a local database and file attributes.
...
Forensics could[^245]'[^248] use all those leaks (see [Local Data Leaks and Forensics:]) to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^252] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^253] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^254] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which does not clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^255] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^256] Operations[^257]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^226] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^264] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^265] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^266] have not be completely fixed to this date[^267] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^268] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^269] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
You will need something to store your data (logins/passwords, identities and TOTP[^273] information).

For this purpose, I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^274] authentication with the authenticator feature.
...
Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^275], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^277] information).

For this purpose, I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^278] authentication with the authenticator feature.
...
Captcha[^279] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^280] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCaptcha service[^281]) and Cloudflare (hCaptcha[^282]). hCaptcha is used on 15% of the internet by their own metrics[^283].

They are designed to separate bots from humans but are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you will quickly encounter many captchas everywhere[^284]. Quite often when using Tor, even if you succeed in solving all the puzzles, you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^285]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^286].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it is "human".

-   They track your behavior before/during/after the tests to ensure you are "human"[^287].
...
You will often experience several in a row (sometimes endless[^288]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You will also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^290].
...
Browser and Device[^291] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^292]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
While this might seem innocent, this is obviously an ID verification and de-anonymization method. This is just indirectly relying on third party financial KYC[^130] regulations.

This is for instance now the case on YouTube for some European Users[^293] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^294] and unfortunately sometimes required.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^295]. Unfortunately, this method is now also sometimes being used on Facebook[^296] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^297].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^298] instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is the real term for this[^299]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience less issues with captchas and online platforms[^292] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^300] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^226]**.**

Here is also a good guide on this specific topic: <https://gendersec.tacticaltech.org/wiki/index.php/Complete_manual#.22Real.22_names>
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^301].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^302].
...
Some platforms are by-passing this requirement all-together by requiring a valid payment method instead (see [Financial transactions:][54]). While this does not directly require a real-name through their ToS, this has the same results as they usually only accept mainstream (not Monero/Cash) payment methods (such as Visa/MasterCard/Maestro or PayPal) which do require a real-name legally as part of their KYC[^130] regulations. The result is the same and arguably even better than a simple real-name policy you could ignore in some countries such as Germany.

### Overview:

This section will show you an overview of the current various requirements on some platforms.

-   **Consider using the recommended tools on <https://privacytools.io/> for your various purposes instead of the usual mainstream ones.**

-   **Consider using the recommended tools on <https://www.whonix.org/wiki/Documentation> as well instead of the usual mainstream ones.**
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^303] will not help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
If your account gets suspended [^306] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^307] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^308].
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^309] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They are recommended by privacytools.io[^310]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^311]. If their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
This time you will need to submit an appeal using a form[^312], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it is possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You are going to have to find a different way to post there using at least seven proxies[^313] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
Instead, you should use better options (as recommend by privacytools.io[^314]'[^315]) such as:
...
If you must do so, you should take some steps because you cannot really trust any organization to protect your anonymity[^316]:
...
For this, I strongly recommend the use of SecureDrop[^317] (<https://securedrop.org/>) which is an open-source project from the Freedom of the Press foundation.
...
So, the fact is that it is very unlikely[^324]'[^325] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^326]'[^327]'[^328] if they are fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^329] for 11\$ and use it to erase any disk.
...
-   If your file system is APFS, you do not need to worry about Trim, it apparently happens asynchronously as the OS writes data[^335] according to their own documentation.
...
Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^336]. So, you are left with using other tools:
...
Consider using <https://github.com/kanzure/pdfparanoia> which will remove metadata and watermarks on any PDF.
...
Alternatively, on Windows, MacOS, Qubes OS, and Linux I would recommend ExifTool (<https://exiftool.org/>) and/or ExifCleaner (<https://exifcleaner.com/>) that allows viewing and/or removing those properties
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^337] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^338] and available on TAILS by default[^339].
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^255]), your network connections, app crash information and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^248] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you should not do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^259]). Secure Deletion with Trim[^256] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^340] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^341] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^342] and Secure Communication Obfuscation[^343] through Steganography[^344] and/or Kleptography[^345] and has many names such as Koalang[^346] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^226]**.**
...
-   Read those tips here <https://www.whonix.org/wiki/DoNot>
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> [^347]
...
-   The Windows USN journal which stores plenty of information[^350].

-   The Windows System Resource Usage Monitor (SRUM)[^351].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^352].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^353]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^354] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you if it is supported or not like CrystalDiskInfo[^355] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
According to Apple Documentation[^335], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
Download the latest Debian testing amd64 netinst ISO from <https://www.debian.org/CD/netinst/>
...
-   If you really must take them with you, you could consider the use of a faraday cage[^356] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^357] for their effectiveness. If you cannot afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^358] and touch DNA[^359].
...
-   Do not lick the envelope or the stamps[^360] if you use them to avoid leaving DNA traces.
...
In that case my last resort recommendation is to connect safely **from a distance** to a Public Wi-Fi (See [Find some safe places with decent public Wi-Fi:]) using your laptop and TAILS "unsafe browser". See <https://tails.boum.org/contribute/design/Unsafe_Browser/>.
...
So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^218].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^361].
...
Usually, integrity checks[^362] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^363], more recently MD5[^364] but those present several weaknesses (CRC, MD5[^365]) that makes them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^366] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it is usually recommended to use SHA[^367] based hashes and the most used is probably the SHA-2[^368] based SHA256 for verifying file integrity. SHA is much more resistant to collisions than CRC and MD5.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^371] for instance) using public key cryptography[^372].
...
You might be asking yourself, what about Anti-Virus solutions? Well, no ... these are not perfect solutions against many modern malware and viruses using polymorphic code[^373]. But it does not mean they cannot help against less sophisticated and known attacks. It depends how to use them as AV software can become an attack vector in itself.
...
Some will just argue against them broadly like Whonix[^374] but this topic is being discussed and disputed even at Whonix[^375] by other members of their community.
...
-   Yes, there are viruses and malware for Linux[^376]'[^377]'[^378]'[^379]'[^380]

-   Yes, there are viruses and malware for MacOS[^376]'[^381][^382]'[^383][^384]
...
-   Do use Open-Source non-real time offline Anti-Virus/Anti-Malware tools as an added measure to scan some files such as:
    ...
    -   Windows/Linux/MacOS/Qubes OS: ClamAV (<https://www.clamav.net/>)

    -   Linux/Qubes OS: RFXN Linux Malware Detect (<https://github.com/rfxn/linux-malware-detect>)

    -   Linux/Qubes OS: Chkrootkit (<http://www.chkrootkit.org/>)

-   You could also use online services for **non-sensitive files\*** such as VirusTotal (<https://www.virustotal.com/gui>)

    -   You could also just check the VirusTotal database for the hash of your file if you don't want to send it over (see <https://developers.virustotal.com/v3.0/docs/search-by-hash> (See the [Integrity (if available):] section again for guidance on how to generate hashes).

-   Other tools are also available for non-sensitive files and a convenient list is right here: <https://github.com/rshipp/awesome-malware-analysis#online-scanners-and-sandboxes>

\* **Please be aware that while VirusTotal might seem very practical for scanning various files, their "privacy policy" is problematic (see <https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy>) and states:**
...
Again, regarding the PDFs of this guide and as explained in the README of my repository, you could check for anomalies using PDFID which you can download at <https://blog.didierstevens.com/programs/pdf-tools/>
...
-   **Qubes OS:** Consider using <https://github.com/QubesOS/qubes-app-linux-pdf-converter> which will convert your PDF into a flattened image file. This should theoretically remove any malicious code in it. Note that this will also render the PDF formatting useless (such as links, headings, bookmarks, and references).

-   **(Deprecated) Linux/Qubes OS** (or possibly MacOS through Homebrew or Windows through Cygwin): Consider not using <https://github.com/firstlookmedia/pdf-redact-tools> which will also turn your PDF into a flattened image file. Again, this should theoretically remove any malicious code in it. Again, this will also render the PDF formatting useless (such as links, headings, bookmarks, and references). **Note that this tool is deprecated and relies on a library called "ImageMagick" which is known for several security issues**[^385]**. You should not use this tool even if it is recommended in some other guides.**

-   **Windows/Linux/Qubes/OS/MacOS:** Consider using <https://github.com/firstlookmedia/dangerzone> which was inspired by Qubes PDF Converted above and does the same but is still maintained and works on all OSes. This tool also works with Images, ODF files and Office files (**Warning: this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together**[^386]**).**

#### Other type of files:

Here are some various resources for this purpose where you will find what tool to use for what type:

-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> (warning, many of those tools might be harder to use on Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^387] that might be of use).

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis>

-   This interesting PDF detailing which tool to use for which file type <https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf>
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^388]. Those apps do not require installation your system and can be run from an USB key or anywhere else.
...
[^16]: Geek Feminism, <https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F>

[^17]: Tor Project, Tor Users <https://2019.www.torproject.org/about/torusers.html.en>

[^18]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL>

[^19]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>
...
[^21]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany>

[^22]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil>

[^23]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address>

[^24]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention>

[^25]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^26]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>

[^27]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System>

[^28]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking>

[^29]: CensoredPlanet <https://censoredplanet.org/>

[^30]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf>

[^31]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole>

[^32]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS>

[^33]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS>

[^34]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole>

[^35]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication>

[^36]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^37]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^38]: Mozilla Blog, Encrypted Client Hello: the future of ESNI in Firefox <https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/>

[^39]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf>

[^40]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network>

[^41]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/>

[^42]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^43]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^44]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^45]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^46]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^47]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^48]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^49]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^50]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^51]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>

[^52]: Bellingcat, The GRU Globetrotters: Mission London <https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/>

[^53]: Bellingcat,\"V\" For "Vympel": FSB's Secretive Department "V" Behind Assassination Of Georgian Asylum Seeker In Germany <https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/>

[^54]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^55]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^56]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^57]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^58]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^59]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^60]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^61]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>

[^62]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^63]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^64]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^65]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^66]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^67]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^68]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf>

[^69]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/>)

[^70]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault>

[^71]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/>

[^72]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html>

[^73]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/>

[^74]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant>

[^75]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A>

[^76]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden>

[^77]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>
...
[^82]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/>

[^83]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT>

[^84]: ZDnet, I asked Apple for all my data. Here\'s what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/>

[^85]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^86]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^87]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^88]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^89]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^90]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>
...
[^97]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^98]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^99]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^100]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>
...
[^104]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^105]: BBC, China social media: WeChat and the Surveillance State <https://www.bbc.com/news/blogs-china-blog-48552907>

[^106]: The Intercept, Revealed: Massive Chinese Police Database <https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/>

[^107]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^108]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark>

[^109]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark>

[^110]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html>

[^111]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking>

[^112]: Vobile Solutions, <https://www.vobilegroup.com/solutions>

[^113]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html>

[^114]: Imatag, <https://www.imatag.com/>

[^115]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^116]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921>

[^117]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140>

[^118]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746>

[^119]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification>

[^120]: Wikipedia, Printing Steganography <https://en.wikipedia.org/wiki/Machine_Identification_Code>

[^121]: MIT, SeeingYellow, <http://seeingyellow.com/>

[^122]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^123]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^124]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^125]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^126]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^127]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^128]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>

[^129]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/>

[^130]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^131]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^132]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^133]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler>

[^134]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity>

[^135]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^136]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361>

[^137]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^138]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^139]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^140]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^141]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^142]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^143]: Libreboot, <https://libreboot.org/>

[^144]: Digi.Ninja, Jasager <https://digi.ninja/jasager/>

[^145]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple>

[^146]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack>

[^147]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal>

[^148]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^149]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^150]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work>

[^151]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^152]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^153]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^154]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/>

[^155]: Wikipedia, Stylometry <https://en.wikipedia.org/wiki/Stylometry>

[^156]: Paul Moore Blog, Behavioral Profiling: The password you can\'t change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/>

[^157]: Wikipedia, Sentiment Analysis, <https://en.wikipedia.org/wiki/Sentiment_analysis>

[^158]: EFF CoverYourTracks, <https://coveryourtracks.eff.org/>

[^159]: SecuredTouch Blog, Behavioral Biometrics 101: Behavioral Biometrics vs. Behavioral Analytics <https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics>

[^160]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>

[^161]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/>

[^162]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^163]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^164]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>

[^165]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/>

[^166]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/>

[^167]: Brown Institute, Open-Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^168]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^169]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^170]: APNews, Chinese \'gait recognition\' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a>

[^171]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/>

[^172]: Nature.com, Facial recognition technology can expose political orientation from naturalistic facial images <https://www.nature.com/articles/s41598-020-79310-1>

[^173]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^174]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^175]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^176]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^177]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^178]: Anyvision <https://www.anyvision.co/>

[^179]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^180]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^181]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^182]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^183]: BBC, WhatsApp photo drug dealer caught by \'groundbreaking\' work <https://www.bbc.com/news/uk-wales-43711477>

[^184]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^185]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^186]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^187]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake>

[^188]: Econotimes, Deepfake Voice Technology: The Good. The Bad. The Future <https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278>

[^189]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^190]: Forbes, A Voice Deepfake Was Used To Scam A CEO Out Of \$243,000 <https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/>

[^191]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^192]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^193]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^194]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^195]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^196]: Wikipedia, Social Engineering <https://en.wikipedia.org/wiki/Social_engineering_(security)>

[^197]: Grayshirt, <https://www.grayshift.com/>

[^198]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^199]: Loup-Vaillant.fr, Rolling Your Own Crypto <https://loup-vaillant.fr/articles/rolling-your-own-crypto>

[^200]: Dhole Moments, Crackpot Cryptography and Security Theater <https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/>

[^201]: Jack Poon, The myth of military grade encryption <https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369>

[^202]: Congruen, Stop calling it \"Military-Grade Encryption\" <https://blog.congruentlabs.co/military-grade-encryption/>

[^203]: IronCoreLabs Blog, "Military Grade Encryption" <https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588>

[^204]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^205]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^206]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^207]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^208]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^209]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^210]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^211]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^212]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^213]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^214]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^215]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^216]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^217]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^218]: Wikipedia, OONI, <https://en.wikipedia.org/wiki/OONI>

[^219]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^220]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^221]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf>

[^222]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^223]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^224]: Coreboot, <https://www.coreboot.org/>

[^225]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^226]: XKCD, Password Strength <https://xkcd.com/936/>

[^227]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^228]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^229]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^230]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^231]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^232]: Sneak.Berlin, Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^233]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^234]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^235]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^236]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^237]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption>

[^238]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^239]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^240]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^241]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^242]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^243]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^244]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^245]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^246]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^247]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^248]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^249]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^250]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^251]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^252]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^253]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^254]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^255]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^256]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^257]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^258]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^259]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^260]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^261]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^262]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^263]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^264]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^265]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^266]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^267]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^268]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^269]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^270]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^271]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^272]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^273]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^274]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^275]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^276]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^277]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^278]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^279]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^280]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^281]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^282]: hCaptcha <https://www.hcaptcha.com/>

[^283]: hCaptcha, hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^284]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/>

[^285]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^286]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^287]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^288]: Tor Project Community, Cloudflare Captcha Monitoring <https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/>

[^289]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^290]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^291]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^292]: Developers Google Blog,
...
[^293]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^294]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^295]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^296]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html>

[^297]: Vice.com, PornHub Announces \'Biometric Technology\' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id>

[^298]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^299]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^300]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam>

[^301]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^302]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^303]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^304]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^305]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^306]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^307]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^308]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en>

[^309]: Jumio, ID verification features <https://www.jumio.com/features/>

[^310]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^311]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^312]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^313]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^314]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/>

[^315]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/>

[^316]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/>

[^317]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop>

[^318]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^319]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^320]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^321]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^322]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^323]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^324]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^325]: ResearchGate, Live forensics method for acquisition on the Solid-State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^326]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^327]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^328]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^329]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^330]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^331]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^332]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^333]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^334]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^335]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html>

[^336]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^337]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^338]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^339]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^340]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^341]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^342]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^343]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^344]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^345]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^346]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^347]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/>

[^348]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^349]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^350]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^351]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^352]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^353]: DBAN, <https://dban.org/>

[^354]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^355]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>

[^356]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^357]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^358]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M>

[^359]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA>

[^360]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss\..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps>

[^361]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection>

[^362]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification>

[^363]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check>

[^364]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5>

[^365]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security>

[^366]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance>

[^367]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms>

[^368]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2>

[^369]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity>

[^370]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f>

[^371]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard>

[^372]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography>

[^373]: Wikipedia, Polymorphic Code <https://en.wikipedia.org/wiki/Polymorphic_code>

[^374]: Whonix Documentation, Use of AV, <https://www.whonix.org/wiki/Malware_and_Firmware_Trojans#The_Utility_of_Antivirus_Tools>

[^375]: Whonix Forums, <https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8>

[^376]: AV-Test Security Report 2018-2019, <https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf>

[^377]: ZDNet, ESET discovers 21 new Linux malware families <https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/>

[^378]: NakeSecurity, EvilGnome -- Linux malware aimed at your desktop, not your servers <https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/>

[^379]: Immunify, HiddenWasp: How to detect malware hidden on Linux & IoT <https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot>

[^380]: Wikipedia, Linux Malware <https://en.wikipedia.org/wiki/Linux_malware>

[^381]: Wikipedia, MacOS Malware <https://en.wikipedia.org/wiki/MacOS_malware>

[^382]: MacWorld, List of Mac viruses, malware and security flaws <https://www.macworld.co.uk/feature/mac-viruses-list-3668354/>

[^383]: JAMF, The Mac Malware of 2020 <https://resources.jamf.com/documents/macmalware-2020.pdf>

[^384]: MacOS Security and Privacy Guide, <https://github.com/drduh/macOS-Security-and-Privacy-Guide#viruses-and-malware>

[^385]: ImageTragick.com, <https://imagetragick.com/>

[^386]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html>

[^387]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/>

[^388]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application>
  ...
  [22]: media/image1.jpeg
  [23]: media/image2.jpeg 
  [Your Face, Voice, Biometrics and Pictures: 28]: #_Toc65931628
  [24]: media/image3.jpeg
  [25]: media/image4.jpeg 
  [www.pornhub.com]: http://www.pornhub.com
  [26]: Anonymized#_Your_
  [27]: media/image5.jpeg
  [28]: media/image6.jpeg
  [29]: media/image7.jpeg
  [30]: media/image8.jpeg
  [31]: media/image9.jpeg 
  [32]: media/image10.jpeg
  [33]: media/image11.jpeg
  [34]: media/image12.jpeg
  [35]: media/image13.jpeg 
  [36]: media/image14.jpeg 
  [37]: media/image15.jpeg
  [38]: media/image16.jpeg 
  [39]: media/image17.jpeg
  [40]: media/image18.jpeg
  [41]: media/image19.jpeg
  [42]: media/image20.jpeg 
  [43]: media/image21.jpeg
  [44]: media/image22.jpeg
  [45]: media/image23.jpeg
  [46]: media/image24.jpeg
  ...
  [47]: media/image25.jpeg
  [48]: media/image26.jpeg
  [49]: media/image27.jpeg
  [50]: media/image28.jpeg
  [51]: media/image29.jpeg
  [52]: media/image30.jpeg
  [Online Phone Number (less recommended):]: #online-phone-number-less-recommended
  [53]: media/image31.jpeg 
  [54]: #_Financial_transactions:
  [55]: media/image32.jpeg
  [56]: media/image33.jpeg
  [57]: media/image34.jpeg
  [58]: media/image35.jpeg
  [59]: media/image36.jpeg
  [Extra Tools Cleaning:]: #extra-tools-cleaning
  [60]: media/image37.jpeg
  [61]: media/image38.jpeg
  [62]: media/image39.jpeg
  [63]: media/image40.jpeg
