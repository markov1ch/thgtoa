...
Version 0.9.6b (**draft**), June 2021 by AnonymousPlanet.
...
If you are feeling a bit more adventurous, you could install your own BIOS using Libreboot[^97] or Coreboot[^249] if your laptop supports it (be aware that Coreboot does contain some propriety code unlike its fork Libreboot).
...
-   This technology is also widely used in CAPTCHAS[^310] services to verify that you are "human" and can be used to fingerprint a user.
...
-   2021, The Germany based mail provider Tutanota was forced to monitor specific accounts[^229]

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to intercept and save copies of the unencrypted e-mails of one user[^230] (they did not decrypt the stored e-mail).

-   2017, PureVPN was forced to disclose information of one user to the FBI[^231].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^232].

-   2014, HideMyAss user was de-anonymized and logs were provided to the FBI[^233].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^234].

Some providers have implemented the use of a Warrant Canary[^235] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^236] and Omnisec story[^237]).
...
(Illustration: excellent movie I highly recommend: Das Leben der Anderen[^238])

There are many advanced techniques that can be used by skilled adversaries[^239] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> <sup>[[Archive.org]][152]</sup> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^240] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw> <sup>[[Invidious]][169]</sup>
...
Zero-Trust Security[^331] ("Never trust, always verify").
...
-   If you absolutely want system wide plausible deniability[^258]'[^241] despite the risks[^242]'[^261]:
...
-   Check some data yourself here on the Tor Project OONI[^243] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/> <sup>[[Archive.org]][200]</sup>
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^252]**.**
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^244].
...
Some might be very efficient[^245] but many are gimmicky gadgets that offer no real protection[^246].
...
TAILS[^247] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^248]**.**
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm> <sup>[[Archive.org]][223]</sup>) or System76 laptops (<https://system76.com/> <sup>[[Archive.org]][224]</sup>) while using Coreboot[^249] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. And, because AFAIK, AMD PSP was audited and contrary to IME was not found to have any "evil" functionalities[^250]. However, if you are going for the Qubes OS Route consider Intel as they do not support AMD with their anti-evil-maid system[^251].
...
-   Enable BIOS/UEFI password and use a long passphrase[^252] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^253]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^254]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^255]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> <sup>[[Archive.org]][233]</sup> [^256] (with pictures).
...
This route will make extensive use of Virtual Machines[^257], they will require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to conveniently use Plausible Deniability[^258] (aka Deniable Encryption[^259]) easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^260] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It is the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^261].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^262]'[^263] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
In the case of Windows, plausible deniability is also the reason you should ideally have Windows 10 Home (and not Pro). This is because Windows 10 Pro natively offers a full-disk encryption system (Bitlocker[^264]) where Windows 10 Home offers no full-disk encryption at all. We will later use a third-party open-source software for encryption that will allow full-disk encryption on Windows 10 Home. This will give you a good (plausible) excuse to use this software. While using this software on Windows 10 Pro would be suspicious.

**Note about Linux:** So, what about Linux and plausible deniability? Yes, it is kind of possible to achieve plausible deniability with Linux too[^265]. But it is complicated to set-up and IMHO requires a skill level high enough that you probably do not need this guide to help you try it.
...
Cold Boot attacks[^267] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^268], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^269] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^270] and by the maintainer of Veracrypt[^271] so I would not worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^273].

-   Windows Indexing keeping traces of the files present in your user folder by default[^274].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^275].
...
-   Gatekeeper[^276] and XProtect keeping track of your download history in a local database and file attributes.
...
Please refer to this excellent guide <https://madaidans-insecurities.github.io/guides/linux-hardening.html> <sup>[[Archive.org]][258]</sup>
...
Veracrypt[^277] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^278] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^279] (settings > performance/driver options > encrypt RAM) at a cost of 5-15% performance. This setting will also disable hibernation (which does not actively clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings > Preferences > Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^280] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^281] Operations[^282]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^252] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^289] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^290] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

So, you should be aware that Virtualbox is not the virtualization software with the best track record in terms of security and some of the reported issues[^291] have not be completely fixed to this date[^292] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM> <sup>[[Archive.org]][270]</sup> and here <https://www.whonix.org/wiki/KVM#Why_Use_KVM_Over_VirtualBox.3F> <sup>[[Archive.org]][271]</sup>
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^293].

Stream isolation is a mitigation technique used to prevent some correlation attacks by having different Tor Circuits for each application. Here is an illustration to show what stream isolation is:![][276]

(Illustration from Marcelo Martins, <https://stakey.club/en/decred-via-tor-network/> <sup>[[Archive.org]][277]</sup>)

VPN/Proxy over Tor falls on the right-side[^294] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
-   <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][278]</sup>

-   <https://tails.boum.org/contribute/design/stream_isolation/> <sup>[[Archive.org]][279]</sup>

-   <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][280]</sup>
...
-   <https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services#Tor_and_VPN_Services_Comparison> <sup>[[Archive.org]][282]</sup>

-   <https://www.whonix.org/wiki/Why_does_Whonix_use_Tor> <sup>[[Archive.org]][283]</sup>

-   <https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study> <sup>[[Archive.org]][284]</sup>

-   <https://gist.github.com/joepie91/5a9909939e6ce7d09e29#file-vpn-md> <sup>[[Archive.org]][285]</sup>

-   <https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html> <sup>[[Archive.org]][286]</sup>
...
-   Tor Project: <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN> <sup>[[Archive.org]][287]</sup>

-   Tails Documentation:

    -   <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/> <sup>[[Archive.org]][288]</sup>

    -   <https://tails.boum.org/support/faq/index.en.html#index20h2> <sup>[[Archive.org]][289]</sup>

-   Whonix Documentation (in this order):

    -   <https://www.whonix.org/wiki/Tunnels/Introduction> <sup>[[Archive.org]][290]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN> <sup>[[Archive.org]][291]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][281]</sup>
...
This route will use Virtualization and Whonix[^295] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^296] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
This will allow you to turn your VMs into a kind of a disposable "Live Operating Systems" (like TAILS discussed earlier). Meaning that you will be able to erase all the traces of your activities within a VM by restoring a Snapshot to an earlier state. Of course, this will not be "as good" as TAILS (where everything is stored in memory) as there might be traces of this activity left on your hard disk. Forensics studies have shown the ability to recover data from a reverted VM[^297]. Fortunately, there will be ways to remove those traces after deletion or reverting to a previous snapshot. Such techniques will be discussed in the [Some additional measures against forensics][Some additional measures against forensics:] section of this guide.

#### Download Virtualbox and Whonix utilities:

You should download a few things within the host OS.

-   The latest version of the Virtualbox installer according to your Host OS <https://www.virtualbox.org/wiki/Downloads> <sup>[[Archive.org]][294]</sup>

-   (Skip this if you cannot use Tor natively of through a VPN) The latest Whonix OVA file from <https://www.whonix.org/wiki/Download> <sup>[[Archive.org]][295]</sup> according to your preference (Linux/Windows, with a Desktop interface XFCE for simplicity or only with the text-client for advanced users)
...
For ideal security, you should follow the recommendations provided here for each Virtualbox Virtual Machine <https://www.whonix.org/wiki/Virtualization_Platform_Security#VirtualBox_Hardening> <sup>[[Archive.org]][296]</sup> :
...
Finally, also follow this recommendation to desync the clock you are your VM compared to your host OS <https://www.whonix.org/wiki/Network_Time_Synchronization#Spoof_the_Initial_Virtual_Hardware_Clock_Offset> <sup>[[Archive.org]][297]</sup>
...
Also consider applying these mitigations from VirtualBox to mitigate Spectre[^298]/Meltdown[^299] vulnerabilities by running this command from the VirtualBox Program Directory. All of these are described here: <https://www.whonix.org/wiki/Spectre_Meltdown> <sup>[[Archive.org]][74]</sup> (be aware these can impact severely the performance of your VMs but should be done for best security).

Finally consider the security advice from Virtualbox themselves here <https://www.virtualbox.org/manual/ch13.html> <sup>[[Archive.org]][298]</sup>
...
Remember at this stage that if you are having issues connecting to Tor due to censorship or blocking, you should consider connecting using Bridges as explained in this tutorial <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][299]</sup>.

-   Update the Whonix VMs by following the instructions on <https://www.whonix.org/wiki/Operating_System_Software_and_Updates#Updates> <sup>[[Archive.org]][300]</sup>
...
**Important Note: You should also read these very good recommendations over there <https://www.whonix.org/wiki/DoNot>** <sup>[[Archive.org]][301]</sup> **as most of those principles will also apply to this guide. You should also read their general documentation here <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][302]</sup> **which will also provide tons of advice like this guide.**
...
**It is also the only VM that will provide Stream Isolation pre-configured for most apps by default**[^300]**.**

If you want additional software on the Workstation (such as another Browser), follow their guide here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][303]</sup>

Consider running Whonix in Live Mode if for extra malware protection, See <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][304]</sup>

Do not forget to apply the VM hardening recommendations here: Virtualbox Hardening recommendations:

#### Linux (any distro):

**Be careful, any customization you make to the non-Whonix guest VMs (keyboard layout, language, time zone, screen resolution or other) could be used to fingerprint your VMs later. See <https://www.whonix.org/wiki/VM_Fingerprinting>** <sup>[[Archive.org]][305]</sup>
...
Refer to this tutorial <https://www.whonix.org/wiki/Other_Operating_Systems> <sup>[[Archive.org]][306]</sup> for detailed instructions.
...
-   Go with Windows AME (Ameliorated) from the <https://ameliorated.info/> <sup>[[Archive.org]][307]</sup> project which is a special Windows 10 build stripped from all telemetry/advertising and update components. **Note that you will not be able to update this version with the latest security patched and will have to just re-download a new release. See [Appendix Y: Windows AME installation][Appendix Y: Windows AME download and installation]**
...
-   Download and install Brave browser from <https://brave.com/download/> <sup>[[Archive.org]][308]</sup>
...
Skip these if you used Windows AME from <https://ameliorated.info/> <sup>[[Archive.org]][307]</sup>
...
Basically, follow the tutorial here: <https://www.android-x86.org/documentation/virtualbox.html> <sup>[[Archive.org]][309]</sup>
...
-   Windows Host OS:
    ...
    -   Virtualbox Catalina Tutorial: <https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/> <sup>[[Archive.org]][310]</sup>

    -   Virtualbox Big Sur Tutorial: <https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/> <sup>[[Archive.org]][311]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^301] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^302] authentication with the authenticator feature.
...
-   Whonix: [https://www.whonix.org/wiki/KeePassXC][] <sup>[[Archive.org]][313]</sup>

-   Linux:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][314]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_linux> <sup>[[Archive.org]][315]</sup>

-   Windows:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][314]</sup>

    -   Follow the tutorial here [https://KeePassXC.org/docs/KeePassXC_GettingStarted.html#_microsoft_windows][] <sup>[[Archive.org]][315]</sup>

-   MacOS:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][314]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_macos> <sup>[[Archive.org]][315]</sup>
...
**This way is not supported by the Whonix project**[^303] but I will go ahead and give this option anyway. IMHO this is helpful to prevent your Host OS from leaking any information while you are using the Whonix VMs.
...
-   Qube OS Introduction, <https://www.qubes-os.org/intro/> <sup>[[Archive.org]][319]</sup>

-   Qube OS Video Tours, <https://www.qubes-os.org/video-tours/> <sup>[[Archive.org]][320]</sup>

-   Qube OS Getting Started, <https://www.qubes-os.org/doc/getting-started/> <sup>[[Archive.org]][321]</sup>

-   YouTube, Life Behind the Tinfoil: A Look at Qubes and Copperhead - Konstantin Ryabitsev, The Linux Foundation <https://www.youtube.com/watch?v=8cU4hQg6GvU> <sup>[[Invidious]][322]</sup>

-   YouTube, I used the reasonably-secure Qubes OS for 6 months and survived - Matty McFatty [@themattymcfatty] <https://www.youtube.com/watch?v=sbN5Bz3v-uA> <sup>[[Invidious]][323]</sup>

-   YouTube, Qubes OS: How it works, and a demo of this VM-centric OS <https://www.youtube.com/watch?v=YPAvoFsvSbg> <sup>[[Invidious]][324]</sup>

This OS is recommended by prominent figures such as Edward Snowden and Privacytools.io.

Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^304], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM. If you want a comfortable experience, you should have 16GB, if you want a very good experience, you should have 24GB or 32GB.
...
You should also check their hardware compatibility here <https://www.qubes-os.org/hcl/> <sup>[[Archive.org]][325]</sup> before proceeding. Your mileage might vary and you might experience several issues with regards to hardware compatibility that you will have to troubleshoot and solve yourself.

I think that if you can afford it and are comfortable with the idea of using Linux, you should go with this route as it is probably the best one in terms of security and privacy. The only disadvantage of this route is that it does not provide a way to enable OS wide plausible deniability[^258] unlike the Whonix route.
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^305].

Stream isolation is a mitigation technique used to prevent some correlation attacks by having different Tor Circuits for each application. Here is an illustration to show what stream isolation is:![][276]

(Illustration from Marcelo Martins, <https://stakey.club/en/decred-via-tor-network/> <sup>[[Archive.org]][277]</sup>)

VPN/Proxy over Tor falls on the right-side[^306] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
Note, if you're having issues accessing the Tor Network due to blocking/censorship, you could try using Tor Bridges (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][215]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][299]</sup>).
...
We will follow the instructions from their own guide <https://www.qubes-os.org/doc/installation-guide/> <sup>[[Archive.org]][328]</sup>:

Secure Boot is not supported as per their FAQ: <https://www.qubes-os.org/faq/#is-secure-boot-supported> <sup>[[Archive.org]][329]</sup> so it should be disabled in the BIOS/UEFI settings.
...
-   Install Qubes OS according to the installation guide:
    ...
    -   **If you want to use Tor or VPN over Tor: Check the** "**Enabling system and template updates over the Tor anonymity network using Whonix" during the last step. This will force all Qubes OS updates to go through Tor. While this will significantly reduce your update speed, it will increase your anonymity from the start.** (If you are having issues connecting to Tor due to censorship or blocking, consider using Tor Bridges as recommended earlier. Just follow the tutorial provided here: <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][299]</sup>)
...
This is based on the tutorial provided by Qube OS themselves (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/vpn.md> <sup>[[Archive.org]][330]</sup>). If you are familiar with this process, you can follow their tutorial. Here is mine:
...
-   Test your Connectivity:
    ...
    -   If you are going for VPN over Tor, Test the VM connectivity to Tor by launching a Browser within the ProxyVM and going to <https://check.torproject.org> <sup>[[Archive.org]][331]</sup> (It should say you are connected to Tor)
...
> (These will block outbound traffic when the VPN is down, it is a kill switch, more information here <https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux> <sup>[[Archive.org]][332]</sup> )
...
-   Test the ProxyVM VPN connectivity by starting a Browser within it and going to your VPN provider test page. It should now say you are connected to a VPN:
    ...
    -   Mullvad: <https://mullvad.net/en/check/> <sup>[[Archive.org]][333]</sup>

    -   IVPN: <https://www.ivpn.net/> <sup>[[Archive.org]][334]</sup> (check the top banner)

    -   ProtonVPN: Follow their instructions here <https://protonvpn.com/support/vpn-ip-change/> <sup>[[Archive.org]][335]</sup>
...
-   Create a Whonix Workstation Disposable VM (follow this tutorial <https://www.whonix.org/wiki/Qubes/DisposableVM> <sup>[[Archive.org]][336]</sup>)
...
Apply the instructions from <https://brave.com/linux/> <sup>[[Archive.org]][337]</sup> (Fedora 28+ section) and run the following commands:
...
Edit the Whonix Disposable VM template and follow instructions here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][303]</sup>
...
Now you will have to follow the instructions from here: <https://github.com/anbox/anbox-modules> <sup>[[Archive.org]][338]</sup>
...
Now we will follow their other tutorial from here: <https://github.com/anbox/anbox/blob/master/docs/install.md> <sup>[[Archive.org]][339]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^308] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^309] authentication with the authenticator feature.
...
Captcha[^310] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^311] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCaptcha service[^312]) and Cloudflare (hCaptcha[^313]). hCaptcha is used on 15% of the internet by their own metrics[^314].

They are designed to separate bots from humans but are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you will quickly encounter many captchas everywhere[^315]. Quite often when using Tor, even if you succeed in solving all the puzzles, you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor> <sup>[[Archive.org]][274]</sup>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^316]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^317].
...
You will often experience several in a row (sometimes endless[^319]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You will also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.

You will also have (in my experience) more Captchas (reCaptcha) in Google if you do not use Chrome. But this can be mitigated by using Chromium based browsers such as Brave or Ungoogled-Chromium. There is also a Browser extension called Buster that could help you those <https://github.com/dessant/buster> <sup>[[Archive.org]][341]</sup>.

As for Cloudflare (hCaptcha), you could also use their Accessibility solution here (<https://www.hcaptcha.com/accessibility> <sup>[[Archive.org]][342]</sup>) which would allow you to sign-up (with your anonymous identity created later) and set a cookie within your Browser that would allow you to bypass their captchas. Another solution to mitigate hCaptcha would be to use their own solution called "Privacy Pass"[^320] <https://privacypass.github.io/> <sup>[[Archive.org]][343]</sup> in the form of a Browser extension you could install in your VM Browser.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^321].
...
Browser and Device[^322] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^323]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Chromium based Browsers such as Brave Browser over Tor Browser within this VM.
...
This is for instance now the case on YouTube for some European Users[^324] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^325] and unfortunately sometimes required.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^326]. Unfortunately, this method is now also sometimes being used on Facebook[^327] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^328].

This verification is very hard to defeat but possible. A method to possibly defeat those would be to use "deep fake" technology software such as the open-source FaceSwap <https://github.com/deepfakes/faceswap> <sup>[[Archive.org]][347]</sup> to generate the required verification pictures using a randomly computer-generated face that would be swapped over the picture of a complicit model (or a stock photo).
...
Some countries have even developed hotlines where you can report any subversive content[^329].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^330] (or alternatively "Never trust, always verify" if you are more hardcore about it and want to apply Zero-Trust Security[^331]) instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is the real term for this[^332]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience less issues with captchas and online platforms[^323] if you use Brave than if you use Tor Browser (feel free to try this yourself).

-   Every identity you should have a matching profile picture associated to it. For this purpose, I recommend you just go to <https://thispersondoesnotexist.com/> <sup>[[Archive.org]][348]</sup> and generate a computer-generated profile picture. You can also generate such pictures yourself from your computer if you prefer by using the open-source StyleGan project here <https://github.com/NVlabs/stylegan2> <sup>[[Archive.org]][349]</sup>. Just refresh the page until you find a picture that matches your identity in all aspects (age, sex, and ethnicity) and save that picture. It would be even better to have several pictures associated to that identity but I do not have an "easy way" of doing that yet.

    -   **Bonus**, you could also make it more real by using this service (with an anonymous identity) <https://www.myheritage.com/deep-nostalgia> <sup>[[Archive.org]][350]</sup> to make a picture more lifelike. Here is an example:
...
You could also achieve the same result without using MyHeritage and by doing it yourself using for example <https://github.com/AliaksandrSiarohin/first-order-model> <sup>[[Archive.org]][352]</sup> but this will require more manual operations (**and requires an NVIDIA GPU**).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^333] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^252]**.**

Here is also a good guide on this specific topic: <https://gendersec.tacticaltech.org/wiki/index.php/Complete_manual#.22Real.22_names> <sup>[[Archive.org]][353]</sup>
...
-   Whonix/Tails: Create/Edit a file ```/usr/local/etc/torrc.d/50_user.conf```[^334].

-   On Tor Browser: Edit the torrc file located at ```Browser/TorBrowser/Data/Tor```[^335].
...
**Please note that this is restricting Onion Routing could limit your Anonymity if you are too restrictive. You can see a visualized list of available Exit Nodes here: <https://www.bigdatacloud.com/insights/tor-exit-nodes>** <sup>[[Archive.org]][354]</sup>

Here is the list of possibilities (this is a general list and many of those countries might not have Exit nodes at all): <https://b3rn3d.herokuapp.com/blog/2014/03/05/tor-country-codes/> <sup>[[Archive.org]][355]</sup>
...
On my side, I strongly condemn this type of real-name policy. See for instance this Wikipedia article giving some examples: <https://en.wikipedia.org/wiki/Facebook_real-name_policy_controversy> <sup>[[Archive.org]][356]</sup>

Here are some more references about the German case for reference:

-   <https://slate.com/technology/2018/02/why-some-americans-are-cheering-germany-for-taking-on-facebooks-real-name-policy.html> <sup>[[Archive.org]][357]</sup>

-   <https://www.theverge.com/2018/2/12/17005746/facebook-real-name-policy-illegal-german-court-rules> <sup>[[Archive.org]][358]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][359]</sup>

-   <https://www.vzbv.de/sites/default/files/downloads/2018/02/14/18-02-12_vzbv_pm_facebook-urteil_en.pdf> <sup>[[Archive.org]][360]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][359]</sup>

-   <https://www.reuters.com/article/us-germany-facebook/german-court-rules-facebook-use-of-personal-data-illegal-idUSKBN1FW1FI> <sup>[[Archive.org]][361]</sup>
...
-   South Korea (see <https://en.wikipedia.org/wiki/Real-name_system#South_Korea> <sup>[[Archive.org]][362]</sup>)
...
-   **Consider using the recommended tools on <https://privacytools.io/>** <sup>[[Archive.org]][363]</sup> **for your better privacy instead of the usual mainstream ones.**

-   **Consider using the recommended tools on <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][302]</sup> **as well instead of the usual mainstream ones such as E-mail providers: <https://www.whonix.org/wiki/E-Mail#Anonymity_Friendly_Email_Provider_List>** <sup>[[Archive.org]][364]</sup>

**The following overview does not mention the privacy practices of those platforms but only their requirements for registering an account. If you want to use privacy-aware tools and platforms, head on to <https://privacytools.io/>** <sup>[[Archive.org]][363]</sup>
...
-   Is this against their ToS? No but yes <https://www.amazon.com/gp/help/customer/display.html?nodeId=202140280> <sup>[[Archive.org]][365]</sup>
...
-   Is this against their ToS? Yes <https://www.apple.com/legal/internet-services/icloud/en/terms.html> <sup>[[Archive.org]][366]</sup>
...
-   Is this against their ToS? No <https://briarproject.org/privacy-policy/> <sup>[[Archive.org]][367]</sup>
...
-   Is this against their ToS? No <https://discord.com/terms> <sup>[[Archive.org]][368]</sup>
...
-   Is this against their ToS? No <https://element.io/terms-of-service> <sup>[[Archive.org]][369]</sup>
...
-   Is this against their ToS? Yes <https://www.facebook.com/terms.php> <sup>[[Archive.org]][370]</sup>
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^336] will not help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
-   Is this against their ToS? No <https://docs.github.com/en/free-pro-team@latest/github/site-policy/github-terms-of-service> <sup>[[Archive.org]][371]</sup>
...
-   Is this against their ToS? No <https://about.gitlab.com/handbook/legal/subscription-agreement/> <sup>[[Archive.org]][372]</sup>
...
-   Is this against their ToS? No <https://policies.google.com/terms> <sup>[[Archive.org]][373]</sup>
...
If your account gets suspended [^339] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^340] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^341].

#### HackerNews:

-   Is this against their ToS? No <https://www.ycombinator.com/legal/#tou> <sup>[[Archive.org]][374]</sup>
...
-   Is this against their ToS? **Maybe?** I am not sure <https://help.instagram.com/581066165581870?ref=dp> <sup>[[Archive.org]][375]</sup>
...
-   Is this against their ToS? No <https://jami.net/privacy-policy/> <sup>[[Archive.org]][376]</sup>
...
-   Is this against their ToS? No <https://www.ivpn.net/tos/> <sup>[[Archive.org]][377]</sup>
...
-   Is this against their ToS? Yes <https://www.linkedin.com/legal/user-agreement> <sup>[[Archive.org]][378]</sup>
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^342] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
-   Is this against their ToS? No unless it is about crypto <https://policy.medium.com/medium-terms-of-service-9db0094a1e0f> <sup>[[Archive.org]][379]</sup>
...
-   Is this against their ToS? Yes <https://www.microsoft.com/en/servicesagreement/> <sup>[[Archive.org]][380]</sup>
...
-   Is this against their ToS? No <https://mullvad.net/en/help/terms-service/> <sup>[[Archive.org]][381]</sup>
...
-   Is this against their ToS? No <https://njal.la/tos/> <sup>[[Archive.org]][382]</sup>
...
-   Is this against their ToS? No <https://ProtonMail.com/terms-and-conditions> <sup>[[Archive.org]][383]</sup>
...
They are recommended by privacytools.io[^343]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^344]. If their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
-   Is this against their ToS? No <https://protonvpn.com/terms-and-conditions> <sup>[[Archive.org]][384]</sup>
...
-   Is this against their ToS? No <https://www.redditinc.com/policies> <sup>[[Archive.org]][385]</sup>
...
-   Is this against their ToS? Yes <https://slashdotmedia.com/terms-of-use/> <sup>[[Archive.org]][386]</sup>
...
-   Is this against their ToS? No <https://telegram.org/tos> <sup>[[Archive.org]][387]</sup>
...
Their appeal process is just sending an e-mail to <recover@telegram.org> <sup>[[Archive.org]][388]</sup> stating your phone number and issue and hope they answer.
...
-   Is this against their ToS? No <https://tutanota.com/terms/> <sup>[[Archive.org]][389]</sup>
...
This time you will need to submit an appeal using a form[^345], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it is possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
-   Is this against their ToS? No <https://www.twitch.tv/p/en/legal/terms-of-service/> <sup>[[Archive.org]][390]</sup>
...
-   Is this against their ToS? **Yes** <https://www.whatsapp.com/legal/updates/terms-of-service-eea> <sup>[[Archive.org]][391]</sup>
...
You are going to have to find a different way to post there using at least seven proxies[^346] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
End-to-end Encryption[^347] (aka e2ee) is a rather simple concept. It just means only you and your destination know each-others public encryption keys and no one in between that would be eavesdropping would be able to decrypt the communication.
...
More on this topic on this YouTube video: <https://www.youtube.com/watch?v=zSQtyW_ywZc> <sup>[[Invidious]][394]</sup>
...
Zero-Access Encryption[^349] at rest is used when you store data at some provider (let us say your chat history or chat backups) but this history or backup is encrypted on your side and cannot be read or decrypted by the provider hosting it.
...
Such feature would have prevented important hacks such as the Cambridge Analytica scandal[^350] if it was implemented.
...
-   The Sealed Sender option[^351].

-   The Private Contact Discovery[^352].

-   The Private Group System[^353].
...
-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_instant_messaging_protocols> <sup>[[Archive.org]][395]</sup>

-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_cross-platform_instant_messaging_clients> <sup>[[Archive.org]][396]</sup>

-   Secure Messaging Apps <https://www.securemessagingapps.com/> <sup>[[Archive.org]][397]</sup>

-   ProtonMail Blog, <https://protonmail.com/blog/whatsapp-alternatives/> <sup>[[Archive.org]][398]</sup>

-   Whonix Documentation, Instant Messenger Chat <https://www.whonix.org/wiki/Chat> <sup>[[Archive.org]][399]</sup>
...
I will recommend these option (as also recommend by privacytools.io[^354]'[^355]):

-   Briar (<https://briarproject.org/> <sup>[[Archive.org]][400]</sup>)*

-   OnionShare version >2.3 (<https://onionshare.org/> <sup>[[Archive.org]][401]</sup>)*

-   Jami (<https://jami.net/> <sup>[[Archive.org]][402]</sup>)

-   Element/Matrix.org (<https://element.io/> <sup>[[Archive.org]][403]</sup>)
...
If you must do so, you should take some steps because you cannot really trust any organization to protect your anonymity[^356]:
...
For this, I strongly recommend the use of SecureDrop[^357] (<https://securedrop.org/> <sup>[[Archive.org]][404]</sup>) which is an open-source project from the Freedom of the Press foundation.

Ideally you should use SecureDrop over Tor and you will find a curated list of those here <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][405]</sup>
...
-   Check regularly the eventual appearance of compromise of any of your identities using <https://haveibeenpwned.com/> <sup>[[Archive.org]][406]</sup> (obviously from a safe environment).
...
The process is fairly simple and all you will need is to follow Veracrypt tutorial here: <https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html> <sup>[[Archive.org]][407]</sup>
...
Qubes OS recommends using their own utility for backups as documented here <https://www.qubes-os.org/doc/backup-restore/> <sup>[[Archive.org]][408]</sup> . But I think it is just a hassle and provides limited added value unless you just want to back-up a single Qube. So instead, I am also recommending just making a full image with Clonezilla which will remove all the hassle and bring you back a working system in a few easy steps.
...
Self-hosting (using Nextcloud for instance) is also a possibility provided you do have an anonymous hosting. You could pick one from the list here that does accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][412]</sup>

**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][413]</sup>**) and it is Njalla <https://njal.la/>**

#### Cloud-hosting:

For smaller files, consider Cryptpad.fr as recommended by Privacytools.io at <https://privacytools.io/providers/cloud-storage/> <sup>[[Archive.org]][414]</sup> (limited to 1GB total).
...
If you just want to save information (text), I will recommend the use secure and private pastebins[^359]. Mostly I will stick to the ones recommended by privacytools.io (<https://privacytools.io/providers/paste/> <sup>[[Archive.org]][415]</sup> ):
...
To that the answer is very simple and a clear consensus for everyone: <https://syncthing.net/> <sup>[[Archive.org]][416]</sup>
...
So, the fact is that it is very unlikely[^365]'[^366] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^367]'[^368]'[^369] if they are fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^370] for 11$ and use it to erase any disk.
...
Trim should be sufficient in most cases and you could just use the blkdiscard command to force an entire device trim as explained here: <https://wiki.archlinux.org/index.php/Solid_state_drive#Trim_an_entire_device> <sup>[[Archive.org]][422]</sup>
...
-   Option A: Follow one of these tutorials:
    ...
    -   <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][425]</sup>

    -   <https://linoxide.com/linux-command/commands-wipe-disk-linux/> <sup>[[Archive.org]][426]</sup>

    -   <https://wiki.archlinux.org/index.php/Securely_wipe_disk> <sup>[[Archive.org]][427]</sup>

I recommend using dd or shred for this purpose.

-   Option B: Install and use BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][423]</sup> or follow this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][428]</sup>
...
-   Option B: Use external tools such as:
    ...
    -   Eraser (open-source): <https://eraser.heidi.ie/download/> <sup>[[Archive.org]][429]</sup>

    -   KillDisk Free: <http://killdisk.com/killdisk-freeware.htm> <sup>[[Archive.org]][430]</sup>
...
Unfortunately, the MacOS Recovery disk utility will not be able to perform a secure erase of your SSD drive as stated in Apple documentation <https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac> <sup>[[Archive.org]][431]</sup>.
...
-   <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][432]</sup>

-   <https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac> <sup>[[Archive.org]][433]</sup> or using Veracrypt full disk encryption.
...
Follow this tutorial: <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][432]</sup> and use the secure erase option from Disk Utility which should work fine on HDD and Thumb drives.
...
-   Right Click a file/folder:
    ...
    -   BleachBit: Shred with BleachBit (or see this tutorial from the EFF <https://ssd.eff.org/en/module/how-delete-your-data-securely-windows> <sup>[[Archive.org]][435]</sup>)
...
-   Decrypt and re-encrypt (using LUKS for instance following this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][436]</sup>) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.

-   Trim using ```fstrim --all``` and then fill up the entire free space of the disk using a utility such as:

    -   BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][423]</sup>
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][424]</sup>) run these commands on the drive you want to fill:
...
-   You can do this the graphical way with BleachBit following this tutorial from the EFF: <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][428]</sup>

-   Or you can do this from the command line following this tutorial: <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][425]</sup> (For this purpose I recommend wipe and shred).
...
-   Decrypt and re-encrypt (using LUKS using this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][436]</sup> or Veracrypt from the graphical interface for instance) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.
...
-   Fill the free space using one of those methods:
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][424]</sup>) run these commands:
...
Please follow this documentation to Trim within Qubes OS: <https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][437]</sup>
...
-   Decrypt and re-encrypt the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space. I didn't find a reliable tutorial on how to do this safely on Qubes OS but it's possible this Tutorial could work as well <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][436]</sup> (at your own risk, this has not been tested yet).

-   Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][437]</sup>) and then trim using "fstrim --all" and then fill up the entire free space of the disk using an utility such as:
...
If Trim is supported and enabled by your Linux Distribution for your external SSD drive. There should be no issue in securely deleting data normally and just issue an "fstrim --all" from terminal to trim the drive. Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][437]</sup>) to enable trim on a drive.
...
-   If your file system is APFS, you do not need to worry about Trim, it apparently happens asynchronously as the OS writes data[^376] according to their own documentation.
...
-   If your file system is HFS+, you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][438]</sup>)

![][439]

#### System/Internal, External HDD drive or a Thumb Drive:

Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^377]. So, you are left with using other tools:

-   Permanent Eraser <http://www.edenwaith.com/products/permanent%20eraser/> <sup>[[Archive.org]][440]</sup>

-   From the terminal you can use the "rm --P filename" command which should erase the file and overwrite it as explained in this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][441]</sup>.
...
Consider also reading this documentation if you're going with Whonix <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][304]</sup> as well as their general hardening tutorial for all platforms here <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][442]</sup>

### Removing Metadata from Files/Documents/Pictures:

#### Pictures and videos:

On Windows, MacOS and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][443]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][444]</sup>) that allows viewing and/or removing those properties.

**ExifTool is natively available on TAILS and Whonix Workstation.**

##### ExifCleaner:

Just install it from <https://exifcleaner.com/> <sup>[[Archive.org]][444]</sup>, run and drag and drop the files into the GUI.
...
Here is a tutorial to remove metadata from a Picture using OS provided tools: <https://www.purevpn.com/internet-privacy/how-to-remove-metadata-from-photos> <sup>[[Archive.org]][445]</sup>

##### Cloaking/Obfuscating to prevent picture recognition:

Consider the use of Fawkes <https://sandlab.cs.uchicago.edu/fawkes/> <sup>[[Archive.org]][446]</sup> (<https://github.com/Shawn-Shan/fawkes> <sup>[[Archive.org]][447]</sup>) to cloak the images from picture recognition tech on various platforms.

Or if you want on-line versions, consider:

-   <https://lowkey.umiacs.umd.edu/> <sup>[[Archive.org]][448]</sup>

-   <https://adversarial.io/> <sup>[[Archive.org]][449]</sup>

#### PDF Documents:

##### PDFParanoia (Linux/Windows/MacOS/QubesOS):

Consider using <https://github.com/kanzure/pdfparanoia> <sup>[[Archive.org]][450]</sup> which will remove metadata and watermarks on any PDF.
...
First, here is a tutorial to remove metadata from Office documents: <https://support.microsoft.com/en-us/office/remove-hidden-data-and-personal-information-by-inspecting-documents-presentations-or-workbooks-356b7b5d-77af-44fe-a07f-9aa4d085966f> <sup>[[Archive.org]][451]</sup>. Make sure however that you do use the latest version of Office with the latest security updates.

Alternatively, on Windows, MacOS, Qubes OS, and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][443]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][444]</sup>) that allows viewing and/or removing those properties
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^378] (<https://0xacab.org/jvoisin/mat2> <sup>[[Archive.org]][452]</sup>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^379] and available on TAILS by default[^380].
...
Note that it's possible to run Whonix in Live mode leaving no traces when you shut down the VMs, consider reading their documentation here <https://www.whonix.org/wiki/VM_Live_Mode> <sup>[[Archive.org]][454]</sup> and here <https://www.whonix.org/wiki/Warning#Whonix_.E2.84.A2_Persistence_vs_Live_vs_Amnesic> <sup>[[Archive.org]][196]</sup>.
...
Lastly you can also disable Gatekeeper altogether by issuing the following command in terminal[^381]:
...
Many of those can be deleted using some various commercial third-party tools but I would personally recommend using the free and well-known Onyx which you can find here: <https://www.titanium-software.fr/en/onyx.html> <sup>[[Archive.org]][455]</sup>. Unfortunately, it is closed-source but it is notarized, signed and has been trusted for many years.
...
-   If your file system is HFS+ (or any other than APFS), you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][438]</sup>).

![][439]

### Linux (Qubes OS):

Please consider their guidelines <https://github.com/Qubes-Community/Contents/blob/master/docs/security/security-guidelines.md> <sup>[[Archive.org]][456]</sup>

If you are using Whonix on Qubes OS, please consider following some of their guides:

-   Whonix System Hardening guide <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][442]</sup>

-   Enabling App Armor on Qubes <https://www.whonix.org/wiki/Qubes/AppArmor> <sup>[[Archive.org]][457]</sup>

-   Also consider the use of Linux Kernel Guard <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][458]</sup>
...
Nevertheless, you might want to clean some logs. Just use this convenient tool: <https://github.com/sundowndev/go-covermyass> <sup>[[Archive.org]][459]</sup> (instructions on the page)
...
Consider the use of Linux Kernel Guard as an added measure <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][458]</sup>
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^280]), your network connections, app crash information and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   Launch Regedit using this tutorial: <https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11> <sup>[[Archive.org]][460]</sup>
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^273] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you should not do this on the Decoy OS.

-   Download Shellbag Analyzer & Cleaner from <https://privazer.com/en/download-shellbag-analyzer-shellbag-cleaner.php> <sup>[[Archive.org]][461]</sup>
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php> <sup>[[Archive.org]][462]</sup>
    ...
    -   (If you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^284]). Secure Deletion with Trim[^281] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
-   Get and install the latest version from BleachBit here <https://www.bleachbit.org/download> <sup>[[Archive.org]][463]</sup>
...
-   <https://justdeleteme.xyz/> <sup>[[Archive.org]][464]</sup>

-   <https://justgetmydata.com/> <sup>[[Archive.org]][465]</sup>
...
Go to their "Remove outdated content from Google Search" page here: <https://search.google.com/search-console/remove-outdated-content> <sup>[[Archive.org]][466]</sup> and submit a request accordingly.
...
Go to their "Content Removal" page here: <https://www.bing.com/webmasters/tools/contentremoval> <sup>[[Archive.org]][467]</sup> and submit a request accordingly.
...
DuckDuckGo does not store cached version of pages[^382] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^383] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
Once have your Yandex account, head to the Yandex Webmaster tools <https://webmaster.yandex.com> <sup>[[Archive.org]][468]</sup> and then select Tools and Delete URL <https://webmaster.yandex.com/tools/del-url/> <sup>[[Archive.org]][469]</sup>
...
Yes, Yahoo Search still exists but as per their help page <https://help.yahoo.com/kb/SLN4530.html> <sup>[[Archive.org]][470]</sup> , there is no way to remove information or refresh information besides waiting. This could take 6 to 8 weeks.
...
As far as I know, there is no way to remove information from Wikipedia articles themselves but if you just want to remove traces of your username from it (as a user that contributed), you can do so by following these steps: <https://en.wikipedia.org/wiki/Wikipedia:Courtesy_vanishing> <sup>[[Archive.org]][471]</sup>
...
A commonly used technique which combines the idea of a Dead Drop[^384] and Secure Communication Obfuscation[^385] through Steganography[^386] and/or Kleptography[^387] and has many names such as Koalang[^388] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^252]**.**
...
-   Consider the use of Haven <https://guardianproject.github.io/haven/> <sup>[[Archive.org]][472]</sup> on some old android phone to keep watch on your home/room while you are away.

-   Doxx "yourself" and your identities from time to time by looking for them yourself online using various search engines to monitor your online identities. You can even automate the process somewhat using various tools such as Google Alerts <https://www.google.com/alerts> <sup>[[Archive.org]][473]</sup>.
...
-   Never ever leave your laptop unattended/on/unlocked anywhere when conducting sensitive activities. Remember the story of Ross Ulbricht and his arrest <https://en.wikipedia.org/wiki/Ross_Ulbricht#Silk_Road,_arrest_and_trial> <sup>[[Archive.org]][474]</sup>.

-   Check for tampering regularly (not only your devices but also your home/room).

-   If you can, do not talk to the police (at least if you are in the US) <https://www.youtube.com/watch?v=d-7o9xYp7eE> <sup>[[Invidious]][475]</sup> without a lawyer.
...
-   Read those tips here <https://www.whonix.org/wiki/DoNot> <sup>[[Archive.org]][301]</sup>

-   Finally, have common sense, do not be dumb, look and learn from others' mistakes:

    -   RSA Conference 2020, When Cybercriminals with Good OpSec Attack <https://www.youtube.com/watch?v=zXmZnU2GdVk> <sup>[[Invidious]][476]</sup>

    -   DEFCON 22, Adrian Crenshaw- Dropping Docs on Darknets: How People Got Caught, <https://www.youtube.com/watch?v=eQ2OZKitRwc> <sup>[[Invidious]][477]</sup> ([Slides][] <sup>[[Archive.org]][478]</sup>)

    -   Bad Opsec - How Tor Users Got Caught, <https://www.youtube.com/watch?v=GR_U0G-QGA0> <sup>[[Invidious]][479]</sup>
...
Keep in mind that many countries have specific laws to compel you to reveal your passwords that could override your "right to remain silent". See this Wikipedia article: <https://en.wikipedia.org/wiki/Key_disclosure_law> <sup>[[Archive.org]][480]</sup> and this other visual resource with law references <https://www.gp-digital.org/world-map-of-encryption/> <sup>[[Archive.org]][481]</sup>.
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> <sup>[[Archive.org]][482]</sup> [^389]

"In the end the Party would announce that two and two made five, and you would have to believe it." -- George Orwell, 1984.

# Donations:

This project has no funding and donations are welcome at <https://anonymousplanet.org/donations.html> <sup>[[Mirror]][483]</sup> <sup>[[Archive.org]][484]</sup> <sup>[[Tor Mirror]][485]</sup> or directly by sending Monero (XMR) to this address: ```4549BGJrEPBfpiPRL9CVGzGMgJnC1Dzf8EXLVfY8Ukrnj7LzkTV611dGf9tuQHiSQjbixsNWiffNiV5fPB3LkyF7UXi3vwQ```
...
-   **Special Thanks to Edward Snowden and who inspired me to write this guide (buy and read his book please <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>** <sup>[[Archive.org]][489]</sup>**)**
...
As written earlier in this guide and as noted by Privacytools.io[^390], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple of utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   Download and install W10Privacy from <https://www.w10privacy.de/english-home/> <sup>[[Archive.org]][490]</sup>
...
-   Download and run WindowsSpyBlocker from <https://crazymax.dev/WindowsSpyBlocker/download/> <sup>[[Archive.org]][491]</sup>
...
<https://www.microsoft.com/en-us/software-download/windows10> <sup>[[Archive.org]][492]</sup>
...
-   System Rescue:
    ...
    -   Create a System Rescue USB disk following these instructions <https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/> <sup>[[Archive.org]][493]</sup> (download the ISO and write to an USB stick with Rufus).
...
-   SATA SSD:
    ...
    -   ATA Secure Erase:
        ...
        -   Follow one of these tutorials
            ...
            -   <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing> <sup>[[Archive.org]][494]</sup>

            -   <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase> <sup>[[Archive.org]][495]</sup>

            -   <https://tinyapps.org/docs/wipe_drives_hdparm.html> <sup>[[Archive.org]][496]</sup>

    -   ATA Sanitize:

        -   Follow this tutorial <https://tinyapps.org/docs/ata_sanitize_hdparm.html> <sup>[[Archive.org]][497]</sup>
...
-   NVMe SSD:
    ...
    -   Follow one of these tutorials:
        ...
        -   <https://tinyapps.org/docs/nvme-secure-erase.html> <sup>[[Archive.org]][498]</sup>

        -   <https://tinyapps.org/docs/nvme-sanitize.html> <sup>[[Archive.org]][499]</sup>

# Appendix E: Clonezilla

-   Get Clonezilla by just following these instructions: <https://clonezilla.org/liveusb.php> <sup>[[Archive.org]][500]</sup> (I recommend the Alternative version AMD64 that should work with most recent laptops)

-   Boot from Clonezilla

-   Follow these steps to make a backup: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image> <sup>[[Archive.org]][501]</sup>
...
-   You are done, if you need to restore, follow these instructions: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image> <sup>[[Archive.org]][502]</sup>
...
-   Download and install Tor Browser according to the instructions from <https://www.torproject.org/download/> <sup>[[Archive.org]][503]</sup>

-   Open Tor Browser

-   Click the little shield Icon and select your Security level (see <https://tb-manual.torproject.org/security-settings/> <sup>[[Archive.org]][504]</sup> for details):
...
If you are experiencing issues connecting to Tor due to Censorship or Blocking, you might consider using Tor bridges as explained here: <https://bridges.torproject.org/> <sup>[[Archive.org]][505]</sup>
...
-   Native Tools:
    ...
    -   Windows 10 Disk Cleanup Utility: <https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68> <sup>[[Archive.org]][506]</sup>
...
-   Windows 10 Optimize Utility (Defrag on HDD Drives): <https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a> <sup>[[Archive.org]][507]</sup>
...
-   Third Party Tools:
    ...
    -   The open-source utility BleachBit <https://www.bleachbit.org/> <sup>[[Archive.org]][508]</sup>

    -   The closed-source utility PrivaZer <https://privazer.com/> <sup>[[Archive.org]][509]</sup>
...
-   The Windows USN journal which stores plenty of information[^392].

-   The Windows System Resource Usage Monitor (SRUM)[^393].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^394].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^395]) or System Rescue CD (<https://www.system-rescue.org/> <sup>[[Archive.org]][510]</sup>) for this but personally, I will recommend the use of ShredOS.

Feel free do go with DBAN instead if you want (using this tutorial: <https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148> <sup>[[Archive.org]][511]</sup>), the process is basically the same but will not work out of the box with UEFI laptops.

If you want to go with System-Rescue, just head to their website and follow the instructions.

## Windows:

-   Download ShredOS from <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][512]</sup>

-   Unzip the ISO file

-   Download Rufus from <https://rufus.ie/> <sup>[[Archive.org]][513]</sup>
...
-   Follow instructions on <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][512]</sup>
...
-   SanDisk DashBoard: <https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information> <sup>[[Archive.org]][514]</sup>

-   Seagate SeaTools: <https://www.seagate.com/support/downloads/seatools/> <sup>[[Archive.org]][515]</sup>

-   Samsung Magican: <https://www.samsung.com/semiconductor/minisite/ssd/download/tools/> <sup>[[Archive.org]][516]</sup>

-   Kingston SSD Manager: <https://www.kingston.com/unitedstates/en/support/technical/ssdmanager> <sup>[[Archive.org]][517]</sup>

-   Lenovo:

    -   Most likely native utility available within the BIOS/UEFI, please check

    -   Drive Erase Utility: <https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad> <sup>[[Archive.org]][518]</sup>

-   Crucial Storage Executive: <https://www.crucial.com/support/storage-executive> <sup>[[Archive.org]][519]</sup>

-   Western Digital Dashboard: <https://support.wdc.com/downloads.aspx?p=279> <sup>[[Archive.org]][520]</sup>

-   HP: Follow instructions on <https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd> <sup>[[Archive.org]][521]</sup>

-   Transcend SSD Scope: <https://www.transcend-info.com/Support/Software-10/> <sup>[[Archive.org]][522]</sup>

-   Dell:

    -   Most likely native utility available within the BIOS/UEFI, please check <https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt> <sup>[[Archive.org]][523]</sup>

## Tools that provide only support from running OS (for external drives).

-   Toshiba Storage Tools: <https://www.toshiba-storage.com/downloads/> <sup>[[Archive.org]][524]</sup>
...
Some might be very efficient[^396] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you if it is supported or not like CrystalDiskInfo[^397] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives][Appendix J: Manufacturer tools for Wiping HDD and SSD drives:].
...
Follow this good tutorial: <https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux> <sup>[[Archive.org]][525]</sup>

### ATA/NVMe Operations (Secure Erase/Sanitize):

**It is not "recommended". Please read the disclaimers here <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase>** <sup>[[Archive.org]][495]</sup> **and here <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing>** <sup>[[Archive.org]][494]</sup>
...
See also this tutorial <https://code.mendhak.com/securely-wipe-ssd/> <sup>[[Archive.org]][526]</sup>
...
According to Apple Documentation[^388], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
If your disk supports it but it is not enabled in MacOS. You could try issuing a "sudo trimforce enable" command from the Terminal and see if it enables Trim on your external SSD. And then again check the first aid command if it is not APFS (see this Tutorial for info <https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789> <sup>[[Archive.org]][527]</sup>)

If it does not work, I am not aware of any reliable method to enable TRIM besides the commercial utility Trim Enabler here <https://cindori.org/trimenabler/> <sup>[[Archive.org]][528]</sup> which claims support for external drives.
...
Download the latest Debian testing amd64 netinst ISO from <https://www.debian.org/CD/netinst/> <sup>[[Archive.org]][529]</sup>
...
-   Lenovo ThinkPads: <https://support.lenovo.com/be/en/solutions/migr-68369> <sup>[[Archive.org]][530]</sup>

-   HP (all): <https://support.hp.com/gb-en/document/c06204100> <sup>[[Archive.org]][531]</sup>

-   Dell (all): <https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe> <sup>[[Archive.org]][532]</sup>

-   Acer (Travelmate only): <https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks> <sup>[[Archive.org]][533]</sup>
...
-   If you really must take them with you, you could consider the use of a faraday cage[^398] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^399] for their effectiveness. If you cannot afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

    -   Warning: consider that sensor data itself can also be reliably used to track you[^400]'[^401].
...
-   **Encrochat: <https://en.wikipedia.org/wiki/EncroChat>** <sup>[[Archive.org]][534]</sup>

-   **Sky ECC: <https://en.wikipedia.org/wiki/Sky_ECC>** <sup>[[Archive.org]][535]</sup>
...
There are three VPN companies recommended by privacytools.io (<https://privacytools.io/providers/vpn/> <sup>[[Archive.org]][536]</sup>) that accept cash payments: Mullvad, iVPN and ProtonVPN.
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^402] and touch DNA[^403].
...
-   Do not lick the envelope or the stamps[^404] if you use them to avoid leaving DNA traces.
...
This does offer some disadvantage as Monero is not perfect as explained earlier in this guide and some global adversaries could maybe still track you. You will need to get Monero from an Exchange using the normal financial system and then pick a hosting (list here <https://www.getmonero.org/community/merchants/#exchanges> <sup>[[Archive.org]][412]</sup>)

You could use <https://evolution-host.com/vps-hosting.php> <sup>[[Archive.org]][537]</sup> (who supports Whonix) which does accept Monero payments. Or pick one from the list here that do accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][412]</sup>

**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][413]</sup>**) and it is Njalla <https://njal.la/>** <sup>[[Archive.org]][538]</sup> **and I would recommend them as well.**
...
There are plenty of tutorials on how to do this like this one <https://proprivacy.com/vpn/guides/create-your-own-vpn-server> <sup>[[Archive.org]][539]</sup>
...
-   (Windows/Linux/MacOS) <https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/> <sup>[[Archive.org]][540]</sup>

-   (Windows/Linux/MacOS) <https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel> <sup>[[Archive.org]][541]</sup>

-   (Windows) <https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/> <sup>[[Archive.org]][542]</sup>

-   (Linux/MacOS) <https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/> <sup>[[Archive.org]][543]</sup>
...
-   Download and install Putty from <https://www.putty.org/> <sup>[[Archive.org]][544]</sup>
...
In that case my last resort recommendation is to connect safely **from a distance** to a Public Wi-Fi (See [Find some safe places with decent public Wi-Fi][Find some safe places with decent public Wi-Fi:]) using your laptop and TAILS "unsafe browser". See <https://tails.boum.org/contribute/design/Unsafe_Browser/> <sup>[[Archive.org]][546]</sup>.
...
Consider reading this older but still relevant guide <https://archive.flossmanuals.net/bypassing-censorship/index.html> <sup>[[Archive.org]][547]</sup>
...
**Personally, I would recommend the AWUS036 series in the Alfa brand of adapters (see <https://www.alfa.com.tw/>** <sup>[[Archive.org]][549]</sup>**).** But you could also go with some other brands if you want such as the TP-Link TL-WN722 (see <https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/> <sup>[[Archive.org]][550]</sup>).

See this post for a comparison of various adapters: <https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html> <sup>[[Archive.org]][551]</sup> (Usually those antennas are used by Penetration Testers to probe Wi-Fis from a distance and are often discussed within the scope of the Kali Linux distribution).
...
**Do not forget tho that this will only delay a motivated adversary. Your signal can be triangulated easily by a motivated adversary in a matter of minutes once they reach the physical location of the Wi-Fi you're connecting to (for instance using a device such as AirCheck <https://www.youtube.com/watch?v=8FV2QZ1BPnw>** <sup>[[Invidious]][552]</sup>**, also see their other products here <https://www.netally.com/products/#wifi_s>** <sup>[[Archive.org]][553]</sup>**). These products can easily be deployed on mobile units (in a Car for instance) and pinpoint your location in a matter of minutes.**
...
-   Whonix Tutorial (should work with any VPN provider): <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][281]</sup> (use the Linux configurations below to get the necessary configuration files)

-   Windows Tutorials:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-windows/> <sup>[[Archive.org]][554]</sup>

    -   iVPN: <https://www.ivpn.net/apps-windows> <sup>[[Archive.org]][555]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-windows-vpn-application/> <sup>[[Archive.org]][556]</sup>

-   MacOS:

    -   Mullvad: <https://mullvad.net/en/help/install-and-use-mullvad-app-macos/> <sup>[[Archive.org]][557]</sup>

    -   IVPN: <https://www.ivpn.net/apps-macos/> <sup>[[Archive.org]][558]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-mac-vpn-application/> <sup>[[Archive.org]][559]</sup>

-   Linux:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-linux/> <sup>[[Archive.org]][560]</sup>

    -   iVPN: <https://www.ivpn.net/apps-linux/> <sup>[[Archive.org]][561]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/linux-vpn-setup/> <sup>[[Archive.org]][562]</sup>
...
-   Windows:
    ...
    -   iVPN: <https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/> <sup>[[Archive.org]][563]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/what-is-kill-switch/> <sup>[[Archive.org]][564]</sup>

    -   Mullvad: <https://mullvad.net/en/help/using-mullvad-vpn-app/#killswitch> <sup>[[Archive.org]][565]</sup>
...
-   MacOS:
    ...
    -   ProtonVPN same as Windows with the client, the option should be in the provided VPN client <https://protonvpn.com/blog/macos-vpn-kill-switch/> <sup>[[Archive.org]][566]</sup>

-   Linux:

    -   Mullvad:

        -   <https://mullvad.net/en/help/wireguard-and-mullvad-vpn/> <sup>[[Archive.org]][567]</sup>

        -   <https://mullvad.net/en/help/linux-openvpn-installation/> <sup>[[Archive.org]][568]</sup>

    -   ProtonVPN: <https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md#kill-switch> <sup>[[Archive.org]][569]</sup>

    -   iVPN:

        -   <https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/> <sup>[[Archive.org]][570]</sup>

        -   <https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/> <sup>[[Archive.org]][571]</sup>

# Appendix S: Check your network for surveillance/censorship using OONI

So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^243].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^405].
...
The probe can be found here: <https://ooni.org/install/> <sup>[[Archive.org]][572]</sup> for various platforms (iOS, Android, Windows, MacOS, and Linux).

# Appendix T: Checking files for malware

## Integrity (if available):

Usually, integrity checks[^406] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^407], more recently MD5[^408] but those present several weaknesses (CRC, MD5[^409]) that makes them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^410] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it is usually recommended to use SHA[^411] based hashes and the most used is probably the SHA-2[^412] based SHA256 for verifying file integrity. SHA is much more resistant to collisions[^413] than CRC and MD5. And collisions with SHA256 or SHA512 are rare and hard to compute for an adversary.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^416] for instance) using public key cryptography[^417].
...
-   Install GPG for your OS:
    ...
    -   Windows: gpg4win (<https://www.gpg4win.org/> <sup>[[Archive.org]][573]</sup>)

    -   MacOS: GPGTools (<https://gpgtools.org/> <sup>[[Archive.org]][574]</sup>)
...
-   <https://support.torproject.org/tbb/how-to-verify-signature/> <sup>[[Archive.org]][575]</sup>

-   <https://tails.boum.org/install/vm-download/index.en.html> <sup>[[Archive.org]][576]</sup> (See Basic OpenPGP verification).

-   <https://www.whonix.org/wiki/Verify_the_Whonix_images> <sup>[[Archive.org]][577]</sup>
...
You might be asking yourself, what about Anti-Virus solutions? Well, no ... these are not perfect solutions against many modern malware and viruses using polymorphic code[^418]. But it does not mean they cannot help against less sophisticated and known attacks. It depends how to use them as AV software can become an attack vector in itself.
...
Some will just argue against them broadly like Whonix[^419] but this topic is being discussed and disputed even at Whonix[^420] by other members of their community.
...
-   Yes, there are viruses and malware for Linux[^421]'[^422]'[^423]'[^424]'[^425]

-   Yes, there are viruses and malware for MacOS[^433]'[^426][^427]'[^428][^429]
...
-   Do use Open-Source non-real time offline Anti-Virus/Anti-Malware tools as an added measure to scan some files such as:
    ...
    -   Windows/Linux/MacOS/Qubes OS: ClamAV (<https://www.clamav.net/> <sup>[[Archive.org]][578]</sup>)

    -   Linux/Qubes OS: RFXN Linux Malware Detect (<https://github.com/rfxn/linux-malware-detect> <sup>[[Archive.org]][579]</sup>)

    -   Linux/Qubes OS: Chkrootkit (<http://www.chkrootkit.org/> <sup>[[Archive.org]][580]</sup>)
...
-   You could also use online services for **non-sensitive files*** such as VirusTotal (<https://www.virustotal.com/gui>) or Hybrid-analysis (<https://hybrid-analysis.com/>).
    ...
    -   You could also just check the VirusTotal database for the hash of your file if you don't want to send it over (see <https://developers.virustotal.com/v3.0/docs/search-by-hash> <sup>[[Archive.org]][581]</sup> (See the [Integrity (if available):] section again for guidance on how to generate hashes).

    -   Other tools are also available for non-sensitive files and a convenient list is right here: <https://github.com/rshipp/awesome-malware-analysis#online-scanners-and-sandboxes> <sup>[[Archive.org]][582]</sup>

* **Please be aware that while VirusTotal might seem very practical for scanning various files, their "privacy policy" is problematic (see <https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy>** <sup>[[Archive.org]][583]</sup>**) and states:**
...
Again, regarding the PDFs of this guide and as explained in the README of my repository, you could check for anomalies using PDFID which you can download at <https://blog.didierstevens.com/programs/pdf-tools/> <sup>[[Archive.org]][584]</sup>
...
-   **Qubes OS:** Consider using <https://github.com/QubesOS/qubes-app-linux-pdf-converter> <sup>[[Archive.org]][585]</sup> which will convert your PDF into a flattened image file. This should theoretically remove any malicious code in it. Note that this will also render the PDF formatting useless (such as links, headings, bookmarks, and references).

-   **(Deprecated) Linux/Qubes OS** (or possibly MacOS through Homebrew or Windows through Cygwin): Consider not using <https://github.com/firstlookmedia/pdf-redact-tools> <sup>[[Archive.org]][586]</sup> which will also turn your PDF into a flattened image file. Again, this should theoretically remove any malicious code in it. Again, this will also render the PDF formatting useless (such as links, headings, bookmarks, and references). **Note that this tool is deprecated and relies on a library called "ImageMagick" which is known for several security issues**[^430]**. You should not use this tool even if it is recommended in some other guides.**

-   **Windows/Linux/Qubes/OS/MacOS:** Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][587]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^431]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

#### Other type of files:

Here are some various resources for this purpose where you will find what tool to use for what type:

-   For Documents/Pictures: Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][587]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^432]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> <sup>[[Archive.org]][588]</sup> (warning, many of those tools might be harder to use on Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^433] that might be of use).

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis> <sup>[[Archive.org]][582]</sup>

-   This interesting PDF detailing which tool to use for which file type <https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf> <sup>[[Archive.org]][589]</sup>
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^434]. Those apps do not require installation your system and can be run from an USB key or anywhere else.
...
So, you might ask yourself, what is Virtualization[^435]?
...
In those cases, it might be necessary to use Tor bridges to connect to the Tor network (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][215]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][299]</sup>).

Bridges are special Tor entry nodes that are not listed on the Tor public directory. Some of those are running on people running the Snowflake Browser extension[^436] while others are running on various servers around the world. Most of those bridges are running some type of obfuscation method called obfs4[^437].

Here is the definition from the Tor Browser Manual[^438]: "obfs4 makes Tor traffic look random, and prevents censors from finding bridges by Internet scanning. obfs4 bridges are less likely to be blocked than its predecessor, obfs3 bridges".
...
As per their definition from their manual[^438]: "meek transports make it look like you are browsing a major web site instead of using Tor. meek-azure makes it look like you are using a Microsoft web site".

First you should, proceed with the following checklist to make sure you cannot circumvent Tor Blocking (double-check) and try to use Tor Bridges (<https://bridges.torproject.org/> <sup>[[Archive.org]][505]</sup>).
...
-   **This project is more known than you think: <https://www.youtube.com/watch?v=nwkiU6GG-YU>** <sup>[[Invidious]][592]</sup>
...
The official guide is here: <https://telegra.ph/AME-Download-Guide-09-07> <sup>[[Archive.org]][593]</sup>

You can also build this ISO yourself from their scripts if you do not trust their provided ISO release using the guide here: <https://wiki.ameliorated.info/doku.php?id=documentation_20H2> <sup>[[Archive.org]][594]</sup>
...
[^1]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf> <sup>[[Archive.org]][595]</sup>. Section 13, Article 6, "The service provider must enable the use of Telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^2]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany> <sup>[[Archive.org]][362]</sup>

[^3]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil> <sup>[[Archive.org]][596]</sup>

[^4]: YouTube, <https://www.youtube.com/watch?v=6DGNZnfKYnU> <sup>[[Invidious]][597]</sup>

[^5]: Wikipedia, OSINT <https://en.wikipedia.org/wiki/Open-source_intelligence> <sup>[[Archive.org]][598]</sup>

[^6]: YouTube Internet Historian Playlist, HWNDU <https://www.youtube.com/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY> <sup>[[Invidious]][599]</sup>

[^7]: Wikipedia, 4chan <https://en.wikipedia.org/wiki/4chan> <sup>[[Archive.org]][600]</sup>

[^8]: PIA, See this good article on the matter <https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/> <sup>[[Archive.org]][601]</sup> (disclaimer: this is not an endorsement or recommendation for this commercial service).
...
[^10]: This World of Ours, James Mickens <https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf> <sup>[[Archive.org]][602]</sup>

[^11]: XKCD, Security <https://xkcd.com/538/> <sup>[[Archive.org]][603]</sup>

[^12]: Wikipedia, Threat Model <https://en.wikipedia.org/wiki/Threat_model> <sup>[[Archive.org]][604]</sup>

[^13]: Bellingcat <https://www.bellingcat.com/> <sup>[[Archive.org]][605]</sup>

[^14]: Wikipedia, Doxing <https://en.wikipedia.org/wiki/Doxing> <sup>[[Archive.org]][606]</sup>

[^15]: YouTube, Internet Historian, The Bikelock Fugitive of Berkeley <https://www.youtube.com/watch?v=muoR8Td44UE> <sup>[[Invidious]][607]</sup>

[^16]: BBC News, Tor Mirror <https://www.bbc.com/news/technology-50150981> <sup>[[Archive.org]][608]</sup>

[^17]: GitHub, Real World Onion websites <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][405]</sup>

[^18]: Tor Project, Who Uses Tor <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][609]</sup>

[^19]: Whonix Documentation, The importance of Anonymity <https://www.whonix.org/wiki/Anonymity> <sup>[[Archive.org]][610]</sup>

[^20]: Geek Feminism, <https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F> <sup>[[Archive.org]][611]</sup>

[^21]: Tor Project, Tor Users <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][609]</sup>

[^22]: PrivacyHub, Internet Privacy in the Age of Surveillance <https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/> <sup>[[Archive.org]][612]</sup>

[^23]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL> <sup>[[Archive.org]][613]</sup>

[^24]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][614]</sup>

[^25]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address> <sup>[[Archive.org]][615]</sup>

[^26]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention> <sup>[[Archive.org]][616]</sup>

[^27]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)> <sup>[[Archive.org]][617]</sup>

[^28]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network> <sup>[[Archive.org]][618]</sup>

[^29]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System> <sup>[[Archive.org]][619]</sup>

[^30]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking> <sup>[[Archive.org]][620]</sup>

[^31]: CensoredPlanet <https://censoredplanet.org/> <sup>[[Archive.org]][201]</sup>

[^32]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf> <sup>[[Archive.org]][621]</sup>

[^33]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole> <sup>[[Archive.org]][622]</sup>

[^34]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS> <sup>[[Archive.org]][623]</sup>

[^35]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS> <sup>[[Archive.org]][624]</sup>

[^36]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole> <sup>[[Archive.org]][625]</sup>

[^37]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication> <sup>[[Archive.org]][626]</sup>

[^38]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Archive.org]][626]</sup>

[^39]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Archive.org]][626]</sup>

[^40]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf> <sup>[[Archive.org]][627]</sup>

[^41]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_
. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/> <sup>[[Archive.org]][708]</sup>

[^130]: Wikipedia, Sentiment Analysis, <https://en.wikipedia.org/wiki/Sentiment_analysis> <sup>[[Archive.org]][709]</sup>

[^131]: EFF CoverYourTracks, <https://coveryourtracks.eff.org/> <sup>[[Archive.org]][710]</sup>

[^132]: Berkeley.edu, On the Feasibility of Internet-Scale Author Identification <https://people.eecs.berkeley.edu/~dawnsong/papers/2012%20On%20the%20Feasibility%20of%20Internet-Scale%20Author%20Identification.pdf> <sup>[[Archive.org]][711]</sup>

[^133]: SecuredTouch Blog, Behavioral Biometrics 101: Behavioral Biometrics vs. Behavioral Analytics <https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics> <sup>[[Archive.org]][712]</sup>

[^134]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/> <sup>[[Archive.org]][713]</sup>

[^135]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/> <sup>[[Archive.org]][714]</sup>

[^136]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/> <sup>[[Archive.org]][715]</sup>

[^137]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/> <sup>[[Archive.org]][716]</sup>

[^138]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html> <sup>[[Archive.org]][717]</sup>

[^139]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/> <sup>[[Archive.org]][718]</sup>

[^140]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/> <sup>[[Archive.org]][719]</sup>

[^141]: Brown Institute, Open-Source Investigation, <https://brown.columbia.edu/open-source-investigation/> <sup>[[Archive.org]][720]</sup>

[^142]: NewScientist, Facebook can recognize you in photos even if you're not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/> <sup>[[Archive.org]][721]</sup>

[^143]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679> <sup>[[Archive.org]][722]</sup>

[^144]: APNews, Chinese 'gait recognition' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a> <sup>[[Archive.org]][723]</sup>

[^145]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/> <sup>[[Archive.org]][724]</sup>

[^146]: Nature.com, Facial recognition technology can expose political orientation from naturalistic facial images <https://www.nature.com/articles/s41598-020-79310-1> <sup>[[Archive.org]][725]</sup>

[^147]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html> <sup>[[Archive.org]][726]</sup>

[^148]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804> <sup>[[Archive.org]][727]</sup>

[^149]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy> <sup>[[Archive.org]][728]</sup>

[^150]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/> <sup>[[Archive.org]][729]</sup>

[^151]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/> <sup>[[Archive.org]][730]</sup>

[^152]: Anyvision <https://www.anyvision.co/> <sup>[[Archive.org]][731]</sup>

[^153]: BuzzFeed.news, Surveillance Nation <https://www.buzzfeednews.com/article/ryanmac/clearview-ai-local-police-facial-recognition> <sup>[[Archive.org]][732]</sup>

[^154]: NEC, Neoface <https://www.nec.com/en/global/solutions/biometrics/face/neofacewatch.html> <sup>[[Archive.org]][733]</sup>

[^155]: The Guardian, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology> <sup>[[Archive.org]][734]</sup>

[^156]: YouTube, The Economist, China: facial recognition and state control <https://www.youtube.com/watch?v=lH2gMNrUuEY> <sup>[[Invidious]][735]</sup>

[^157]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/> <sup>[[Archive.org]][736]</sup>

[^158]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/> <sup>[[Archive.org]][737]</sup>

[^159]: BBC, WhatsApp photo drug dealer caught by 'groundbreaking' work <https://www.bbc.com/news/uk-wales-43711477> <sup>[[Archive.org]][738]</sup>

[^160]: CNN, Drug dealer jailed after sharing a photo of cheese that included his fingerprints <https://edition.cnn.com/2021/05/25/uk/drug-dealer-cheese-sentenced-scli-gbr-intl/index.html> <sup>[[Archive.org]][739]</sup>

[^161]: Vice.com, Cops Got a Drug Dealer's Fingerprints From Photos of His Hand on WhatsApp <https://www.vice.com/en/article/evqk9e/photo-of-fingerprints-used-to-arrest-drug-dealers> <sup>[[Archive.org]][740]</sup>

[^162]: JUSTIA Patent, Identification of taste attributes from an audio signal <https://patents.justia.com/patent/10891948> <sup>[[Archive.org]][741]</sup>

[^163]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177
: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Archive.org]][773]</sup>

[^196]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921> <sup>[[Archive.org]][774]</sup>

[^197]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140> <sup>[[Archive.org]][775]</sup>

[^198]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746> <sup>[[Archive.org]][776]</sup>

[^199]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification> <sup>[[Archive.org]][777]</sup>

[^200]: Wikipedia, Printing Steganography <https://en.wikipedia.org/wiki/Machine_Identification_Code> <sup>[[Archive.org]][778]</sup>

[^201]: MIT, SeeingYellow, <http://seeingyellow.com/> <sup>[[Archive.org]][779]</sup>

[^202]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524> <sup>[[Archive.org]][780]</sup>

[^203]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/> <sup>[[Archive.org]][781]</sup>

[^204]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer> <sup>[[Archive.org]][782]</sup>

[^205]: arXiv.org, Probing the Mystery of Cryptocurrency Theft:An Investigation into Methods for Taint Analysis <https://arxiv.org/pdf/1906.05754.pdf> <sup>[[Archive.org]][783]</sup>

[^206]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y> <sup>[[Invidious]][784]</sup>

[^207]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/> <sup>[[Archive.org]][785]</sup>

[^208]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler> <sup>[[Archive.org]][786]</sup>

[^209]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity> <sup>[[Archive.org]][787]</sup>

[^210]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007> <sup>[[Archive.org]][788]</sup>

[^211]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361> <sup>[[Archive.org]][789]</sup>

[^212]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/> <sup>[[Archive.org]][790]</sup>

[^213]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/> <sup>[[Archive.org]][791]</sup>

[^214]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics> <sup>[[Archive.org]][792]</sup>

[^215]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting> <sup>[[Archive.org]][793]</sup>

[^216]: Grayshirt, <https://www.grayshift.com/> <sup>[[Archive.org]][794]</sup>

[^217]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf> <sup>[[Archive.org]][795]</sup>

[^218]: Loup-Vaillant.fr, Rolling Your Own Crypto <https://loup-vaillant.fr/articles/rolling-your-own-crypto> <sup>[[Archive.org]][796]</sup>

[^219]: Dhole Moments, Crackpot Cryptography and Security Theater <https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/> <sup>[[Archive.org]][797]</sup>

[^220]: Vice.com, Why You Don't Roll Your Own Crypto <https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto> <sup>[[Archive.org]][798]</sup>

[^221]: YouTube, Great Crypto Failures <https://www.youtube.com/watch?v=loy84K3AJ5Q> <sup>[[Invidious]][799]</sup>
...
[^223]: Citizenlab.ca, Move Fast and Roll Your Own Crypto <https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/> <sup>[[Archive.org]][800]</sup>

[^224]: Jack Poon, The myth of military grade encryption <https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369> <sup>[[Archive.org]][801]</sup>

[^225]: Congruent Labs, Stop calling it "Military-Grade Encryption" <https://blog.congruentlabs.co/military-grade-encryption/> <sup>[[Archive.org]][802]</sup>

[^226]: IronCoreLabs Blog, "Military Grade Encryption" <https://blog.ironcorelabs.com/military-grade-encryption-69aae0145
: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][819]</sup>

[^322]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint> <sup>[[Archive.org]][880]</sup>

[^323]: Developers Google Blog,

    Guidance to developers affected by our effort to block less secure browsers and applications <https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html> <sup>[[Archive.org]][881]</sup>

[^324]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085> <sup>[[Archive.org]][882]</sup>

[^325]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern> <sup>[[Archive.org]][883]</sup>

[^326]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight> <sup>[[Archive.org]][884]</sup>

[^327]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html> <sup>[[Archive.org]][885]</sup>

[^328]: Vice.com, PornHub Announces 'Biometric Technology' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id> <sup>[[Archive.org]][886]</sup>

[^329]: Variety, China Launches Hotline to Report Online Comments That 'Distort' History or 'Deny' Its Cultural Excellence <https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/> <sup>[[Archive.org]][887]</sup>

[^330]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][614]</sup>

[^331]: Wikipedia, Zero-trust Security Model <https://en.wikipedia.org/wiki/Zero_trust_security_model> <sup>[[Archive.org]][888]</sup>

[^332]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization> <sup>[[Archive.org]][889]</sup>

[^333]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam> <sup>[[Archive.org]][890]</sup>

[^334]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration> <sup>[[Archive.org]][891]</sup>

[^335]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/> <sup>[[Archive.org]][892]</sup>

[^336]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^337]: Google Help <https://support.google.com/accounts/answer/114129?hl=en> <sup>[[Archive.org]][893]</sup>

[^338]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en> <sup>[[Archive.org]][894]</sup>

[^339]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695> <sup>[[Archive.org]][895]</sup>

[^340]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2> <sup>[[Archive.org]][896]</sup>

[^341]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en> <sup>[[Archive.org]][897]</sup>

[^342]: Jumio, ID verification features <https://www.jumio.com/features/> <sup>[[Archive.org]][898]</sup>

[^343]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/> <sup>[[Archive.org]][899]</sup>

[^344]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/][] <sup>[[Archive.org]][900]</sup>

[^345]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^346]: KnowYourMeme, Good Luck, I'm Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies> <sup>[[Archive.org]][901]</sup>

[^347]: Wikipedia, end-to-end encryption, <https://en.wikipedia.org/wiki/End-to-end_encryption> <sup>[[Archive.org]][902]</sup>

[^348]: Wikipedia, Forward Secrecy, <https://en.wikipedia.org/wiki/Forward_secrecy> <sup>[[Archive.org]][903]</sup>

[^349]: Protonblog, What is zero-access encryption and why it is important for security <https://protonmail.com/blog/zero-access-encryption/> <sup>[[Archive.org]][904]</sup>

[^350]: Wikipedia, Cambridge Analytica Scandal, <https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal> <sup>[[Archive.org]][905]</sup>

[^351]: Signal Blog, Technology preview: Sealed sender for Signal <https://signal.org/blog/sealed-sender/> <sup>[[Archive.org]][906]</sup>

[^352]: Signal Blog, Private Contact Discovery, <https://signal.org/blog/private-contact-discovery/> <sup>[[Archive.org]][907]</sup>

[^353]: Signal Blog, Private Group System, <https://signal.org/blog/signal-private-group-system/> <sup>[[Archive.org]][908]</sup>

[^354]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/> <sup>[[Archive.org]][909]</sup>

[^355]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/> <sup>[[Archive.org]][910]</sup>

[^356]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/> <sup>[[Archive.org]][911]</sup>

[^357]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop> <sup>[[Archive.org]][912]</sup>

[^358]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_
: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf> <sup>[[Archive.org]][947]</sup>

[^400]: arXiv, Deep-Spying: Spying using Smartwatch and Deep Learning <https://arxiv.org/abs/1512.05616> <sup>[[Archive.org]][948]</sup>

[^401]: Acm.org, Privacy Implications of Accelerometer Data: A Review of Possible Inferences <https://dl.acm.org/doi/pdf/10.1145/3309074.3309076> <sup>[[Archive.org]][949]</sup>

[^402]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M> <sup>[[Invidious]][950]</sup>

[^403]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA> <sup>[[Archive.org]][951]</sup>

[^404]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps> <sup>[[Archive.org]][952]</sup>

[^405]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection> <sup>[[Archive.org]][953]</sup>

[^406]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification> <sup>[[Archive.org]][954]</sup>

[^407]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check> <sup>[[Archive.org]][955]</sup>

[^408]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5> <sup>[[Archive.org]][956]</sup>

[^409]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security> <sup>[[Archive.org]][956]</sup>

[^410]: Wikipedia, Collisions <https://en.wikipedia.org/wiki/Collision_(computer_science)> <sup>[[Archive.org]][957]</sup>

[^411]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms> <sup>[[Archive.org]][958]</sup>

[^412]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2> <sup>[[Archive.org]][959]</sup>

[^413]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance> <sup>[[Archive.org]][960]</sup>

[^414]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity> <sup>[[Archive.org]][961]</sup>

[^415]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f> <sup>[[Archive.org]][962]</sup>

[^416]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard> <sup>[[Archive.org]][963]</sup>

[^417]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography> <sup>[[Archive.org]][964]</sup>

[^418]: Wikipedia, Polymorphic Code <https://en.wikipedia.org/wiki/Polymorphic_code> <sup>[[Archive.org]][965]</sup>

[^419]: Whonix Documentation, Use of AV, <https://www.whonix.org/wiki/Malware_and_Firmware_Trojans#The_Utility_of_Antivirus_Tools> <sup>[[Archive.org]][966]</sup>

[^420]: Whonix Forums, <https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8> <sup>[[Archive.org]][967]</sup>

[^421]: AV-Test Security Report 2018-2019, <https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf> <sup>[[Archive.org]][968]</sup>

[^422]: ZDNet, ESET discovers 21 new Linux malware families <https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/> <sup>[[Archive.org]][969]</sup>

[^423]: NakeSecurity, EvilGnome -- Linux malware aimed at your desktop, not your servers <https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/> <sup>[[Archive.org]][970]</sup>

[^424]: Immunify, HiddenWasp: How to detect malware hidden on Linux & IoT <https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot> <sup>[[Archive.org]][971]</sup>

[^425]: Wikipedia, Linux Malware <https://en.wikipedia.org/wiki/Linux_malware> <sup>[[Archive.org]][972]</sup>

[^426]: Wikipedia, MacOS Malware <https://en.wikipedia.org/wiki/MacOS_malware> <sup>[[Archive.org]][973]</sup>

[^427]: MacWorld, List of Mac viruses, malware and security flaws <https://www.macworld.co.uk/feature/mac-viruses-list-3668354/> <sup>[[Archive.org]][974]</sup>

[^428]: JAMF, The Mac Malware of 2020 <https://resources.jamf.com/documents/macmalware-2020.pdf> <sup>[[Archive.org]][975]</sup>

[^429]: MacOS Security and Privacy Guide, <https://github.com/drduh/macOS-Security-and-Privacy-Guide#viruses-and-malware> <sup>[[Archive.org]][259]</sup>

[^430]: ImageTragick.com, <https://imagetragick.com/> <sup>[[Archive.org]][976]</sup>

[^431]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][977]</sup>

[^432]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][977]</sup>

[^433]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/> <sup>[[Archive.org]][978]</sup>

[^434]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application> <sup>[[Archive.org]][979]</sup>

[^435]: Wikipedia, Virtualization <https://en.wikipedia.org/wiki/Virtualization> <sup>[[Archive.org]][980]</sup>

[^436]: Tor Project, Project Snowflake <https://snowflake.torproject.org/> <sup>[[Archive.org]][981]</sup>

[^437]: GitHub, Obfs4 Repository <https://github.com/Yawning/obfs4/> <sup>[[Archive.org]][982]</sup>

[^438]: Tor Browser Manual, Pluggable Transport <https://tb-manual.torproject.org/circumvention/> <sup>[[Archive.org]][983]</sup>
  ...
  [Your Network: 2]: #_Toc73904442
  [Your Face, Voice, Biometrics and Pictures: 2]: #_Toc73904461
  ...
  [271]: https://web.archive.org/web/https://www.whonix.org/wiki/KVM#Why_Use_KVM_Over_VirtualBox.3F
  [272]: media/image23.jpeg 
  [273]: media/image24.jpeg 
  [274]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor
  [275]: media/image25.jpeg 
  [276]: media/image26.jpeg 
  [277]: https://web.archive.org/web/https://stakey.club/en/decred-via-tor-network/
  [278]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation
  [279]: https://web.archive.org/web/https://tails.boum.org/contribute/design/stream_isolation/
  [280]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table
  [281]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor
  [282]: https://web.archive.org/web/https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services
  [283]: https://web.archive.org/web/https://www.whonix.org/wiki/Why_does_Whonix_use_Tor
  [284]: https://web.archive.org/web/https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study
  [285]: https://web.archive.org/web/https://gist.github.com/joepie91/5a9909939e6ce7d09e29
  [286]: https://web.archive.org/web/https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html
  [287]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN
  [288]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/
  [289]: https://web.archive.org/web/https://tails.boum.org/support/faq/index.en.html
  [290]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction
  [291]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN
  [292]: media/image27.jpeg 
  [293]: media/image28.jpeg 
  [294]: https://web.archive.org/web/https://www.virtualbox.org/wiki/Downloads
  [295]: https://web.archive.org/web/https://www.whonix.org/wiki/Download
  [296]: https://web.archive.org/web/https://www.whonix.org/wiki/Virtualization_Platform_Security
  [297]: https://web.archive.org/web/https://www.whonix.org/wiki/Network_Time_Synchronization
  [298]: https://web.archive.org/web/https://www.virtualbox.org/manual/ch13.html
  [299]: https://web.archive.org/web/https://www.whonix.org/wiki/Bridges
  [300]: https://web.archive.org/web/https://www.whonix.org/wiki/Operating_System_Software_and_Updates
  [301]: https://web.archive.org/web/https://www.whonix.org/wiki/DoNot
  [302]: https://web.archive.org/web/https://www.whonix.org/wiki/Documentation
  [303]: https://web.archive.org/web/https://www.whonix.org/wiki/Install_Software
  [304]: https://web.archive.org/web/https://www.whonix.org/wiki/Anti-Forensics_Precautions
  [305]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Fingerprinting
  [306]: https://web.archive.org/web/https://www.whonix.org/wiki/Other_Operating_Systems
  [307]: https://web.archive.org/web/https://ameliorated.info/
  [308]: https://web.archive.org/web/https://brave.com/download/
  [309]: https://web.archive.org/web/https://www.android-x86.org/documentation/virtualbox.html
  [310]: https://web.archive.org/web/https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/
  [311]: https://web.archive.org/web/https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/
  [312]: https://web.archive.org/web/https://github.com/myspaghetti/macos-virtualbox
  [https://www.whonix.org/wiki/KeePassXC]: https://www.whonix.org/wiki/Keepassxc
  [313]: https://web.archive.org/web/https://www.whonix.org/wiki/Keepassxc
  [314]: https://web.archive.org/web/https://keepassxc.org/download/
  [315]: https://web.archive.org/web/https://keepassxc.org/docs/KeePassXC_GettingStarted.html
  ...
  [316]: media/image29.jpeg 
  [317]: media/image30.jpeg 
  [318]: media/image31.jpeg 
  [319]: https://web.archive.org/web/https://www.qubes-os.org/intro/
  [320]: https://web.archive.org/web/https://www.qubes-os.org/video-tours/
  [321]: https://web.archive.org/web/https://www.qubes-os.org/doc/getting-started/
  [322]: https://yewtu.be/watch?v=8cU4hQg6GvU
  [323]: https://yewtu.be/watch?v=sbN5Bz3v-uA
  [324]: https://yewtu.be/watch?v=YPAvoFsvSbg
  [325]: https://web.archive.org/web/https://www.qubes-os.org/hcl/
  [326]: media/image32.jpeg 
  [327]: media/image33.jpeg
  [328]: https://web.archive.org/web/https://www.qubes-os.org/doc/installation-guide/
  [329]: https://web.archive.org/web/https://www.qubes-os.org/faq/
  [330]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/vpn.md
  [331]: https://web.archive.org/web/https://check.torproject.org/
  [332]: https://web.archive.org/web/https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux
  [333]: https://web.archive.org/web/https://mullvad.net/en/check/
  [334]: https://web.archive.org/web/https://www.ivpn.net/
  [335]: https://web.archive.org/web/https://protonvpn.com/support/vpn-ip-change/
  [336]: https://web.archive.org/web/https://www.whonix.org/wiki/Qubes/DisposableVM
  [337]: https://web.archive.org/web/https://brave.com/linux/
  [338]: https://web.archive.org/web/https://github.com/anbox/anbox-modules
  [339]: https://web.archive.org/web/https://github.com/anbox/anbox/blob/master/docs/install.md
  [340]: media/image34.jpeg
  [382]: https://web.archive.org/web/https://njal.la/tos/
  [383]: https://web.archive.org/web/https://protonmail.com/terms-and-conditions
  [384]: https://web.archive.org/web/https://protonvpn.com/terms-and-conditions
  [385]: https://web.archive.org/web/https://www.redditinc.com/policies
  [386]: https://web.archive.org/web/https://slashdotmedia.com/terms-of-use/
  [387]: https://web.archive.org/web/https://telegram.org/tos
  [388]: https://web.archive.org/web/mailto:recover@telegram.org
  [389]: https://web.archive.org/web/https://tutanota.com/terms/
  [390]: https://web.archive.org/web/https://www.twitch.tv/p/en/legal/terms-of-service/
  [391]: https://web.archive.org/web/https://www.whatsapp.com/legal/updates/terms-of-service-eea
  [392]: media/image39.jpeg
  [393]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/
  [394]: https://yewtu.be/watch?v=zSQtyW_ywZc
  [395]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Comparison_of_instant_messaging_protocols
  [396]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Comparison_of_cross-platform_instant_messaging_clients
  [397]: https://web.archive.org/web/https://www.securemessagingapps.com/
  [398]: https://web.archive.org/web/https://protonmail.com/blog/whatsapp-alternatives/
  [399]: https://web.archive.org/web/https://www.whonix.org/wiki/Chat
  [400]: https://web.archive.org/web/https://briarproject.org/
  [401]: https://web.archive.org/web/https://onionshare.org/
  [402]: https://web.archive.org/web/https://jami.net/
  [403]: https://web.archive.org/web/https://element.io/
  [404]: https://web.archive.org/web/https://securedrop.org/
  [405]: https://web.archive.org/web/https://github.com/alecmuffett/real-world-onion-sites
  [406]: https://web.archive.org/web/https://haveibeenpwned.com/
  [407]: https://web.archive.org/web/https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html
  [408]: https://web.archive.org/web/https://www.qubes-os.org/doc/backup-restore/
  [409]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/mac-help/mh21241/mac
  [410]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/mac-help/mh11421/11.0/mac/11.0
  [411]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/disk-utility/dskutl1010/mac
  [412]: https://web.archive.org/web/https://www.getmonero.org/community/merchants/
  [413]: https://web.archive.org/web/https://privacytools.io/providers/hosting/
  [414]: https://web.archive.org/web/https://privacytools.io/providers/cloud-storage/
  [415]: https://web.archive.org/web/https://privacytools.io/providers/paste/
  [416]: https://web.archive.org/web/https://syncthing.net/
  [417]: media/image40.jpeg
  [418]: media/image41.jpeg
  [419]: media/image42.jpeg
  [420]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trim_(computing)
  [421]: media/image43.jpeg
  [422]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive
  [423]: https://web.archive.org/web/https://www.bleachbit.org/download/linux
  [424]: https://web.archive.org/web/https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux
  [425]: https://web.archive.org/web/https://linuxhint.com/completely_wipe_hard_drive_ubuntu/
  [426]: https://web.archive.org/web/https://linoxide.com/linux-command/commands-wipe-disk-linux/
  [427]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Securely_wipe_disk
  [428]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-linux
  [Extra Tools Cleaning]: #extra-tools-cleaning
  [429]: https://web.archive.org/web/https://eraser.heidi.ie/download/
  [430]: https://web.archive.org/web/http://killdisk.com/killdisk-freeware.htm
  [431]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac
  [432]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac
  [433]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac
  [434]: media/image44.jpeg
  [435]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely
://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/
  [483]: https://mirror.anonymousplanet.org/donations.html
  [484]: https://web.archive.org/web/https://anonymousplanet.org/donations.html
  [485]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/donations.html
  [486]: media/image47.jpeg 
  [487]: media/image48.jpeg 
  [488]: media/image49.jpeg 
  [489]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)
  [490]: https://web.archive.org/web/https://www.w10privacy.de/english-home/
  [491]: https://web.archive.org/web/https://crazymax.dev/WindowsSpyBlocker/download/
  [492]: https://web.archive.org/web/https://www.microsoft.com/en-us/software-download/windows10
  [493]: https://web.archive.org/web/https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/
  [494]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing
  [495]: https://web.archive.org/web/https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase
  [496]: https://web.archive.org/web/https://tinyapps.org/docs/wipe_drives_hdparm.html
  [497]: https://web.archive.org/web/https://tinyapps.org/docs/ata_sanitize_hdparm.html
  [498]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-secure-erase.html
  [499]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-sanitize.html
  [500]: https://web.archive.org/web/https://clonezilla.org/liveusb.php
  [501]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image
  [502]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image
  [503]: https://web.archive.org/web/https://www.torproject.org/download/
  [504]: https://web.archive.org/web/https://tb-manual.torproject.org/security-settings/
  [505]: https://web.archive.org/web/https://bridges.torproject.org/
  [506]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68
  [507]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a
  [508]: https://web.archive.org/web/https://www.bleachbit.org/
  [509]: https://web.archive.org/web/https://privazer.com/
  [510]: https://web.archive.org/web/https://www.system-rescue.org/
  [511]: https://web.archive.org/web/https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148
  [512]: https://web.archive.org/web/https://github.com/PartialVolume/shredos.2020.02
  [513]: https://web.archive.org/web/https://rufus.ie/
  [514]: https://web.archive.org/web/https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information
  [515]: https://web.archive.org/web/https://www.seagate.com/support/downloads/seatools/
  [516]: https://web.archive.org/web/https://www.samsung.com/semiconductor/minisite/ssd/download/tools/
  [517]: https://web.archive.org/web/https://www.kingston.com/unitedstates/en/support/technical/ssdmanager
  [518]: https://web.archive.org/web/https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad
  [519]: https://web.archive.org/web/https://www.crucial.com/support/storage-executive
  [520]: https://web.archive.org/web/https://support.wdc.com/downloads.aspx?p=279
  [521]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd
  [522]: https://web.archive.org/web/https://www.transcend-info.com/Support/Software-10/
  [523]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt
  [524]: https://web.archive.org/web/https://www.toshiba-storage.com/downloads/
  [525]: https://web.archive.org/web/https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux
  [526]: https://web.archive.org/web/https://code.mendhak.com/securely-wipe-ssd/
  [527]: https://web.archive.org/web/https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789
  [528]: https://web.archive.org/web/https://cindori.org/trimenabler/
  [529]: https://web.archive.org/web/https://www.debian.org/CD/netinst/
  [530]: https://web.archive.org/web/https://support.lenovo.com/be/en/solutions/migr-68369
  [531]: https://web.archive.org/web/https://support.hp.com/gb-en/document/c06204100
  [532]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe
  [533]: https://web.archive.org/web/https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks
  [534]: https://web.archive.org/web/https://en.wikipedia.org/wiki/EncroChat
  [535]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sky_ECC
  [536]: https://web.archive.org/web/https://privacytools.io/providers/vpn/
  [Printing Watermarking]: #printing-watermarking
  [537]: https://web.archive.org/web/https://evolution-host.com/vps-hosting.php
  [538]: https://web.archive.org/web/https://njal.la/
  [539]: https://web.archive.org/web/https://proprivacy.com/vpn/guides/create-your-own-vpn-server
  [540]: https://web.archive.org/web/https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/
  [541]: https://web.archive.org/web/https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel
  [542]: https://web.archive.org/web/https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/
  [543]: https://web.archive.org/web/https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/
  [544]: https://web.archive.org/web/https://www.putty.org/
  [545]: media/image50.jpeg 
  [546]: https://web.archive.org/web/https://tails.boum.org/contribute/design/Unsafe_Browser/
  [547]: https://web.archive.org/web/https://archive.flossmanuals.net/bypassing-censorship/index.html
  [548]: media/image51.jpeg 
  [549]: https://web.archive.org/web/https://www.alfa.com.tw/
  [550]: https://web.archive.org/web/https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/
  [551]: https://web.archive.org/web/https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html
  [552]: https://yewtu.be/watch?v=8FV2QZ1BPnw
  [553]: https://web.archive.org/web/https://www.netally.com/products/
  [554]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-windows/
  [555]: https://web.archive.org/web/https://www.ivpn.net/apps-windows
  [556]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-windows-vpn-application/
  [557]: https://web.archive.org/web/https://mullvad.net/en/help/install-and-use-mullvad-app-macos/
  [558]: https://web.archive.org/web/https://www.ivpn.net/apps-macos/
  [559]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-mac-vpn-application/
  [560]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-linux/
  [561]: https://web.archive.org/web/https://www.ivpn.net/apps-linux/
  [562]: https://web.archive.org/web/https://protonvpn.com/support/linux-vpn-setup/
  [563]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/
  [564]: https://web.archive.org/web/https://protonvpn.com/support/what-is-kill-switch/
  [565]: https://web.archive.org/web/https://mullvad.net/en/help/using-mullvad-vpn-app/
  [566]: https://web.archive.org/web/https://protonvpn.com/blog/macos-vpn-kill-switch/
  [567]: https://web.archive.org/web/https://mullvad.net/en/help/wireguard-and-mullvad-vpn/
  [568]: https://web.archive.org/web/https://mullvad.net/en/help/linux-openvpn-installation/
  [569]: https://web.archive.org/web/https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md
  [570]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/
  [571]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/
  [572]: https://web.archive.org/web/https://ooni.org/install/
  [573]: https://web.archive.org/web/https://www.gpg4win.org/
  [574]: https://web.archive.org/web/https://gpgtools.org/
  [575]: https://web.archive.org/web/https://support.torproject.org/tbb/how-to-verify-signature/
  [576]: https://web.archive.org/web/https://tails.boum.org/install/vm-download/index.en.html
  [577]: https://web.archive.org/web/https://www.whonix.org/wiki/Verify_the_Whonix_images
  [578]: https://web.archive.org/web/https://www.clamav.net/
  [579]: https://web.archive.org/web/https://github.com/rfxn/linux-malware-detect
  [580]: https://web.archive.org/web/http://www.chkrootkit.org/
  [581]: https://web.archive.org/web/https://developers.virustotal.com/v3.0/docs/search-by-hash
  [582]: https://web.archive.org/web/https://github.com/rshipp/awesome-malware-analysis
  [583]: https://web.archive.org/web/https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy
  [584]: https://web.archive.org/web/https://blog.didierstevens.com/programs/pdf-tools/
  [585]: https://web.archive.org/web/https://github.com/QubesOS/qubes-app-linux-pdf-converter
  [586]: https://web.archive.org/web/https://github.com/firstlookmedia/pdf-redact-tools
  [587]: https://web.archive.org/web/https://github.com/firstlookmedia/dangerzone
  [588]: https://web.archive.org/web/https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf
  [589]: https://web.archive.org/web/https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf
  [590]: media/image52.jpeg 
  [591]: media/image53.jpeg 
  [592]: https://yewtu.be/watch?v=nwkiU6GG-YU
  [593]: https://web.archive.org/web/https://telegra.ph/AME-Download-Guide-09-07
  [594]: https://web.archive.org/web/https://wiki.ameliorated.info/doku.php?id=documentation_20H2
  [595]: https://web.archive.org/web/https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf
  [596]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Don%27t_be_evil
  [597]: https://yewtu.be/watch?v=6DGNZnfKYnU
  [598]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Open-source_intelligence
  [599]: https://yewtu.be/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY
  [600]: https://web.archive.org/web/https://en.wikipedia.org/wiki/4chan
  [601]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/
  [602]: https://web.archive.org/web/https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf
  [603]: https://web.archive.org/web/https://xkcd.com/538/
  [604]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Threat_model
  [605]: https://web.archive.org/web/https://www.bellingcat.com/
  [606]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Doxing
  [607]: https://yewtu.be/watch?v=muoR8Td44UE
  [608]: https://web.archive.org/web/https://www.bbc.com/news/technology-50150981
  [609]: https://web.archive.org/web/https://2019.www.torproject.org/about/torusers.html.en
  [610]: https://web.archive.org/web/https://www.whonix.org/wiki/Anonymity
  [611]: https://web.archive.org/web/https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F
  [612]: https://web.archive.org/web/https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/
  [613]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IANAL
  [614]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trust,_but_verify
  [615]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IP_address
  [616]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Data_retention
  [617]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tor_(anonymity_network)
  [618]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_private_network
  [619]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Domain_Name_System
  [620]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_blocking
  [621]: https://web.archive.org/web/https://arxiv.org/pdf/2001.08288.pdf
  [622]: https://web.archive.org/web/https://labzilla.io/blog/force-dns-pihole
  [623]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_HTTPS
  [624]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_TLS
  [625]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pi-hole
  [626]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Server_Name_Indication
  [627]: https://web.archive.org/web/https://www.usenix.org/system/files/foci19-paper_chai_0.pdf
  [628]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Content_delivery_network
  [629]: https://web.archive.org/web/https://blog.cloudflare.com/encrypted-client-hello/
  [630]: https://web.archive.org/web/https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/
  [631]: https://web.archive.org/web/https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/
  [632]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Online_Certificate_Status_Protocol
  [633]: https://web.archive.org/web/https://madaidans-insecurities.github.io/encrypted-dns.html
  [634]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OCSP_stapling
  [635]: https://web.archive.org/web/https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf
  [636]: https://web.archive.org/web/https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries
  [637]: https://web.archive.org/web/https://nymity.ch/tor-dns/
  [638]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Near-field_communication
  [639]: https://web.archive.org/web/https://shop.samsonite.com/accessories/rfid-accessories/
  [640]: https://web.archive.org/web/https://support.google.com/accounts/answer/3467281?hl=en
  [641]: https://web.archive.org/web/https://support.apple.com/en-us/HT207056
  [642]: https://web.archive.org/web/https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf
  [643]: https://web.archive.org/web/https://digi.ninja/jasager/
  [644]: https://web.archive.org/web/https://shop.hak5.org/products/wifi-pineapple
  [645]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack
  [646]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Captive_portal
  [647]: https://web.archive.org/web/https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html
  [648]: https://web.archive.org/web/https://distrinet.cs.kuleuven.be/software/tor-wf-dl/
  [649]: https://web.archive.org/web/https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/
  [650]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/
  [651]: https://web.archive.org/web/https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/
  [652]: https://web.archive.org/web/https://www.pnas.org/content/early/2020/11/24/2011893117
  [653]: https://web.archive.org/web/https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/
  [654]: https://web.archive.org/web/https://support.apple.com/en-us/HT210515
  [655]: https://web.archive.org/web/https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/
  [656]: https://web.archive.org/web/https://support.apple.com/en-us/HT204756
  [657]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Bluetooth_Low_Energy
  [658]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity
  [659]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_mobile_subscriber_identity
  [660]: https://web.archive.org/web/https://source.android.com/devices/tech/config/device-identifiers
  [661]: https://web.archive.org/web/https://policies.google.com/privacy/embedded?hl=en-US
  [662]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/
  [663]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/
  [664]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Closed-circuit_television
  [665]: https://web.archive.org/web/https://www.apple.com/legal/transparency/device-requests.html
  [666]: https://web.archive.org/web/https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/
  [667]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IMSI-catcher
  [668]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stingray_phone_tracker
  [669]: https://web.archive.org/web/https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778
  [670]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Man-in-the-middle_attack
  [671]: https://web.archive.org/web/https://shop.puri.sm/shop/librem-5/
  [672]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MAC_address
  [673]: https://web.archive.org/web/https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf
  [674]: https://web.archive.org/web/https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf
  [675]: https://web.archive.org/web/https://en.wikipedia.org/wiki/
://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/
  [798]: https://web.archive.org/web/https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto
  [799]: https://yewtu.be/watch?v=loy84K3AJ5Q
  [800]: https://web.archive.org/web/https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/
  [801]: https://web.archive.org/web/https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369
  [802]: https://web.archive.org/web/https://blog.congruentlabs.co/military-grade-encryption/
  [803]: https://web.archive.org/web/https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588
  [804]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gag_order
  [805]: https://web.archive.org/web/https://en.wikipedia.org/wiki/National_security_letter
  [806]: https://web.archive.org/web/https://www.cyberscoop.com/court-rules-encrypted-email-tutanota-monitor-messages/
  [807]: https://web.archive.org/web/https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html
  [808]: https://web.archive.org/web/https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information
  [809]: https://web.archive.org/web/https://archive.is/XNuVw
  [810]: https://web.archive.org/web/https://archive.is/ag9w4
  [811]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Lavabit
  [812]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Warrant_canary
  [813]: https://web.archive.org/web/https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/
  [814]: https://web.archive.org/web/https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432
  [815]: https://web.archive.org/web/https://en.wikipedia.org/wiki/The_Lives_of_Others
  [816]: https://web.archive.org/web/https://www.wired.com/story/air-gap-researcher-mordechai-guri/
  [817]: https://web.archive.org/web/https://www.nassiben.com/lamphone
  [818]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OONI
  [819]: https://web.archive.org/web/https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws
  [820]: https://web.archive.org/web/https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html
  [821]: https://web.archive.org/web/https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf
  [822]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tails_(operating_system)
  [823]: https://web.archive.org/web/https://www.veracrypt.fr/en/Trim%20Operation.html
  [824]: https://web.archive.org/web/https://www.coreboot.org/
  [825]: https://web.archive.org/web/https://xkcd.com/936/
  [826]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface
  [827]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Booting
  [828]: https://web.archive.org/web/https://www.wired.com/2013/12/better-data-security-nail-polish/
  [829]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_machine
  [830]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Plausible_deniability
  [831]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deniable_encryption
  [832]: https://web.archive.org/web/https://privacytools.io/operating-systems/
  [833]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BitLocker
  [834]: https://web.archive.org/web/https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop
  [835]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Evil_maid_attack
  [836]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cold_boot_attack
  [837]: https://yewtu.be/watch?v=JDaicPIgn9U
  [838]: https://web.archive.org/web/https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems
  [839]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500
  [840]: https://web.archive.org/web/https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/
  [841]: https://web.archive.org/web/https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures
  [842]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545
  [843]: https://web.archive.org/web/https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf
  [844]: https://web.archive.org/web/https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf
  [845]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gatekeeper_(macOS)
  [846]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VeraCrypt
  [847]: https://web.archive.org/web/https://ostif.org/the-veracrypt-audit-results/
  [848]: https://web.archive.org/web/https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html
  [849]: https://web.archive.org/web/https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html
  [850]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html
  [851]: https://web.archive.org/web/https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds
  [852]: https://web.archive.org/web/https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance
  [853]: https://web.archive.org/web/https://veracrypt.eu/en/docs/trim-operation/
  [854]: https://web.archive.org/web/https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf
  [855]: https://web.archive.org/web/http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html
  [856]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VirtualBox
  [857]: https://web.archive.org/web/https://www.virtualbox.org/ticket/17987
  [858]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Whonix
  [859]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html
  [860]: https://web.archive.org/web/https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf
  [861]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)
  [862]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)
  [863]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation#By_Settings
  [864]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm
  [865]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Multi-factor_authentication
  [866]: https://web.archive.org/web/https://www.whonix.org/wiki/Whonix-Gateway_Security#Warning:_Bridged_Networking
  [867]: https://web.archive.org/web/https://www.qubes-os.org/doc/system-requirements/
  [868]: https://web.archive.org/web/https://github.com/QubesOS/qubes-issues/issues/2414
  [869]: https://web.archive.org/web/https://en.wikipedia.org/wiki/CAPTCHA
  [870]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Turing_test
  [871]: https://web.archive.org/web/https://www.google.com/recaptcha/about/
  [872]: https://web.archive.org/web/https://www.hcaptcha.com/
  [873]: https://web.archive.org/web/https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service
  [874]: https://web.archive.org/web/https://nearcyan.com/you-probably-dont-need-recaptcha/
  [875]: https://web.archive.org/web/https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/
  [876]: https://web.archive.org/web/https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf
  [877]: https://web.archive.org/web/https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html
  [878]: https://web.archive.org/web/https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/
  [879]: https://web.archive.org/web/https://blog.cloudflare.com/cloudflare-supports-privacy-pass/
  [880]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Device_fingerprint
  [881]: https://web.archive.org/web/https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html
  [882]: https://web.archive.org/web/https://support.google.com/accounts/answer/10071085
  [883]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dark_pattern
  [884]: https://web.archive.org/web/https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight
  [885]: https://web.archive.org/web/https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html
  [886]: https://web.archive.org/web/https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id
  [887]: https://web.archive.org/web/https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/
  [888]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Zero_trust_security_model
  [889]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Espionage
  [890]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SIM_swap_scam
  [891]: https://web.archive.org/web/https://www.whonix.org/wiki/Tor
  [892]: https://web.archive.org/web/https://support.torproject.org/tbb/tbb-editing-torrc/
  [893]: https://web.archive.org/web/https://support.google.com/accounts/answer/114129?hl=en
  [894]: https://web.archive.org/web/https://support.google.com/google-ads/answer/7474263?hl=en
  [895]: https://web.archive.org/web/https://support.google.com/accounts/answer/40695
  [896]: https://web.archive.org/web/https://support.google.com/accounts/contact/disabled2
  [897]: https://web.archive.org/web/https://support.google.com/accounts/answer/1333913?hl=en
  [898]: https://web.archive.org/web/https://www.jumio.com/features/
  [899]: https://web.archive.org/web/https://privacytools.io/providers/email/
  ...
  [900]: https://web.archive.org/web/https://protonmail.com/support/knowledge-base/human-verification/
  [901]: https://web.archive.org/web/https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies
  [902]: https://web.archive.org/web/https://en.wikipedia.org/wiki/End-to-end_encryption
  [903]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Forward_secrecy
  [904]: https://web.archive.org/web/https://protonmail.com/blog/zero-access-encryption/
  [905]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
  [906]: https://web.archive.org/web/https://signal.org/blog/sealed-sender/
  [907]: https://web.archive.org/web/https://signal.org/blog/private-contact-discovery/
  [908]: https://web.archive.org/web/https://signal.org/blog/signal-private-group-system/
  [909]: https://web.archive.org/web/https://privacytools.io/software/file-sharing/
  [910]: https://web.archive.org/web/https://privacytools.io/software/real-time-communication/
  [911]: https://web.archive.org/web/https://www.praxisfilms.org/open-letter-from-laura-poitras/
  [912]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SecureDrop
  [913]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trusted_Platform_Module
  [914]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pastebin
  [915]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wear_leveling
  [916]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Write_amplification
  [917]: https://web.archive.org/web/https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/
  [918]: https://web.archive.org/web/https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function
  [919]: https://web.archive.org/web/https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/
  [920]: https://web.archive.org/web/https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/
  [921]: https://web.archive.org/web/https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand
  [922]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Parted_Magic
  [923]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Hdparm
  [924]: https://web.archive.org/web/https://github.com/linux-nvme/nvme-cli
  [925]: https://web.archive.org/web/https://partedmagic.com/secure-erase/
  [926]: https://web.archive.org/web/https://partedmagic.com/nvme-secure-erase/
  [927]: https://web.archive.org/web/https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php
  [928]: https://web.archive.org/web/https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html
  [929]: https://web.archive.org/web/https://www.privacytools.io/software/productivity/
  [930]: https://web.archive.org/web/https://www.whonix.org/wiki/Metadata
  [931]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/
  [932]: https://web.archive.org/web/https://disable-gatekeeper.github.io/
  [933]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/
  [934]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/
  [935]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dead_drop
  [936]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Obfuscation
  [937]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Kleptography
  [938]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Koalang
  [939]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/
  [940]: https://web.archive.org/web/https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-
  [941]: https://web.archive.org/web/https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e
  [942]: https://web.archive.org/web/https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375
  [943]: https://web.archive.org/web/https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/
  [944]: https://web.archive.org/web/https://dban.org/
  [945]: https://web.archive.org/web/https://crystalmark.info/en/software/crystaldiskinfo/
  [946]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Faraday_cage
  [947]: https://web.archive.org/web/https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf
  [948]: https://web.archive.org/web/https://arxiv.org/abs/1512.05616
  [949]: https://web.archive.org/web/https://dl.acm.org/doi/pdf/10.1145/3309074.3309076
  [950]: https://yewtu.be/watch?v=sO98kDLkh-M
  [951]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Touch_DNA
  [952]: https://web.archive.org/web/https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps
  [953]: https://web.archive.org/web/https://github.com/mhinkie/ooni-detection
  [954]: https://web.archive.org/web/https://en.wikipedia.org/wiki/File_verification
  [955]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cyclic_redundancy_check
  [956]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MD5
  [957]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_(computer_science)
  [958]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Secure_Hash_Algorithms
  [959]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SHA-2
  [960]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_resistance
  [961]: https://web.archive.org/web/https://wiki.gnupg.org/Gpg4win/CheckIntegrity
  [962]: https://web.archive.org/web/https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f
  [963]: https://web.archive.org/web/https://en.wikipedia.org/wiki/GNU_Privacy_Guard
  [964]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Public-key_cryptography
  [965]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Polymorphic_code
  [966]: https://web.archive.org/web/https://www.whonix.org/wiki/Malware_and_Firmware_Trojans
  [967]: https://web.archive.org/web/https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8
  [968]: https://web.archive.org/web/https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf
  [969]: https://web.archive.org/web/https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/
  [970]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/
  [971]: https://web.archive.org/web/https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot
  [972]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Linux_malware
  [973]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MacOS_malware
  [974]: https://web.archive.org/web/https://www.macworld.co.uk/feature/mac-viruses-list-3668354/
  [975]: https://web.archive.org/web/https://resources.jamf.com/documents/macmalware-2020.pdf
  [976]: https://web.archive.org/web/https://imagetragick.com/
  [977]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html
  [978]: https://web.archive.org/web/https://zeltser.com/analyzing-malicious-documents/
  [979]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Portable_application
  [980]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtualization
  [981]: https://web.archive.org/web/https://snowflake.torproject.org/
  [982]: https://web.archive.org/web/https://github.com/Yawning/obfs4/
  [983]: https://web.archive.org/web/https://tb-manual.torproject.org/circumvention/
