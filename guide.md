...
Version 0.9.4 (draft), May 2021 by AnonymousPlanet.
...
**Use at your own risk. Anything in here is not legal advice and you should verify compliance with your local law before use (IANAL**[^23]**). "Trust but verify"**[^24] **all the information yourself (or even better, "Never Trust, always verify"**[^326]**).**
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^158].

Same goes with your voice which can be analyzed by for various purposes as shown in the recent Spotify patent[^159].
...
At this time, there are a few steps[^167] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^168] but not all[^169].
...
-   Consider wearing special sunglasses (expensive unfortunately) called "Reflectacles" <https://www.reflectacles.com/> <sup>[[Archive.org]][113]</sup>. There was a small study showing their efficiency against IBM and Amazon facial recognition[^170].
...
Phishing[^171] is a social engineering[^172] type of attack where an adversary could try to extract information from you by pretending or impersonating something/someone else.
...
These could be simple pixel sized images[^173] hidden in your e-mails that would call a remote server to try and get your IP address.
...
A real use case of this technique was the Freedom Hosting[^175] case in 2013 where the FBI inserted malware[^176] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^177] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^178] which can then be used in combination with other data for state surveillance[^179].
...
There are readily available commercial and cheap "badUSB" [^181]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^182] in 2005.
...
Such watermarks can be inserted by various products[^188]'[^189]'[^190]'[^191] using Steganography[^192] and can resist compression[^193] and re-encoding[^194]'[^195].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^196] which could lead to de-anonymization.
...
**Do not ever print in Color, usually watermarkings are not present without color toners/cartridges**[^198]**.**
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^199]. Most crypto currencies can be tracked accurately through various methods[^200].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^209], Cellebrite Cloud[^210]) that will help an adversary analyze your cloud data with ease.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they are encrypted[^213]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And finally, here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^214].
...
And there are reasons[^215]'[^216]'[^217] for that:
...
-   You should be wary of Apps/Services using a "modified" or proprietary cryptographic method[^218].

-   By default, you should not trust any "Roll your own crypto" until it was audited, peer-reviewed, vetted, and accepted by the cryptography community[^219]'[^220].

-   There is no such thing as "military grade crypto"[^221]'[^222]'[^223].
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^224] or a national security letter[^225]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^226].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^227].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^228].

-   2014, HideMyAss user was de-anonymized and logs were provided to the FBI[^229].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^230].

Some providers have implemented the use of a Warrant Canary[^231] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^232] and Omnisec story[^233]).
...
(illustration: excellent movie I highly recommend: Das Leben der Anderen[^234])

There are many advanced techniques that can be used by skilled adversaries[^235] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> <sup>[[Archive.org]][155]</sup> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^236] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw> <sup>[[Invidious]][172]</sup>
...
Zero-Trust Security[^326] ("Never trust, always verify").
...
-   If you absolutely want system wide plausible deniability[^254]'[^237] despite the risks[^238]'[^257]:
...
-   Check some data yourself here on the Tor Project OONI[^239] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/> <sup>[[Archive.org]][203]</sup>
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^248]**.**
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^240].

So here is a list of places where you can still get them now: <https://prepaid-data-sim-card.fandom.com/wiki/Registration_Policies_Per_Country> <sup>[[Archive.org]][206]</sup>
...
-   **Recommended**: Do not require any identification (even e-mail):
    ...
    -   (UK based) <https://dtmf.io/> <sup>[[Archive.org]][207]</sup> **preferred** because they even provide an onion hidden service address for direct access through the Tor Network at <http://dtmfiovjh42uviqez6qn75igbagtiyo724hy3rdxm77dy2m5tt7lbaqd.onion/>

    -   (Iceland based) <https://crypton.sh> <sup>[[Archive.org]][208]</sup>

    -   (Ukraine based) <https://virtualsim.net/> <sup>[[Archive.org]][209]</sup>

-   Do require identification (valid e-mail):

    -   (Germany based) <https://www.sms77.io/> <sup>[[Archive.org]][210]</sup>

    -   (Russia based) <https://onlinesim.ru/> <sup>[[Archive.org]][211]</sup>

There are some other possibilities listed here <https://cryptwerk.com/companies/sms/xmr/> <sup>[[Archive.org]][212]</sup>. **Use at your own risk.**
...
Please do not buy or use gimmicky self-encrypting devices such as these: <https://syscall.eu/blog/2018/03/12/aigo_part1/> <sup>[[Archive.org]][213]</sup>

Some might be very efficient[^241] but many are gimmicky gadgets that offer no real protection[^242].
...
TAILS[^243] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
You should also read Tails Documentation, Warnings, and limitations, before going further <https://tails.boum.org/doc/about/warning/index.en.html> <sup>[[Archive.org]][214]</sup>
...
<https://tails.boum.org/install/index.en.html> <sup>[[Archive.org]][215]</sup> , pick your flavor and proceed.
...
If you're having issue accessing Tor due to censorship or other issues, you can try using Tor Bridges by following this TAILS tutorial: <https://tails.boum.org/doc/first_steps/welcome_screen/bridge_mode/index.en.html> <sup>[[Archive.org]][216]</sup> and find more information about these on Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][217]</sup>
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^244]**.**
...
-   Download the latest HiddenVM release from <https://github.com/aforensics/HiddenVM/releases> <sup>[[Archive.org]][223]</sup>

-   Download the latest Whonix XFCE release from <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][224]</sup>
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm> <sup>[[Archive.org]][225]</sup>) or System76 laptops (<https://system76.com/> <sup>[[Archive.org]][226]</sup>) while using Coreboot[^245] (where Intel IME is disabled from factory).
...
-   <https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/> <sup>[[Archive.org]][227]</sup>

-   <https://libreboot.org/docs/hardware/> <sup>[[Archive.org]][228]</sup>

-   <https://coreboot.org/status/board-status.html> <sup>[[Archive.org]][229]</sup>
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. And, because AFAIK, AMD PSP was audited and contrary to IME was not found to have any "evil" functionalities[^246]. However, if you are going for the Qubes OS Route consider Intel as they do not support AMD with their anti-evil-maid system[^247].
...
These settings can be accessed through the boot menu of your laptop. Here is a good tutorial from HP explaining all the ways to access the BIOS on various computers: <https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs> <sup>[[Archive.org]][230]</sup>
...
-   Enable BIOS/UEFI password and use a long passphrase[^248] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^249]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^250]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^251]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
-   Defcon 22, <https://www.youtube.com/watch?v=QDSlWa9xQuA> <sup>[[Invidious]][231]</sup>

-   BlackHat 2016, <https://www.youtube.com/watch?v=0fZdL3ufVOI> <sup>[[Invidious]][232]</sup>
...
Take a moment to set a firmware password according to the tutorial here: <https://support.apple.com/en-au/HT204455> <sup>[[Archive.org]][233]</sup>

You should also enable firmware password reset protection (available from Catalina) according to the documentation here: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][234]</sup>
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> <sup>[[Archive.org]][235]</sup> [^252] (with pictures).
...
This route will make extensive use of Virtual Machines[^253], they will require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to conveniently use Plausible Deniability[^254] (aka Deniable Encryption[^255]) easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^256] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It is the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^257].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^258]'[^259] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
This also means you could set-up your own advanced "plausible deniability" setup using any Host OS by storing for instance Virtual Machines on a Veracrypt hidden volume container (be careful for traces in the Host OS tho that would need to be cleaned if the host OS is persistent, see [Some additional measures against forensics][Some additional measures against forensics:] section later). There is a project for achieving this within TAILS (<https://github.com/aforensics/HiddenVM> <sup>[[Archive.org]][218]</sup>) which would make your Host OS non persistent and use plausible deniability within TAILS.

In the case of Windows, plausible deniability is also the reason you should ideally have Windows 10 Home (and not Pro). This is because Windows 10 Pro natively offers a full-disk encryption system (Bitlocker[^260]) where Windows 10 Home offers no full-disk encryption at all. We will later use a third-party open-source software for encryption that will allow full-disk encryption on Windows 10 Home. This will give you a good (plausible) excuse to use this software. While using this software on Windows 10 Pro would be suspicious.

**Note about Linux:** So, what about Linux and plausible deniability? Yes, it is kind of possible to achieve plausible deniability with Linux too[^261]. But it is complicated to set-up and IMHO requires a skill level high enough that you probably do not need this guide to help you try it.
...
Cold Boot attacks[^263] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^264], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^265] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^266] and by the maintainer of Veracrypt[^267] so I would not worry too much about those yet.
...
-   You should limit the use of Sleep stand-by and instead use Shutdown or Hibernate to prevent the encryption keys from staying in RAM when your computer goes to sleep. This is because sleep will maintain power to your memory for resuming your activity faster. Only hibernation and shutdown will actually clear the key from the memory[^268].

See also <https://www.whonix.org/wiki/Cold_Boot_Attack_Defense> <sup>[[Archive.org]][238]</sup> and <https://www.whonix.org/wiki/Protection_Against_Physical_Attacks> <sup>[[Archive.org]][239]</sup>

Here are also some interesting tools to consider for Linux users to defend against these:

-   <https://github.com/0xPoly/Centry> <sup>[[Archive.org]][240]</sup> (unfortunately unmaintained it seems so I made a fork and pull request updating for Veracrypt <https://github.com/AnonymousPlanet/Centry> <sup>[[Archive.org]][241]</sup> which should still work)

-   <https://github.com/hephaest0s/usbkill> <sup>[[Archive.org]][242]</sup> (unfortunately unmaintained as well it seems)

-   <https://github.com/Lvl4Sword/Killer> <sup>[[Archive.org]][243]</sup>

-   <https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks> <sup>[[Archive.org]][244]</sup>

-   (Qubes OS, Intel CPU only) <https://github.com/QubesOS/qubes-antievilmaid> <sup>[[Archive.org]][245]</sup>
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^269].

-   Windows Indexing keeping traces of the files present in your User folder by default[^270].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^271].

-   Many more traces in various logs, please see this convenient interesting poster for more insight: <https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download> <sup>[[Archive.org]][246]</sup>

###### MacOS:

-   Gatekeeper[^272] and XProtect keeping track of your download history in a local database and file attributes.
...
Consider also reading **<https://www.whonix.org/wiki/Full_Disk_Encryption#Encrypting_Whonix_VMs>** <sup>[[Archive.org]][247]</sup>
...
You should always remember that despite the reputation, Linux mainstream distributions (Ubuntu for instance) are not necessarily better at security than other systems such as MacOS and Windows. See this reference to understand why <https://madaidans-insecurities.github.io/linux.html> <sup>[[Archive.org]][248]</sup>.

#### Full disk encryption:

There are two possibilities here with Ubuntu:

-   (Recommended and easy) Encrypt as part of the installation process: <https://ubuntu.com/tutorials/install-ubuntu-desktop> <sup>[[Archive.org]][249]</sup>
...
-   (Tedious but possible) Encrypt after installation: <https://help.ubuntu.com/community/ManualFullSystemEncryption> <sup>[[Archive.org]][250]</sup>
...
-   If you are not sure, just make sure you did not enable any telemetry and follow this tutorial if needed <https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/> <sup>[[Archive.org]][251]</sup>
...
-   Disable Bluetooth if enabled by following this guide <https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/> <sup>[[Archive.org]][252]</sup> or issuing the following command:

    -   ```sudo systemctl disable bluetooth.service --force```

-   Disable Indexing if enabled by default (Ubuntu >19.04) by following this guide <https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html> <sup>[[Archive.org]][253]</sup> or issuing the following commands:
...
Follow this tutorial to enable Hibernate: <https://help.ubuntu.com/16.04/ubuntu-help/power-hibernate.html> <sup>[[Archive.org]][254]</sup>

After Hibernate is enabled, change the behavior so that your laptop will hibernate when you close the lid by following this tutorial for Ubuntu 20.04 <http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/> <sup>[[Archive.org]][255]</sup> and this tutorial for Ubuntu 18.04 <https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/> <sup>[[Archive.org]][256]</sup>

Unfortunately, this will not clean the key from memory directly from memory when hibernating. To avoid this at the cost of some performance, you might consider encrypting the swap file by following this tutorial: <https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap> <sup>[[Archive.org]][257]</sup>

These settings should mitigate cold boot attacks if you can hibernate fast enough.

#### Enable MAC address randomization:

-   Ubuntu, follow these steps <https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses> <sup>[[Archive.org]][258]</sup>.

-   Any other distro: you will have to find the documentation yourself but it should be quite similar to the Ubuntu tutorial.

-   Consider this tutorial which should still work: <https://josh.works/shell-script-basics-change-mac-address> <sup>[[Archive.org]][259]</sup>

#### Hardening Linux:

Please refer to this guide <https://madaidans-insecurities.github.io/guides/linux-hardening.html> <sup>[[Archive.org]][260]</sup>
...
For securing and hardening your MacOS, I recommend reading this GitHub guide which should cover many of the issues: <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][261]</sup>
...
First you should set-up a firmware password following this guide from Apple: <https://support.apple.com/en-us/HT204455> <sup>[[Archive.org]][262]</sup>

Unfortunately, some attacks are still possible and an adversary could disable this password so you should also follow this guide to prevent disabling the firmware password from anyone including Apple: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][234]</sup>
...
These are the infamous "unblockable telemetry" calls from MacOS Big Sur disclosed here: <https://sneak.berlin/20201112/your-computer-isnt-yours/> <sup>[[Archive.org]][263]</sup>
...
But you should probably document yourself on the actual issue before acting. This page is a good place to start: <https://blog.jacopo.io/en/post/apple-ocsp/> <sup>[[Archive.org]][264]</sup>
...
You should enable full disk encryption on your Mac using Filevault according to this part of the guide: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#full-disk-encryption> <sup>[[Archive.org]][261]</sup>
...
Alternatively, you could use this free piece of software: <https://technitium.com/tmac/> <sup>[[Archive.org]][265]</sup>
...
Veracrypt[^273] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^274] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
Go ahead and download and install Veracrypt from: <https://www.veracrypt.fr/en/Downloads.html> <sup>[[Archive.org]][266]</sup>

After installation, please take a moment to review the following options that will help mitigate some attacks:

-   Encrypt the memory with a Veracrypt option[^275] (settings > performance/driver options > encrypt RAM) at a cost of 5-15% performance. This setting will also disable hibernation (which does not actively clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings > Preferences > Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^276] and prevent some local data leaks.
...
By default, Windows 10 might not offer you this possibility so you should enable it by following this Microsoft tutorial: <https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation> <sup>[[Archive.org]][267]</sup>
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^277] Operations[^278]. This will severely degrade the performance/health of your SSD drive over time.

**As you can see, Route C only offers two privacy advantages over the others and it will only be of use against a soft lawful adversary. Remember <https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis>** <sup>[[Archive.org]][222]</sup>**.**
...
Read the Veracrypt Documentation <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][268]</sup> (Process of Creation of Hidden Operating System part) and <https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html> <sup>[[Archive.org]][269]</sup> (Security Requirements and Precautions Pertaining to Hidden Volumes).

This is how your system will look after this process is done:

![][270]

(Illustration from Veracrypt Documentation, <https://veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][268]</sup>)
...
Remember to read <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][268]</sup>
...
-   Use a strong passphrase[^248] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^285] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
Before going to next step, you should learn the way to mount your Outer Volume safely for writing content on it. This is also explained in this official Veracrypt Documentation <https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html> <sup>[[Archive.org]][271]</sup>
...
Take some time to read again the "Possible Explanations for Existence of Two Veracrypt Partitions on Single Drive" of the Veracrypt documentation here <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][268]</sup>
...
In this route, we will make extensive use of the free Oracle Virtualbox[^286] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the virtualization software with the best track record in terms of security and some of the reported issues[^287] have not be completely fixed to this date[^288] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM> <sup>[[Archive.org]][272]</sup>
...
Stream isolation is a mitigation technique used to prevent some correlation attacks by having different Tor Circuits for each application. Here is an illustration to show what stream isolation is:![][277]

(Illustration from Marcelo Martins, <https://stakey.club/en/decred-via-tor-network/> <sup>[[Archive.org]][278]</sup>)

VPN/Proxy over Tor falls on the right-side[^290] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
-   <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][279]</sup>

-   <https://tails.boum.org/contribute/design/stream_isolation/> <sup>[[Archive.org]][280]</sup>

-   <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][281]</sup>
...
-   <https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services#Tor_and_VPN_Services_Comparison> <sup>[[Archive.org]][283]</sup>

-   <https://www.whonix.org/wiki/Why_does_Whonix_use_Tor> <sup>[[Archive.org]][284]</sup>

-   <https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study> <sup>[[Archive.org]][285]</sup>

-   <https://gist.github.com/joepie91/5a9909939e6ce7d09e29#file-vpn-md> <sup>[[Archive.org]][286]</sup>

-   <https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html> <sup>[[Archive.org]][287]</sup>
...
-   Tor Project: <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN> <sup>[[Archive.org]][288]</sup>

-   Tails Documentation:

    -   <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/> <sup>[[Archive.org]][289]</sup>

    -   <https://tails.boum.org/support/faq/index.en.html#index20h2> <sup>[[Archive.org]][290]</sup>

-   Whonix Documentation (in this order):

    -   <https://www.whonix.org/wiki/Tunnels/Introduction> <sup>[[Archive.org]][291]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN> <sup>[[Archive.org]][292]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][282]</sup>
...
This route will use Virtualization and Whonix[^291] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^292] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
This will allow you to turn your VMs into a kind of a disposable "Live Operating Systems" (like TAILS discussed earlier). Meaning that you will be able to erase all the traces of your activities within a VM by restoring a Snapshot to an earlier state. Of course, this will not be "as good" as TAILS (where everything is stored in memory) as there might be traces of this activity left on your hard disk. Forensics studies have shown the ability to recover data from a reverted VM[^293]. Fortunately, there will be ways to remove those traces after deletion or reverting to a previous snapshot. Such techniques will be discussed in the [Some additional measures against forensics][Some additional measures against forensics:] section of this guide.

#### Download Virtualbox and Whonix utilities:

You should download a few things within the host OS.

-   The latest version of the Virtualbox installer according to your Host OS <https://www.virtualbox.org/wiki/Downloads> <sup>[[Archive.org]][295]</sup>

-   (Skip this if you cannot use Tor natively of through a VPN) The latest Whonix OVA file from <https://www.whonix.org/wiki/Download> <sup>[[Archive.org]][296]</sup> according to your preference (Linux/Windows, with a Desktop interface XFCE for simplicity or only with the text-client for advanced users)
...
For ideal security, you should follow the recommendations provided here for each Virtualbox Virtual Machine <https://www.whonix.org/wiki/Virtualization_Platform_Security#VirtualBox_Hardening> <sup>[[Archive.org]][297]</sup> :
...
Finally, also follow this recommendation to desync the clock you are your VM compared to your host OS <https://www.whonix.org/wiki/Network_Time_Synchronization#Spoof_the_Initial_Virtual_Hardware_Clock_Offset> <sup>[[Archive.org]][298]</sup>
...
Also consider applying these mitigations from VirtualBox to mitigate Spectre[^294]/Meltdown[^295] vulnerabilities by running this command from the VirtualBox Program Directory. All of these are described here: <https://www.whonix.org/wiki/Spectre_Meltdown> <sup>[[Archive.org]][76]</sup> (be aware these can impact severely the performance of your VMs but should be done for best security).

Finally consider the security advice from Virtualbox themselves here <https://www.virtualbox.org/manual/ch13.html> <sup>[[Archive.org]][299]</sup>
...
-   Import Whonix file Into Virtualbox following the instructions on <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][224]</sup>

-   Start the Whonix VMs

Remember at this stage that if you are having issues connecting to Tor due to censorship or blocking, you should consider connecting using Bridges as explained in this tutorial <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][300]</sup>.

-   Update the Whonix VMs by following the instructions on <https://www.whonix.org/wiki/Operating_System_Software_and_Updates#Updates> <sup>[[Archive.org]][301]</sup>
...
**Important Note: You should also read these very good recommendations over there <https://www.whonix.org/wiki/DoNot>** <sup>[[Archive.org]][302]</sup> **as most of those principles will also apply to this guide. You should also read their general documentation here <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][303]</sup> **which will also provide tons of advice like this guide.**
...
**It is also the only VM that will provide Stream Isolation pre-configured for most apps by default**[^296]**.**

If you want additional software on the Workstation (such as another Browser), follow their guide here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][304]</sup>

Consider running Whonix in Live Mode if for extra malware protection, See <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][305]</sup>

Do not forget to apply the VM hardening recommendations here: Virtualbox Hardening recommendations:

#### Linux (any distro):

**Be careful, any customization you make to the Non-Whonix guest VMs (keyboard layout, language, time zone, screen resolution or other) could be used to fingerprint your VMs later. See <https://www.whonix.org/wiki/VM_Fingerprinting>** <sup>[[Archive.org]][306]</sup>
...
Refer to this tutorial <https://www.whonix.org/wiki/Other_Operating_Systems> <sup>[[Archive.org]][307]</sup> for detailed instructions.
...
-   Go with Windows AME (Ameliorated) from the <https://ameliorated.info/> <sup>[[Archive.org]][308]</sup> project which is a special Windows 10 build stripped from all telemetry/advertising and update components. **Note that you will not be able to update this version with the latest security patched and will have to just re-download a new release. See [Appendix Y: Windows AME installation][Appendix Y: Windows AME download and installation]**
...
-   Download and install Brave browser from <https://brave.com/download/> <sup>[[Archive.org]][309]</sup>
...
Skip these if you used Windows AME from <https://ameliorated.info/> <sup>[[Archive.org]][308]</sup>
...
Basically, follow the tutorial here: <https://www.android-x86.org/documentation/virtualbox.html> <sup>[[Archive.org]][310]</sup>
...
-   Windows Host OS:
    ...
    -   Virtualbox Catalina Tutorial: <https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/> <sup>[[Archive.org]][311]</sup>

    -   Virtualbox Big Sur Tutorial: <https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/> <sup>[[Archive.org]][312]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^297] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^298] authentication with the authenticator feature.
...
-   Whonix: [https://www.whonix.org/wiki/KeePassXC][] <sup>[[Archive.org]][314]</sup>

-   Linux:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][315]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_linux> <sup>[[Archive.org]][316]</sup>

-   Windows:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][315]</sup>

    -   Follow the tutorial here [https://KeePassXC.org/docs/KeePassXC_GettingStarted.html#_microsoft_windows][] <sup>[[Archive.org]][316]</sup>

-   MacOS:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][315]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_macos> <sup>[[Archive.org]][316]</sup>
...
Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^299], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^300].

Stream isolation is a mitigation technique used to prevent some correlation attacks by having different Tor Circuits for each application. Here is an illustration to show what stream isolation is:![][277]

(Illustration from Marcelo Martins, <https://stakey.club/en/decred-via-tor-network/> <sup>[[Archive.org]][278]</sup>)

VPN/Proxy over Tor falls on the right-side[^301] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can somewhat reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
Note, if you're having issues accessing the Tor Network due to blocking/censorship, you could try using Tor Bridges (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][217]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][300]</sup>).
...
-   Install Qubes OS according to the installation guide:
    ...
    -   **If you want to use Tor or VPN over Tor: Check the** "**Enabling system and template updates over the Tor anonymity network using Whonix" during the last step. This will force all Qubes OS updates to go through Tor. While this will significantly reduce your update speed, it will increase your anonymity from the start.** (If you are having issues connecting to Tor due to censorship or blocking, consider using Tor Bridges as recommended earlier. Just follow the tutorial provided here: <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][300]</sup>)
...
Edit the Whonix Disposable VM template and follow instructions here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][304]</sup>
...
You will need something to store your data (logins/passwords, identities and TOTP[^303] information).

For this purpose, I strongly recommend KeePassXC because of their integrated TOTP feature. This is the ability to create entries for 2FA[^304] authentication with the authenticator feature.
...
Captcha[^305] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^306] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCaptcha service[^307]) and Cloudflare (hCaptcha[^308]). hCaptcha is used on 15% of the internet by their own metrics[^309].

They are designed to separate bots from humans but are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you will quickly encounter many captchas everywhere[^310]. Quite often when using Tor, even if you succeed in solving all the puzzles, you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor> <sup>[[Archive.org]][275]</sup>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^311]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^312].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it is "human".

-   They track your behavior before/during/after the tests to ensure you are "human"[^313].
...
You will often experience several in a row (sometimes endless[^314]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You will also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^316].
...
Browser and Device[^317] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^318]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Chromium based Browsers such as Brave Browser over Tor Browser within this VM.
...
While this might seem innocent, this is obviously an ID verification and de-anonymization method. This is just indirectly relying on third party financial KYC[^201] regulations.

This is for instance now the case on YouTube for some European Users[^319] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^320] and unfortunately sometimes required.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^321]. Unfortunately, this method is now also sometimes being used on Facebook[^322] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^323].
...
Some countries have even developed hotlines where you can report any subversive content[^324].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^325] (or alternatively "Never trust, always verify" if you are more hardcore about it and want to apply Zero-Trust Security[^326]) instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is the real term for this[^327]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience less issues with captchas and online platforms[^318] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^328] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^248]**.**
...
-   Whonix/Tails: Create/Edit a file ```/usr/local/etc/torrc.d/50_user.conf```[^329].

-   On Tor Browser: Edit the torrc file located at ```Browser/TorBrowser/Data/Tor```[^330].
...
Some platforms are by-passing this requirement all-together by requiring a valid payment method instead (see [Financial transactions:]). While this does not directly require a real-name through their ToS, this has the same results as they usually only accept mainstream (not Monero/Cash) payment methods (such as Visa/MasterCard/Maestro or PayPal) which do require a real-name legally as part of their KYC[^201] regulations. The result is the same and arguably even better than a simple real-name policy you could ignore in some countries such as Germany.
...
-   **Consider using the recommended tools on <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][303]</sup> **as well instead of the usual mainstream ones such as E-mail providers: <https://www.whonix.org/wiki/E-Mail#Anonymity_Friendly_Email_Provider_List>** <sup>[[Archive.org]][362]</sup>
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^331] will not help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
If your account gets suspended [^334] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^335] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^336].
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^337] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They are recommended by privacytools.io[^338]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^339]. If their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
This time you will need to submit an appeal using a form[^340], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it is possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You are going to have to find a different way to post there using at least seven proxies[^341] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
End-to-end Encryption[^342] (aka e2ee) is a rather simple concept. It just means only you and your destination know each-others public encryption keys and no one in between that would be eavesdropping would be able to decrypt the communication.
...
Zero-Access Encryption[^344] at rest is used when you store data at some provider (let us say your chat history or chat backups) but this history or backup is encrypted on your side and cannot be read or decrypted by the provider hosting it.
...
Such feature would have prevented important hacks such as the Cambridge Analytica scandal[^345] if it was implemented.
...
-   The Sealed Sender option[^346].

-   The Private Contact Discovery[^347].

-   The Private Group System[^348].
...
I will recommend these option (as also recommend by privacytools.io[^349]'[^350]):
...
If you must do so, you should take some steps because you cannot really trust any organization to protect your anonymity[^351]:
...
For this, I strongly recommend the use of SecureDrop[^352] (<https://securedrop.org/> <sup>[[Archive.org]][402]</sup>) which is an open-source project from the Freedom of the Press foundation.
...
**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][411]</sup>**) and it is Njalla <https://njal.la/>** <sup>[[Archive.org]][412]</sup>
...
So, the fact is that it is very unlikely[^359]'[^360] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^361]'[^362]'[^363] if they are fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^364] for 11$ and use it to erase any disk.
...
-   If your file system is APFS, you do not need to worry about Trim, it apparently happens asynchronously as the OS writes data[^370] according to their own documentation.
...
Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^371]. So, you are left with using other tools:
...
Consider also reading this documentation if you're going with Whonix <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][305]</sup> as well as their general hardening tutorial for all platforms here <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][439]</sup>
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^372] (<https://0xacab.org/jvoisin/mat2> <sup>[[Archive.org]][449]</sup>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^373] and available on TAILS by default[^374].
...
Most of the info from this section can also be found at this nice guide <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][261]</sup>
...
Lastly you can also disable Gatekeeper altogether by issuing the following command in terminal[^375]:

-   ```sudo spctl --master-disable```

Refer to this section of this guide for further information <https://github.com/drduh/macOS-Security-and-Privacy-Guide#gatekeeper-and-xprotect> <sup>[[Archive.org]][261]</sup>
...
See this section of this guide for guidance on where to find and how to delete such artifacts: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#metadata-and-artifacts> <sup>[[Archive.org]][261]</sup>
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^276]), your network connections, app crash information and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^269] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you should not do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php> <sup>[[Archive.org]][459]</sup>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^280]). Secure Deletion with Trim[^277] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^376] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^377] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^378] and Secure Communication Obfuscation[^379] through Steganography[^380] and/or Kleptography[^381] and has many names such as Koalang[^382] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^248]**.**
...
-   Read those tips here <https://www.whonix.org/wiki/DoNot> <sup>[[Archive.org]][302]</sup>
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> <sup>[[Archive.org]][478]</sup> [^383]
...
As written earlier in this guide and as noted by Privacytools.io[^384], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple of utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^386].

-   The Windows System Resource Usage Monitor (SRUM)[^387].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^388].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^389]) or System Rescue CD (<https://www.system-rescue.org/> <sup>[[Archive.org]][506]</sup>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^390] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you if it is supported or not like CrystalDiskInfo[^391] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives][Appendix J: Manufacturer tools for Wiping HDD and SSD drives:].
...
According to Apple Documentation[^382], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
-   If you really must take them with you, you could consider the use of a faraday cage[^392] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^393] for their effectiveness. If you cannot afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

    -   Warning: consider that sensor data itself can also be reliably used to track you[^394]'[^395].
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^396] and touch DNA[^397].
...
-   Do not lick the envelope or the stamps[^398] if you use them to avoid leaving DNA traces.
...
You could use <https://evolution-host.com/vps-hosting.php> <sup>[[Archive.org]][533]</sup> (who supports Whonix) which does accept Monero payments. Or pick one from the list here that do accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][410]</sup>

**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][411]</sup>**) and it is Njalla <https://njal.la/>** <sup>[[Archive.org]][412]</sup> **and I would recommend them as well.**
...
There are plenty of tutorials on how to do this like this one <https://proprivacy.com/vpn/guides/create-your-own-vpn-server> <sup>[[Archive.org]][534]</sup>
...
-   (Windows/Linux/MacOS) <https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/> <sup>[[Archive.org]][535]</sup>

-   (Windows/Linux/MacOS) <https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel> <sup>[[Archive.org]][536]</sup>

-   (Windows) <https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/> <sup>[[Archive.org]][537]</sup>

-   (Linux/MacOS) <https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/> <sup>[[Archive.org]][538]</sup>
...
-   Download and install Putty from <https://www.putty.org/> <sup>[[Archive.org]][539]</sup>
...
In that case my last resort recommendation is to connect safely **from a distance** to a Public Wi-Fi (See [Find some safe places with decent public Wi-Fi][Find some safe places with decent public Wi-Fi:]) using your laptop and TAILS "unsafe browser". See <https://tails.boum.org/contribute/design/Unsafe_Browser/> <sup>[[Archive.org]][541]</sup>.
...
Consider reading this older but still relevant guide <https://archive.flossmanuals.net/bypassing-censorship/index.html> <sup>[[Archive.org]][542]</sup>
...
**Personally, I would recommend the AWUS036 series in the Alfa brand of adapters (see <https://www.alfa.com.tw/>** <sup>[[Archive.org]][544]</sup>**).** But you could also go with some other brands if you want such as the TP-Link TL-WN722 (see <https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/> <sup>[[Archive.org]][545]</sup>).

See this post for a comparison of various adapters: <https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html> <sup>[[Archive.org]][546]</sup> (Usually those antennas are used by Penetration Testers to probe Wi-Fis from a distance and are often discussed within the scope of the Kali Linux distribution).
...
**Do not forget tho that this will only delay a motivated adversary. Your signal can be triangulated easily by a motivated adversary in a matter of minutes once they reach the physical location of the Wi-Fi you're connecting to (for instance using a device such as AirCheck <https://www.youtube.com/watch?v=8FV2QZ1BPnw>** <sup>[[Invidious]][547]</sup>**, also see their other products here <https://www.netally.com/products/#wifi_s>** <sup>[[Archive.org]][548]</sup>**). These products can easily be deployed on mobile units (in a Car for instance) and pinpoint your location in a matter of minutes.**
...
-   Whonix Tutorial (should work with any VPN provider): <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][282]</sup> (use the Linux configurations below to get the necessary configuration files)

-   Windows Tutorials:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-windows/> <sup>[[Archive.org]][549]</sup>

    -   iVPN: <https://www.ivpn.net/apps-windows> <sup>[[Archive.org]][550]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-windows-vpn-application/> <sup>[[Archive.org]][551]</sup>

-   MacOS:

    -   Mullvad: <https://mullvad.net/en/help/install-and-use-mullvad-app-macos/> <sup>[[Archive.org]][552]</sup>

    -   IVPN: <https://www.ivpn.net/apps-macos/> <sup>[[Archive.org]][553]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-mac-vpn-application/> <sup>[[Archive.org]][554]</sup>

-   Linux:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-linux/> <sup>[[Archive.org]][555]</sup>

    -   iVPN: <https://www.ivpn.net/apps-linux/> <sup>[[Archive.org]][556]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/linux-vpn-setup/> <sup>[[Archive.org]][557]</sup>
...
-   Windows:
    ...
    -   iVPN: <https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/> <sup>[[Archive.org]][558]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/what-is-kill-switch/> <sup>[[Archive.org]][559]</sup>

    -   Mullvad: <https://mullvad.net/en/help/using-mullvad-vpn-app/#killswitch> <sup>[[Archive.org]][560]</sup>
...
-   MacOS:
    ...
    -   ProtonVPN same as Windows with the client, the option should be in the provided VPN client <https://protonvpn.com/blog/macos-vpn-kill-switch/> <sup>[[Archive.org]][561]</sup>

-   Linux:

    -   Mullvad:

        -   <https://mullvad.net/en/help/wireguard-and-mullvad-vpn/> <sup>[[Archive.org]][562]</sup>

        -   <https://mullvad.net/en/help/linux-openvpn-installation/> <sup>[[Archive.org]][563]</sup>

    -   ProtonVPN: <https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md#kill-switch> <sup>[[Archive.org]][564]</sup>

    -   iVPN:

        -   <https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/> <sup>[[Archive.org]][565]</sup>

        -   <https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/> <sup>[[Archive.org]][566]</sup>

# Appendix S: Check your network for surveillance/censorship using OONI

So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^239].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^399].
...
The probe can be found here: <https://ooni.org/install/> <sup>[[Archive.org]][567]</sup> for various platforms (iOS, Android, Windows, MacOS, and Linux).

# Appendix T: Checking files for malware

## Integrity (if available):

Usually, integrity checks[^400] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^401], more recently MD5[^402] but those present several weaknesses (CRC, MD5[^403]) that makes them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^404] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it is usually recommended to use SHA[^405] based hashes and the most used is probably the SHA-2[^406] based SHA256 for verifying file integrity. SHA is much more resistant to collisions[^407] than CRC and MD5. And collisions with SHA256 or SHA512 are rare and hard to compute for an adversary.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^410] for instance) using public key cryptography[^411].
...
-   Install GPG for your OS:
    ...
    -   Windows: gpg4win (<https://www.gpg4win.org/> <sup>[[Archive.org]][568]</sup>)

    -   MacOS: GPGTools (<https://gpgtools.org/> <sup>[[Archive.org]][569]</sup>)
...
-   <https://support.torproject.org/tbb/how-to-verify-signature/> <sup>[[Archive.org]][570]</sup>

-   <https://tails.boum.org/install/vm-download/index.en.html> <sup>[[Archive.org]][571]</sup> (See Basic OpenPGP verification).

-   <https://www.whonix.org/wiki/Verify_the_Whonix_images> <sup>[[Archive.org]][572]</sup>
...
You might be asking yourself, what about Anti-Virus solutions? Well, no ... these are not perfect solutions against many modern malware and viruses using polymorphic code[^412]. But it does not mean they cannot help against less sophisticated and known attacks. It depends how to use them as AV software can become an attack vector in itself.
...
Some will just argue against them broadly like Whonix[^413] but this topic is being discussed and disputed even at Whonix[^414] by other members of their community.
...
-   Yes, there are viruses and malware for Linux[^415]'[^416]'[^417]'[^418]'[^419]

-   Yes, there are viruses and malware for MacOS[^427]'[^420][^421]'[^422][^423]
...
-   Do use Open-Source non-real time offline Anti-Virus/Anti-Malware tools as an added measure to scan some files such as:
    ...
    -   Windows/Linux/MacOS/Qubes OS: ClamAV (<https://www.clamav.net/> <sup>[[Archive.org]][573]</sup>)

    -   Linux/Qubes OS: RFXN Linux Malware Detect (<https://github.com/rfxn/linux-malware-detect> <sup>[[Archive.org]][574]</sup>)

    -   Linux/Qubes OS: Chkrootkit (<http://www.chkrootkit.org/> <sup>[[Archive.org]][575]</sup>)
...
-   You could also use online services for **non-sensitive files*** such as VirusTotal (<https://www.virustotal.com/gui>) or Hybrid-analysis (<https://hybrid-analysis.com/>).
    ...
    -   You could also just check the VirusTotal database for the hash of your file if you don't want to send it over (see <https://developers.virustotal.com/v3.0/docs/search-by-hash> <sup>[[Archive.org]][576]</sup> (See the [Integrity (if available):] section again for guidance on how to generate hashes).

    -   Other tools are also available for non-sensitive files and a convenient list is right here: <https://github.com/rshipp/awesome-malware-analysis#online-scanners-and-sandboxes> <sup>[[Archive.org]][577]</sup>

* **Please be aware that while VirusTotal might seem very practical for scanning various files, their "privacy policy" is problematic (see <https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy>** <sup>[[Archive.org]][578]</sup>**) and states:**
...
Again, regarding the PDFs of this guide and as explained in the README of my repository, you could check for anomalies using PDFID which you can download at <https://blog.didierstevens.com/programs/pdf-tools/> <sup>[[Archive.org]][579]</sup>
...
-   **Qubes OS:** Consider using <https://github.com/QubesOS/qubes-app-linux-pdf-converter> <sup>[[Archive.org]][580]</sup> which will convert your PDF into a flattened image file. This should theoretically remove any malicious code in it. Note that this will also render the PDF formatting useless (such as links, headings, bookmarks, and references).

-   **(Deprecated) Linux/Qubes OS** (or possibly MacOS through Homebrew or Windows through Cygwin): Consider not using <https://github.com/firstlookmedia/pdf-redact-tools> <sup>[[Archive.org]][581]</sup> which will also turn your PDF into a flattened image file. Again, this should theoretically remove any malicious code in it. Again, this will also render the PDF formatting useless (such as links, headings, bookmarks, and references). **Note that this tool is deprecated and relies on a library called "ImageMagick" which is known for several security issues**[^424]**. You should not use this tool even if it is recommended in some other guides.**

-   **Windows/Linux/Qubes/OS/MacOS:** Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][582]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^425]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

#### Other type of files:

Here are some various resources for this purpose where you will find what tool to use for what type:

-   For Documents/Pictures: Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][582]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^426]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> <sup>[[Archive.org]][583]</sup> (warning, many of those tools might be harder to use on Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^427] that might be of use).

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis> <sup>[[Archive.org]][577]</sup>

-   This interesting PDF detailing which tool to use for which file type <https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf> <sup>[[Archive.org]][584]</sup>
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^428]. Those apps do not require installation your system and can be run from an USB key or anywhere else.
...
So, you might ask yourself, what is Virtualization[^429]?
...
In those cases, it might be necessary to use Tor bridges to connect to the Tor network (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][217]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][300]</sup>).

Bridges are special Tor entry nodes that are not listed on the Tor public directory. Some of those are running on people running the Snowflake Browser extension[^430] while others are running on various servers around the world. Most of those bridges are running some type of obfuscation method called obfs4[^431].

Here is the definition from the Tor Browser Manual[^432]: "obfs4 makes Tor traffic look random, and also prevents censors from finding bridges by Internet scanning. obfs4 bridges are less likely to be blocked than its predecessor, obfs3 bridges".
...
As per their definition from their manual[^432]: "meek transports make it look like you are browsing a major web site instead of using Tor. meek-azure makes it look like you are using a Microsoft web site".
...
-   **This project is more known than you think: <https://www.youtube.com/watch?v=nwkiU6GG-YU>** <sup>[[Invidious]][587]</sup>
...
The official guide is here: <https://telegra.ph/AME-Download-Guide-09-07> <sup>[[Archive.org]][588]</sup>

You can also build this ISO yourself from their scripts if you do not trust their provided ISO release using the guide here: <https://wiki.ameliorated.info/doku.php?id=documentation_20H2> <sup>[[Archive.org]][589]</sup>
...
[^1]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf> <sup>[[Archive.org]][590]</sup>. Section 13, Article 6, "The service provider must enable the use of Telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^2]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany> <sup>[[Archive.org]][360]</sup>

[^3]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil> <sup>[[Archive.org]][591]</sup>

[^4]: YouTube, <https://www.youtube.com/watch?v=6DGNZnfKYnU> <sup>[[Invidious]][592]</sup>

[^5]: Wikipedia, OSINT <https://en.wikipedia.org/wiki/Open-source_intelligence> <sup>[[Archive.org]][593]</sup>

[^6]: YouTube Internet Historian Playlist, HWNDU <https://www.youtube.com/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY> <sup>[[Invidious]][594]</sup>

[^7]: Wikipedia, 4chan <https://en.wikipedia.org/wiki/4chan> <sup>[[Archive.org]][595]</sup>

[^8]: PIA, See this good article on the matter <https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/> <sup>[[Archive.org]][596]</sup> (disclaimer: this is not an endorsement or recommendation for this commercial service).
...
[^10]: This World of Ours, James Mickens <https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf> <sup>[[Archive.org]][597]</sup>

[^11]: XKCD, Security <https://xkcd.com/538/> <sup>[[Archive.org]][598]</sup>

[^12]: Wikipedia, Threat Model <https://en.wikipedia.org/wiki/Threat_model> <sup>[[Archive.org]][599]</sup>

[^13]: Bellingcat <https://www.bellingcat.com/> <sup>[[Archive.org]][600]</sup>

[^14]: Wikipedia, Doxing <https://en.wikipedia.org/wiki/Doxing> <sup>[[Archive.org]][601]</sup>

[^15]: YouTube, Internet Historian, The Bikelock Fugitive of Berkeley <https://www.youtube.com/watch?v=muoR8Td44UE> <sup>[[Invidious]][602]</sup>

[^16]: BBC News, Tor Mirror <https://www.bbc.com/news/technology-50150981> <sup>[[Archive.org]][603]</sup>

[^17]: GitHub, Real World Onion websites <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][403]</sup>

[^18]: Tor Project, Who Uses Tor <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][604]</sup>

[^19]: Whonix Documentation, The importance of Anonymity <https://www.whonix.org/wiki/Anonymity> <sup>[[Archive.org]][605]</sup>

[^20]: Geek Feminism, <https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F> <sup>[[Archive.org]][606]</sup>

[^21]: Tor Project, Tor Users <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][604]</sup>

[^22]: PrivacyHub, Internet Privacy in the Age of Surveillance <https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/> <sup>[[Archive.org]][607]</sup>

[^23]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL> <sup>[[Archive.org]][608]</sup>

[^24]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][609]</sup>

[^25]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address> <sup>[[Archive.org]][610]</sup>

[^26]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention> <sup>[[Archive.org]][611]</sup>

[^27]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)> <sup>[[Archive.org]][612]</sup>

[^28]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network> <sup>[[Archive.org]][613]</sup>

[^29]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System> <sup>[[Archive.org]][614]</sup>

[^30]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking> <sup>[[Archive.org]][615]</sup>

[^31]: CensoredPlanet <https://censoredplanet.org/> <sup>[[Archive.org]][204]</sup>

[^32]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf> <sup>[[Archive.org]][616]</sup>

[^33]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole> <sup>[[Archive.org]][617]</sup>

[^34]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS> <sup>[[Archive.org]][618]</sup>

[^35]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS> <sup>[[Archive.org]][619]</sup>

[^36]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole> <sup>[[Archive.org]][620]</sup>

[^37]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication> <sup>[[Archive.org]][621]</sup>

[^38]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Archive.org]][621]</sup>

[^39]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Archive.org]][621]</sup>

[^40]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf> <sup>[[Archive.org]][622]</sup>

[^41]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network> <sup>[[Archive.org]][623]</sup>

[^42]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/> <sup>[[Archive.org]][624]</sup>

[^43]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/> <sup>[[Archive.org]][625]</sup>

[^44]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/> <sup>[[Archive.org]][626]</sup>

[^45]: Wikipedia, OCSP <https://en.wikipedia.org/wiki/Online_Certificate_Status_Protocol> <sup>[[Archive.org]][627]</sup>

[^46]: Madaidans Insecurities, Why encrypted DNS is ineffective <https://madaidans-insecurities.github.io/encrypted-dns.html> <sup>[[Archive.org]][628]</sup>

[^47]: Wikipedia, OCSP Stapling <https://en.wikipedia.org/wiki/OCSP_stapling> <sup>[[Archive.org]][629]</sup>

[^48]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf> <sup>[[Archive.org]][630]</sup>

[^49]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries> <sup>[[Archive.org]][631]</sup>

[^50]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/> <sup>[[Archive.org]][632]</sup>

[^51]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification> <sup>[[Archive.org]][58]</sup>

[^52]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication> <sup>[[Archive.org]][633]</sup>

[^53]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/> <sup>[[Archive.org]][634]</sup>

[^54]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en> <sup>[[Archive.org]][635]</sup>

[^55]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056> <sup>[[Archive.org]][636]</sup>

[^56]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf> <sup>[[Archive.org]][637]</sup>

[^57]: Digi.Ninja, Jasager <https://digi.ninja/jasager/> <sup>[[Archive.org]][638]</sup>

[^58]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple> <sup>[[Archive.org]][639]</sup>

[^59]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack> <sup>[[Archive.org]][640]</sup>

[^60]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal> <sup>[[Archive.org]][641]</sup>

[^61]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html> <sup>[[Archive.org]][642]</sup>

[^62]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/> <sup>[[Archive.org]][643]</sup>

[^63]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/> <sup>[[Archive.org]][644]</sup>

[^64]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/> <sup>[[Archive.org]][645]</sup>

[^65]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/> <sup>[[Archive.org]][646]</sup>

[^66]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117> <sup>[[Archive.org]][647]</sup>

[^67]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/> <sup>[[Archive.org]][648]</sup>

[^68]: Apple Support <https://support.apple.com/en-us/HT210515> <sup>[[Archive.org]][649]</sup>

[^69]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/> <sup>[[Archive.org]][650]</sup>

[^70]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756> <sup>[[Archive.org]][651]</sup>

[^71]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy> <sup>[[Archive.org]][652]</sup>

[^72]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/> <sup>[[Archive.org]][648]</sup>

[^73]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity> <sup>[[Archive.org]][653]</sup>

[^74]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity> <sup>[[Archive.org]][654]</sup>

[^75]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers> <sup>[[Archive.org]][655]</sup>

[^76]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US> <sup>[[Archive.org]][656]</sup>

[^77]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law> <sup>[[Archive.org]][653]</sup>

[^78]: Bellingcat, The GRU Globetrotters: Mission London <https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/> <sup>[[Archive.org]][657]</sup>

[^79]: Bellingcat,"V" For "Vympel": FSB's Secretive Department "V" Behind Assassination Of Georgian Asylum Seeker In Germany <https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/> <sup>[[Archive.org]][658]</sup>

[^80]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television> <sup>[[Archive.org]][659]</sup>

[^81]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html> <sup>[[Archive.org]][660]</sup>

[^82]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/> <sup>[[Archive.org]][661]</sup>

[^83]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher> <sup>[[Archive.org]][662]</sup>

[^84]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker> <sup>[[Archive.org]][663]</sup>

[^85]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous 'Stingrays' Become 'Obsolete' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778> <sup>[[Archive.org]][664]</sup>

[^86]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack> <sup>[[Archive.org]][665]</sup>

[^87]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/> <sup>[[Archive.org]][666]</sup>

[^88]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address> <sup>[[Archive.org]][667]</sup>

[^89]: Acyclica Road Trend Product Sheet, <https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf> <sup>[[Archive.org]][668]</sup>

[^90]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf> <sup>[[Archive.org]][669]</sup>

[^91]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit> <sup>[[Archive.org]][670]</sup>

[^92]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine> <sup>[[Archive.org]][671]</sup>

[^93]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor> <sup>[[Archive.org]][672]</sup>

[^94]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities> <sup>[[Archive.org]][671]</sup>

[^95]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor> <sup>[[Archive.org]][671]</sup>

[^96]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME> <sup>[[Archive.org]][671]</sup>

[^97]: Libreboot, <https://libreboot.org/> <sup>[[Archive.org]][673]</sup>

[^98]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf> <sup>[[Archive.org]][674]</sup>

[^99]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy> <sup>[[Archive.org]][675]</sup>
...
[^101]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT> <sup>[[Archive.org]][676]</sup>

[^102]: ZDnet, I asked Apple for all my data. Here's what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/> <sup>[[Archive.org]][677]</sup>

[^103]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27> <sup>[[Archive.org]][678]</sup>

[^104]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/> <sup>[[Archive.org]][679]</sup>

[^105]: Bellingcat, How to Use and Interpret Data from Strava's Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/> <sup>[[Archive.org]][680]</sup>

[^106]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases> <sup>[[Archive.org]][681]</sup>

[^107]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/> <sup>[[Archive.org]][682]</sup>

[^108]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59> <sup>[[Archive.org]][683]</sup>

[^109]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/> <sup>[[Archive.org]][684]</sup>)

[^110]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault> <sup>[[Archive.org]][685]</sup>

[^111]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/> <sup>[[Archive.org]][686]</sup>

[^112]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html> <sup>[[Archive.org]][687]</sup>

[^113]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/> <sup>[[Archive.org]][688]</sup>

[^114]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant> <sup>[[Archive.org]][689]</sup>

[^115]: Vice.com, Military Unit That Conducts Drone Strikes Bought Location Data From Ordinary Apps <https://www.vice.com/en/article/y3g97x/location-data-apps-drone-strikes-iowa-national-guard> <sup>[[Archive.org]][690]</sup>

[^116]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A> <sup>[[Archive.org]][691]</sup>

[^117]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden> <sup>[[Archive.org]][692]</sup>

[^118]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)> <sup>[[Archive.org]][485]</sup>

[^119]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore> <sup>[[Archive.org]][693]</sup>

[^120]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html> <sup>[[Archive.org]][694]</sup>

[^121]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)> <sup>[[Archive.org]][695]</sup>

[^122]: Wikipedia, SORM <https://en.wikipedia.org/wiki/SORM> <sup>[[Archive.org]][696]</sup>

[^123]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)> <sup>[[Archive.org]][697]</sup>

[^124]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/> <sup>[[Archive.org]][698]</sup>

[^125]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/> <sup>[[Archive.org]][699]</sup>

[^126]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/> <sup>[[Archive.org]][700]</sup>

[^127]: Wikipedia, Stylometry <https://en.wikipedia.org/wiki/Stylometry> <sup>[[Archive.org]][701]</sup>

[^128]: Paul Moore Blog, Behavioral Profiling: The password you can't change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/> <sup>[[Archive.org]][702]</sup>

[^129]: Wikipedia, Sentiment Analysis, <https://en.wikipedia.org/wiki/Sentiment_analysis> <sup>[[Archive.org]][703]</sup>

[^130]: EFF CoverYourTracks, <https://coveryourtracks.eff.org/> <sup>[[Archive.org]][704]</sup>

[^131]: Berkeley.edu, On the Feasibility of Internet-Scale Author Identification <https://people.eecs.berkeley.edu/~dawnsong/papers/2012%20On%20the%20Feasibility%20of%20Internet-Scale%20Author%20Identification.pdf> <sup>[[Archive.org]][705]</sup>

[^132]: SecuredTouch Blog, Behavioral Biometrics 101: Behavioral Biometrics vs. Behavioral Analytics <https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics> <sup>[[Archive.org]][706]</sup>

[^133]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/> <sup>[[Archive.org]][707]</sup>

[^134]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/> <sup>[[Archive.org]][708]</sup>

[^135]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/> <sup>[[Archive.org]][709]</sup>

[^136]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/> <sup>[[Archive.org]][710]</sup>

[^137]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html> <sup>[[Archive.org]][711]</sup>

[^138]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/> <sup>[[Archive.org]][712]</sup>

[^139]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/> <sup>[[Archive.org]][713]</sup>

[^140]: Brown Institute, Open-Source Investigation, <https://brown.columbia.edu/open-source-investigation/> <sup>[[Archive.org]][714]</sup>

[^141]: NewScientist, Facebook can recognize you in photos even if you're not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/> <sup>[[Archive.org]][715]</sup>

[^142]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679> <sup>[[Archive.org]][716]</sup>

[^143]: APNews, Chinese 'gait recognition' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a> <sup>[[Archive.org]][717]</sup>

[^144]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/> <sup>[[Archive.org]][718]</sup>

[^145]: Nature.com, Facial recognition technology can expose political orientation from naturalistic facial images <https://www.nature.com/articles/s41598-020-79310-1> <sup>[[Archive.org]][719]</sup>

[^146]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html> <sup>[[Archive.org]][720]</sup>

[^147]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804> <sup>[[Archive.org]][721]</sup>

[^148]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy> <sup>[[Archive.org]][722]</sup>

[^149]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/> <sup>[[Archive.org]][723]</sup>

[^150]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/> <sup>[[Archive.org]][724]</sup>

[^151]: Anyvision <https://www.anyvision.co/> <sup>[[Archive.org]][725]</sup>

[^152]: BuzzFeed.news, Surveillance Nation <https://www.buzzfeednews.com/article/ryanmac/clearview-ai-local-police-facial-recognition> <sup>[[Archive.org]][726]</sup>

[^153]: NEC, Neoface <https://www.nec.com/en/global/solutions/biometrics/face/neofacewatch.html> <sup>[[Archive.org]][727]</sup>

[^154]: The Guardian, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology> <sup>[[Archive.org]][728]</sup>

[^155]: YouTube, The Economist, China: facial recognition and state control <https://www.youtube.com/watch?v=lH2gMNrUuEY> <sup>[[Invidious]][729]</sup>

[^156]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/> <sup>[[Archive.org]][730]</sup>

[^157]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/> <sup>[[Archive.org]][731]</sup>

[^158]: BBC, WhatsApp photo drug dealer caught by 'groundbreaking' work <https://www.bbc.com/news/uk-wales-43711477> <sup>[[Archive.org]][732]</sup>

[^159]: JUSTIA Patent, Identification of taste attributes from an audio signal <https://patents.justia.com/patent/10891948> <sup>[[Archive.org]][733]</sup>

[^160]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/> <sup>[[Archive.org]][734]</sup>

[^161]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578> <sup>[[Archive.org]][735]</sup>

[^162]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689> <sup>[[Archive.org]][736]</sup>

[^163]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake> <sup>[[Archive.org]][737]</sup>

[^164]: Econotimes, Deepfake Voice Technology: The Good. The Bad. The Future <https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278> <sup>[[Archive.org]][738]</sup>

[^165]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events> <sup>[[Archive.org]][737]</sup>

[^166]: Forbes, A Voice Deepfake Was Used To Scam A CEO Out Of $243,000 <https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/> <sup>[[Archive.org]][739]</sup>

[^167]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/> <sup>[[Archive.org]][740]</sup>

[^168]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf> <sup>[[Archive.org]][741]</sup>

[^169]: BBC, Facial recognition identifies people wearing masks <https://www.bbc.com/news/technology-55573802> <sup>[[Archive.org]][742]</sup>

[^170]: University of Wisconsin, Exploring Reflectacles As Anti-Surveillance Glasses and for Adversarial Machine Learning in Computer Vision <http://diglib.uwgb.edu/digital/api/collection/p17003coll4/id/71/download> <sup>[[Archive.org]][743]</sup>

[^171]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing> <sup>[[Archive.org]][744]</sup>

[^172]: Wikipedia, Social Engineering <https://en.wikipedia.org/wiki/Social_engineering_(security)> <sup>[[Archive.org]][745]</sup>

[^173]: BBC, Spy pixels in emails have become endemic <https://www.bbc.com/news/technology-56071437> <sup>[[Archive.org]][746]</sup>

[^174]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)> <sup>[[Archive.org]][747]</sup>

[^175]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting> <sup>[[Archive.org]][748]</sup>

[^176]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/> <sup>[[Archive.org]][749]</sup>

[^177]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach> <sup>[[Archive.org]][750]</sup>

[^178]: BBC, China social media: WeChat and the Surveillance State <https://www.bbc.com/news/blogs-china-blog-48552907> <sup>[[Archive.org]][751]</sup>

[^179]: The Intercept, Revealed: Massive Chinese Police Database <https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/> <sup>[[Archive.org]][752]</sup>

[^180]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)> <sup>[[Archive.org]][753]</sup>

[^181]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/> <sup>[[Archive.org]][754]</sup>

[^182]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet> <sup>[[Archive.org]][755]</sup>

[^183]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work> <sup>[[Archive.org]][756]</sup>

[^184]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/> <sup>[[Archive.org]][757]</sup>

[^185]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark> <sup>[[Archive.org]][758]</sup>

[^186]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark> <sup>[[Archive.org]][759]</sup>

[^187]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html> <sup>[[Archive.org]][760]</sup>

[^188]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking> <sup>[[Archive.org]][761]</sup>

[^189]: Vobile Solutions, <https://www.vobilegroup.com/solutions> <sup>[[Archive.org]][762]</sup>

[^190]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html> <sup>[[Archive.org]][763]</sup>

[^191]: Imatag, <https://www.imatag.com/> <sup>[[Archive.org]][764]</sup>

[^192]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Archive.org]][765]</sup>

[^193]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921> <sup>[[Archive.org]][766]</sup>

[^194]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140> <sup>[[Archive.org]][767]</sup>

[^195]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746> <sup>[[Archive.org]][768]</sup>

[^196]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification> <sup>[[Archive.org]][769]</sup>

[^197]: Wikipedia, Printing Steganography <https://en.wikipedia.org/wiki/Machine_Identification_Code> <sup>[[Archive.org]][770]</sup>

[^198]: MIT, SeeingYellow, <http://seeingyellow.com/> <sup>[[Archive.org]][771]</sup>

[^199]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524> <sup>[[Archive.org]][772]</sup>

[^200]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/> <sup>[[Archive.org]][773]</sup>

[^201]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer> <sup>[[Archive.org]][774]</sup>

[^202]: arXiv.org, Probing the Mystery of Cryptocurrency Theft:An Investigation into Methods for Taint Analysis <https://arxiv.org/pdf/1906.05754.pdf> <sup>[[Archive.org]][775]</sup>

[^203]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y> <sup>[[Invidious]][776]</sup>

[^204]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/> <sup>[[Archive.org]][777]</sup>

[^205]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler> <sup>[[Archive.org]][778]</sup>

[^206]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity> <sup>[[Archive.org]][779]</sup>

[^207]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007> <sup>[[Archive.org]][780]</sup>

[^208]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361> <sup>[[Archive.org]][781]</sup>

[^209]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/> <sup>[[Archive.org]][782]</sup>

[^210]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/> <sup>[[Archive.org]][783]</sup>

[^211]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics> <sup>[[Archive.org]][784]</sup>

[^212]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting> <sup>[[Archive.org]][785]</sup>

[^213]: Grayshirt, <https://www.grayshift.com/> <sup>[[Archive.org]][786]</sup>

[^214]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf> <sup>[[Archive.org]][787]</sup>

[^215]: Loup-Vaillant.fr, Rolling Your Own Crypto <https://loup-vaillant.fr/articles/rolling-your-own-crypto> <sup>[[Archive.org]][788]</sup>

[^216]: Dhole Moments, Crackpot Cryptography and Security Theater <https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/> <sup>[[Archive.org]][789]</sup>

[^217]: Vice.com, Why You Don't Roll Your Own Crypto <https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto> <sup>[[Archive.org]][790]</sup>

[^218]: YouTube, Great Crypto Failures <https://www.youtube.com/watch?v=loy84K3AJ5Q> <sup>[[Invidious]][791]</sup>

[^219]: Cryptography Dispatches, The Most Backdoor-Looking Bug I've Ever Seen <https://buttondown.email/cryptography-dispatches/archive/cryptography-dispatches-the-most-backdoor-looking/> <sup>[[Archive.org]][152]</sup>

[^220]: Citizenlab.ca, Move Fast and Roll Your Own Crypto <https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/> <sup>[[Archive.org]][792]</sup>

[^221]: Jack Poon, The myth of military grade encryption <https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369> <sup>[[Archive.org]][793]</sup>

[^222]: Congruent Labs, Stop calling it "Military-Grade Encryption" <https://blog.congruentlabs.co/military-grade-encryption/> <sup>[[Archive.org]][794]</sup>

[^223]: IronCoreLabs Blog, "Military Grade Encryption" <https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588> <sup>[[Archive.org]][795]</sup>

[^224]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order> <sup>[[Archive.org]][796]</sup>

[^225]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter> <sup>[[Archive.org]][797]</sup>

[^226]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html> <sup>[[Archive.org]][798]</sup>

[^227]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information> <sup>[[Archive.org]][799]</sup>

[^228]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1> <sup>[[Archive.org]][800]</sup>

[^229]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1> <sup>[[Archive.org]][801]</sup>

[^230]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order> <sup>[[Archive.org]][802]</sup>

[^231]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary> <sup>[[Archive.org]][803]</sup>

[^232]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/> <sup>[[Archive.org]][804]</sup>

[^233]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432> <sup>[[Archive.org]][805]</sup>

[^234]: Wikipedia, Das Leben der Anderen <https://en.wikipedia.org/wiki/The_Lives_of_Others> <sup>[[Archive.org]][806]</sup>

[^235]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/> <sup>[[Archive.org]][807]</sup>

[^236]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone> <sup>[[Archive.org]][808]</sup>

[^237]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^238]: Defuse.ca, TrueCrypt's Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm> <sup>[[Archive.org]][221]</sup>

[^239]: Wikipedia, OONI, <https://en.wikipedia.org/wiki/OONI> <sup>[[Archive.org]][809]</sup>

[^240]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][810]</sup>

[^241]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html> <sup>[[Archive.org]][811]</sup>

[^242]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf> <sup>[[Archive.org]][812]</sup>

[^243]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)> <sup>[[Archive.org]][813]</sup>

[^244]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html> <sup>[[Archive.org]][814]</sup>

[^245]: Coreboot, <https://www.coreboot.org/> <sup>[[Archive.org]][815]</sup>

[^246]: YouTube, 36C3 - Uncover, Understand, Own - Regaining Control Over Your AMD CPU <https://www.youtube.com/watch?v=bKH5nGLgi08&t=2834s> <sup>[[Invidious]][72]</sup>

[^247]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid> <sup>[[Archive.org]][245]</sup>

[^248]: XKCD, Password Strength <https://xkcd.com/936/> <sup>[[Archive.org]][816]</sup>

[^249]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported> <sup>[[Archive.org]][327]</sup>

[^250]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot> <sup>[[Archive.org]][817]</sup>

[^251]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting> <sup>[[Archive.org]][818]</sup>

[^252]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/> <sup>[[Archive.org]][819]</sup>

[^253]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine> <sup>[[Archive.org]][820]</sup>

[^254]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability> <sup>[[Archive.org]][821]</sup>

[^255]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption> <sup>[[Archive.org]][822]</sup>

[^256]: Privacytools.io, Don't use Windows 10 - It's a privacy nightmare <https://privacytools.io/operating-systems/#win10> <sup>[[Archive.org]][823]</sup>

[^257]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption> <sup>[[Archive.org]][822]</sup>

[^258]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law> <sup>[[Archive.org]][476]</sup>

[^259]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/> <sup>[[Archive.org]][477]</sup>

[^260]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker> <sup>[[Archive.org]][824]</sup>

[^261]: Alpine Linux Wiki, Setting up a laptop <https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop> <sup>[[Archive.org]][825]</sup>

[^262]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack> <sup>[[Archive.org]][826]</sup>

[^263]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack> <sup>[[Archive.org]][827]</sup>

[^264]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>) <sup>[[Invidious]][828]</sup>

[^265]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems> <sup>[[Archive.org]][829]</sup>

[^266]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500> <sup>[[Archive.org]][830]</sup>

[^267]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/> <sup>[[Archive.org]][831]</sup>

[^268]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures> <sup>[[Archive.org]][832]</sup>

[^269]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545> <sup>[[Archive.org]][833]</sup>

[^270]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf> <sup>[[Archive.org]][834]</sup>

[^271]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf> <sup>[[Archive.org]][835]</sup>

[^272]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)> <sup>[[Archive.org]][836]</sup>

[^273]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt> <sup>[[Archive.org]][837]</sup>

[^274]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/> <sup>[[Archive.org]][838]</sup>

[^275]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html> <sup>[[Archive.org]][839]</sup>

[^276]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html> <sup>[[Archive.org]][840]</sup>

[^277]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)> <sup>[[Archive.org]][417]</sup>

[^278]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html> <sup>[[Archive.org]][814]</sup>

[^279]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html> <sup>[[Archive.org]][841]</sup>

[^280]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds> <sup>[[Archive.org]][842]</sup>

[^281]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance> <sup>[[Archive.org]][843]</sup>

[^282]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/> <sup>[[Archive.org]][844]</sup>

[^283]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf> <sup>[[Archive.org]][845]</sup>

[^284]: Milan Broz's Blog, TRIM & dm-crypt ... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html> <sup>[[Archive.org]][846]</sup>

[^285]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html> <sup>[[Archive.org]][841]</sup>

[^286]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox> <sup>[[Archive.org]][847]</sup>

[^287]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987> <sup>[[Archive.org]][848]</sup>

[^288]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox> <sup>[[Archive.org]][76]</sup>

[^289]: Whonix Documentation, Stream Isolation <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][279]</sup>

[^290]: Whonix Documentation, Tunnels Comparison Table, <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][281]</sup>

[^291]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix> <sup>[[Archive.org]][849]</sup>

[^292]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html> <sup>[[Archive.org]][850]</sup>

[^293]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf> <sup>[[Archive.org]][851]</sup>

[^294]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)> <sup>[[Archive.org]][852]</sup>

[^295]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)> <sup>[[Archive.org]][853]</sup>

[^296]: Whonix Documentation, Stream Isolation, By Settings <https://www.whonix.org/wiki/Stream_Isolation#By_Settings> <sup>[[Archive.org]][854]</sup>

[^297]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Archive.org]][855]</sup>

[^298]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Archive.org]][856]</sup>

[^299]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/> <sup>[[Archive.org]][857]</sup>

[^300]: Whonix Documentation, Stream Isolation <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][279]</sup>

[^301]: Whonix Documentation, Tunnels Comparison Table, <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][281]</sup>

[^302]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk #2414 <https://github.com/QubesOS/qubes-issues/issues/2414> <sup>[[Archive.org]][858]</sup>

[^303]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Archive.org]][855]</sup>

[^304]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Archive.org]][856]</sup>

[^305]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA> <sup>[[Archive.org]][859]</sup>

[^306]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test> <sup>[[Archive.org]][860]</sup>

[^307]: Google reCaptcha <https://www.google.com/recaptcha/about/> <sup>[[Archive.org]][861]</sup>

[^308]: hCaptcha <https://www.hcaptcha.com/> <sup>[[Archive.org]][862]</sup>

[^309]: hCaptcha, hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service> <sup>[[Archive.org]][863]</sup>

[^310]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/> <sup>[[Archive.org]][864]</sup>

[^311]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/> <sup>[[Archive.org]][865]</sup>

[^312]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf> <sup>[[Archive.org]][866]</sup>

[^313]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html> <sup>[[Archive.org]][867]</sup>

[^314]: Tor Project Community, Cloudflare Captcha Monitoring <https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/> <sup>[[Archive.org]][868]</sup>

[^315]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/> <sup>[[Archive.org]][869]</sup>

[^316]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][810]</sup>

[^317]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint> <sup>[[Archive.org]][870]</sup>

[^318]: Developers Google Blog,

    Guidance to developers affected by our effort to block less secure browsers and applications <https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html> <sup>[[Archive.org]][871]</sup>

[^319]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085> <sup>[[Archive.org]][872]</sup>

[^320]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern> <sup>[[Archive.org]][873]</sup>

[^321]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight> <sup>[[Archive.org]][874]</sup>

[^322]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html> <sup>[[Archive.org]][875]</sup>

[^323]: Vice.com, PornHub Announces 'Biometric Technology' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id> <sup>[[Archive.org]][876]</sup>

[^324]: Variety, China Launches Hotline to Report Online Comments That 'Distort' History or 'Deny' Its Cultural Excellence <https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/> <sup>[[Archive.org]][877]</sup>

[^325]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Archive.org]][609]</sup>

[^326]: Wikipedia, Zero-trust Security Model <https://en.wikipedia.org/wiki/Zero_trust_security_model> <sup>[[Archive.org]][878]</sup>

[^327]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization> <sup>[[Archive.org]][879]</sup>

[^328]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam> <sup>[[Archive.org]][880]</sup>

[^329]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration> <sup>[[Archive.org]][881]</sup>

[^330]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/> <sup>[[Archive.org]][882]</sup>

[^331]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^332]: Google Help <https://support.google.com/accounts/answer/114129?hl=en> <sup>[[Archive.org]][883]</sup>

[^333]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en> <sup>[[Archive.org]][884]</sup>

[^334]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695> <sup>[[Archive.org]][885]</sup>

[^335]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2> <sup>[[Archive.org]][886]</sup>

[^336]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en> <sup>[[Archive.org]][887]</sup>

[^337]: Jumio, ID verification features <https://www.jumio.com/features/> <sup>[[Archive.org]][888]</sup>

[^338]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/> <sup>[[Archive.org]][889]</sup>

[^339]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/][] <sup>[[Archive.org]][890]</sup>

[^340]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^341]: KnowYourMeme, Good Luck, I'm Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies> <sup>[[Archive.org]][891]</sup>

[^342]: Wikipedia, end-to-end encryption, <https://en.wikipedia.org/wiki/End-to-end_encryption> <sup>[[Archive.org]][892]</sup>

[^343]: Wikipedia, Forward Secrecy, <https://en.wikipedia.org/wiki/Forward_secrecy> <sup>[[Archive.org]][893]</sup>

[^344]: Protonblog, What is zero-access encryption and why it is important for security <https://protonmail.com/blog/zero-access-encryption/> <sup>[[Archive.org]][894]</sup>

[^345]: Wikipedia, Cambridge Analytica Scandal, <https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal> <sup>[[Archive.org]][895]</sup>

[^346]: Signal Blog, Technology preview: Sealed sender for Signal <https://signal.org/blog/sealed-sender/> <sup>[[Archive.org]][896]</sup>

[^347]: Signal Blog, Private Contact Discovery, <https://signal.org/blog/private-contact-discovery/> <sup>[[Archive.org]][897]</sup>

[^348]: Signal Blog, Private Group System, <https://signal.org/blog/signal-private-group-system/> <sup>[[Archive.org]][898]</sup>

[^349]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/> <sup>[[Archive.org]][899]</sup>

[^350]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/> <sup>[[Archive.org]][900]</sup>

[^351]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/> <sup>[[Archive.org]][901]</sup>

[^352]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop> <sup>[[Archive.org]][902]</sup>

[^353]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module> <sup>[[Archive.org]][903]</sup>

[^354]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling> <sup>[[Archive.org]][904]</sup>

[^355]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM> <sup>[[Archive.org]][905]</sup>

[^356]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification> <sup>[[Archive.org]][905]</sup>

[^357]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages> <sup>[[Archive.org]][417]</sup>

[^358]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection> <sup>[[Archive.org]][905]</sup>

[^359]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/> <sup>[[Archive.org]][906]</sup>

[^360]: ResearchGate, Live forensics method for acquisition on the Solid-State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function> <sup>[[Archive.org]][907]</sup>

[^361]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/> <sup>[[Archive.org]][908]</sup>

[^362]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/> <sup>[[Archive.org]][909]</sup>

[^363]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand> <sup>[[Archive.org]][910]</sup>

[^364]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic> <sup>[[Archive.org]][911]</sup>

[^365]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm> <sup>[[Archive.org]][912]</sup>

[^366]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli> <sup>[[Archive.org]][913]</sup>

[^367]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/> <sup>[[Archive.org]][914]</sup>

[^368]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/> <sup>[[Archive.org]][915]</sup>

[^369]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php> <sup>[[Archive.org]][916]</sup>

[^370]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html> <sup>[[Archive.org]][917]</sup>

[^371]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][438]</sup>

[^372]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/> <sup>[[Archive.org]][918]</sup>

[^373]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata> <sup>[[Archive.org]][919]</sup>

[^374]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/> <sup>[[Archive.org]][920]</sup>

[^375]: GitHub, Disable Gatekeeper on macOS Big Sur (11.x) <https://disable-gatekeeper.github.io/> <sup>[[Archive.org]][921]</sup>

[^376]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/> <sup>[[Archive.org]][922]</sup>

[^377]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/> <sup>[[Archive.org]][923]</sup>

[^378]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop> <sup>[[Archive.org]][924]</sup>

[^379]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication> <sup>[[Archive.org]][925]</sup>

[^380]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Archive.org]][765]</sup>

[^381]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography> <sup>[[Archive.org]][926]</sup>

[^382]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang> <sup>[[Archive.org]][927]</sup>

[^383]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/> <sup>[[Archive.org]][928]</sup>

[^384]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/> <sup>[[Archive.org]][823]</sup>

[^385]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-> <sup>[[Archive.org]][929]</sup>

[^386]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e> <sup>[[Archive.org]][930]</sup>

[^387]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375> <sup>[[Archive.org]][931]</sup>

[^388]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/> <sup>[[Archive.org]][932]</sup>

[^389]: DBAN, <https://dban.org/> <sup>[[Archive.org]][933]</sup>

[^390]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html> <sup>[[Archive.org]][811]</sup>

[^391]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/> <sup>[[Archive.org]][934]</sup>

[^392]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage> <sup>[[Archive.org]][935]</sup>

[^393]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf> <sup>[[Archive.org]][936]</sup>

[^394]: arXiv, Deep-Spying: Spying using Smartwatch and Deep Learning <https://arxiv.org/abs/1512.05616> <sup>[[Archive.org]][937]</sup>

[^395]: Acm.org, Privacy Implications of Accelerometer Data: A Review of Possible Inferences <https://dl.acm.org/doi/pdf/10.1145/3309074.3309076> <sup>[[Archive.org]][938]</sup>

[^396]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M> <sup>[[Invidious]][939]</sup>

[^397]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA> <sup>[[Archive.org]][940]</sup>

[^398]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps> <sup>[[Archive.org]][941]</sup>

[^399]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection> <sup>[[Archive.org]][942]</sup>

[^400]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification> <sup>[[Archive.org]][943]</sup>

[^401]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check> <sup>[[Archive.org]][944]</sup>

[^402]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5> <sup>[[Archive.org]][945]</sup>

[^403]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security> <sup>[[Archive.org]][945]</sup>

[^404]: Wikipedia, Collisions <https://en.wikipedia.org/wiki/Collision_(computer_science)> <sup>[[Archive.org]][946]</sup>

[^405]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms> <sup>[[Archive.org]][947]</sup>

[^406]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2> <sup>[[Archive.org]][948]</sup>

[^407]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance> <sup>[[Archive.org]][949]</sup>

[^408]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity> <sup>[[Archive.org]][950]</sup>

[^409]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f> <sup>[[Archive.org]][951]</sup>

[^410]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard> <sup>[[Archive.org]][952]</sup>

[^411]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography> <sup>[[Archive.org]][953]</sup>

[^412]: Wikipedia, Polymorphic Code <https://en.wikipedia.org/wiki/Polymorphic_code> <sup>[[Archive.org]][954]</sup>

[^413]: Whonix Documentation, Use of AV, <https://www.whonix.org/wiki/Malware_and_Firmware_Trojans#The_Utility_of_Antivirus_Tools> <sup>[[Archive.org]][955]</sup>

[^414]: Whonix Forums, <https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8> <sup>[[Archive.org]][956]</sup>

[^415]: AV-Test Security Report 2018-2019, <https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf> <sup>[[Archive.org]][957]</sup>

[^416]: ZDNet, ESET discovers 21 new Linux malware families <https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/> <sup>[[Archive.org]][958]</sup>

[^417]: NakeSecurity, EvilGnome -- Linux malware aimed at your desktop, not your servers <https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/> <sup>[[Archive.org]][959]</sup>

[^418]: Immunify, HiddenWasp: How to detect malware hidden on Linux & IoT <https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot> <sup>[[Archive.org]][960]</sup>

[^419]: Wikipedia, Linux Malware <https://en.wikipedia.org/wiki/Linux_malware> <sup>[[Archive.org]][961]</sup>

[^420]: Wikipedia, MacOS Malware <https://en.wikipedia.org/wiki/MacOS_malware> <sup>[[Archive.org]][962]</sup>

[^421]: MacWorld, List of Mac viruses, malware and security flaws <https://www.macworld.co.uk/feature/mac-viruses-list-3668354/> <sup>[[Archive.org]][963]</sup>

[^422]: JAMF, The Mac Malware of 2020 <https://resources.jamf.com/documents/macmalware-2020.pdf> <sup>[[Archive.org]][964]</sup>

[^423]: MacOS Security and Privacy Guide, <https://github.com/drduh/macOS-Security-and-Privacy-Guide#viruses-and-malware> <sup>[[Archive.org]][261]</sup>

[^424]: ImageTragick.com, <https://imagetragick.com/> <sup>[[Archive.org]][965]</sup>

[^425]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][966]</sup>

[^426]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][966]</sup>

[^427]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/> <sup>[[Archive.org]][967]</sup>

[^428]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application> <sup>[[Archive.org]][968]</sup>

[^429]: Wikipedia, Virtualization <https://en.wikipedia.org/wiki/Virtualization> <sup>[[Archive.org]][969]</sup>

[^430]: Tor Project, Project Snowflake <https://snowflake.torproject.org/> <sup>[[Archive.org]][970]</sup>

[^431]: GitHub, Obfs4 Repository <https://github.com/Yawning/obfs4/> <sup>[[Archive.org]][971]</sup>

[^432]: Tor Browser Manual, Pluggable Transport <https://tb-manual.torproject.org/circumvention/> <sup>[[Archive.org]][972]</sup>
  ...
  [Your Network: 10]: #_Toc71031948
  [Your Face, Voice, Biometrics and Pictures: 26]: #_Toc71031967
  ...
  [85]: https://web.archive.org/web/https://www.shodan.io/
  ...
  [205]: media/image18.jpeg
  [206]: https://web.archive.org/web/https://prepaid-data-sim-card.fandom.com/wiki/Registration_Policies_Per_Country
  [207]: https://web.archive.org/web/https://dtmf.io/
  [208]: https://web.archive.org/web/https://crypton.sh/
  [209]: https://web.archive.org/web/https://virtualsim.net/
  [210]: https://web.archive.org/web/https://www.sms77.io/
  [211]: https://web.archive.org/web/https://onlinesim.ru/
  [212]: https://web.archive.org/web/https://cryptwerk.com/companies/sms/xmr/
  [213]: https://web.archive.org/web/https://syscall.eu/blog/2018/03/12/aigo_part1/
  [214]: https://web.archive.org/web/https://tails.boum.org/doc/about/warning/index.en.html
  [215]: https://web.archive.org/web/https://tails.boum.org/install/index.en.html
  [216]: https://web.archive.org/web/https://tails.boum.org/doc/first_steps/welcome_screen/bridge_mode/index.en.html
  [217]: https://web.archive.org/web/https://2019.www.torproject.org/docs/bridges
  [218]: https://web.archive.org/web/https://github.com/aforensics/HiddenVM
  [219]: media/image19.jpeg 
  [Tor over VPN]: #tor-over-vpn
  [220]: https://web.archive.org/web/https://www.whonix.org/wiki/Whonix-Host
  [221]: https://web.archive.org/web/https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm
  [222]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis
  [223]: https://web.archive.org/web/https://github.com/aforensics/HiddenVM/releases
  [224]: https://web.archive.org/web/https://www.whonix.org/wiki/VirtualBox/XFCE
  [225]: https://web.archive.org/web/https://puri.sm/
  [226]: https://web.archive.org/web/https://system76.com/
  [227]: https://web.archive.org/web/https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/
  [228]: https://web.archive.org/web/https://libreboot.org/docs/hardware/
  [229]: https://web.archive.org/web/https://coreboot.org/status/board-status.html
  [230]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs
  [231]: https://yewtu.be/watch?v=QDSlWa9xQuA
  [232]: https://yewtu.be/watch?v=0fZdL3ufVOI
  [233]: https://web.archive.org/web/https://support.apple.com/en-au/HT204455
  [234]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/security/sec28382c9ca/web
  [235]: https://web.archive.org/web/https://mullvad.net/en/help/how-tamper-protect-laptop/
  [236]: media/image20.jpeg
  [237]: media/image21.jpeg
  [238]: https://web.archive.org/web/https://www.whonix.org/wiki/Cold_Boot_Attack_Defense
  [239]: https://web.archive.org/web/https://www.whonix.org/wiki/Protection_Against_Physical_Attacks
  [240]: https://web.archive.org/web/https://github.com/0xPoly/Centry
  [241]: https://web.archive.org/web/https://github.com/AnonymousPlanet/Centry
  [242]: https://web.archive.org/web/https://github.com/hephaest0s/usbkill
  [243]: https://web.archive.org/web/https://github.com/Lvl4Sword/Killer
  [244]: https://web.archive.org/web/https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks
  [245]: https://web.archive.org/web/https://github.com/QubesOS/qubes-antievilmaid
  [246]: https://web.archive.org/web/https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download
  [247]: https://web.archive.org/web/https://www.whonix.org/wiki/Full_Disk_Encryption
  [248]: https://web.archive.org/web/https://madaidans-insecurities.github.io/linux.html
  [249]: https://web.archive.org/web/https://ubuntu.com/tutorials/install-ubuntu-desktop
  [250]: https://web.archive.org/web/https://help.ubuntu.com/community/ManualFullSystemEncryption
  [251]: https://web.archive.org/web/https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/
  [252]: https://web.archive.org/web/https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/
  [253]: https://web.archive.org/web/https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html
  [254]: https://web.archive.org/web/https://help.ubuntu.com/16.04/ubuntu-help/power-hibernate.html
  [255]: https://web.archive.org/web/http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/
  [256]: https://web.archive.org/web/https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/
  [257]: https://web.archive.org/web/https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap
  [258]: https://web.archive.org/web/https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses
  [259]: https://web.archive.org/web/https://josh.works/shell-script-basics-change-mac-address
  [260]: https://web.archive.org/web/https://madaidans-insecurities.github.io/guides/linux-hardening.html
  [261]: https://web.archive.org/web/https://github.com/drduh/macOS-Security-and-Privacy-Guide
  [262]: https://web.archive.org/web/https://support.apple.com/en-us/HT204455
  [263]: https://web.archive.org/web/https://sneak.berlin/20201112/your-computer-isnt-yours/
  [264]: https://web.archive.org/web/https://blog.jacopo.io/en/post/apple-ocsp/
  [265]: https://web.archive.org/web/https://technitium.com/tmac/
  [266]: https://web.archive.org/web/https://www.veracrypt.fr/en/Downloads.html
  [267]: https://web.archive.org/web/https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation
  [268]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html
  [269]: https://web.archive.org/web/https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html
  [270]: media/image22.jpeg
  [271]: https://web.archive.org/web/https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html
  [272]: https://web.archive.org/web/https://www.whonix.org/wiki/KVM
  [273]: media/image23.jpeg 
  [274]: media/image24.jpeg 
  [275]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor
  [276]: media/image25.jpeg 
  [277]: media/image26.jpeg 
  [278]: https://web.archive.org/web/https://stakey.club/en/decred-via-tor-network/
  [279]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation
  [280]: https://web.archive.org/web/https://tails.boum.org/contribute/design/stream_isolation/
  [281]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table
  [282]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor
  [283]: https://web.archive.org/web/https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services
  [284]: https://web.archive.org/web/https://www.whonix.org/wiki/Why_does_Whonix_use_Tor
  [285]: https://web.archive.org/web/https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study
  [286]: https://web.archive.org/web/https://gist.github.com/joepie91/5a9909939e6ce7d09e29
  [287]: https://web.archive.org/web/https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html
  [288]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN
  [289]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/
  [290]: https://web.archive.org/web/https://tails.boum.org/support/faq/index.en.html
  [291]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction
  [292]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN
  [293]: media/image27.jpeg 
  [294]: media/image28.jpeg 
  [295]: https://web.archive.org/web/https://www.virtualbox.org/wiki/Downloads
  [296]: https://web.archive.org/web/https://www.whonix.org/wiki/Download
  [297]: https://web.archive.org/web/https://www.whonix.org/wiki/Virtualization_Platform_Security
  [298]: https://web.archive.org/web/https://www.whonix.org/wiki/Network_Time_Synchronization
  [299]: https://web.archive.org/web/https://www.virtualbox.org/manual/ch13.html
  [300]: https://web.archive.org/web/https://www.whonix.org/wiki/Bridges
  [301]: https://web.archive.org/web/https://www.whonix.org/wiki/Operating_System_Software_and_Updates
  [302]: https://web.archive.org/web/https://www.whonix.org/wiki/DoNot
  [303]: https://web.archive.org/web/https://www.whonix.org/wiki/Documentation
  [304]: https://web.archive.org/web/https://www.whonix.org/wiki/Install_Software
  [305]: https://web.archive.org/web/https://www.whonix.org/wiki/Anti-Forensics_Precautions
  [306]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Fingerprinting
  [307]: https://web.archive.org/web/https://www.whonix.org/wiki/Other_Operating_Systems
  [308]: https://web.archive.org/web/https://ameliorated.info/
  [309]: https://web.archive.org/web/https://brave.com/download/
  [310]: https://web.archive.org/web/https://www.android-x86.org/documentation/virtualbox.html
  [311]: https://web.archive.org/web/https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/
  [312]: https://web.archive.org/web/https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/
  [313]: https://web.archive.org/web/https://github.com/myspaghetti/macos-virtualbox
  [https://www.whonix.org/wiki/KeePassXC]: https://www.whonix.org/wiki/Keepassxc
  [314]: https://web.archive.org/web/https://www.whonix.org/wiki/Keepassxc
  [315]: https://web.archive.org/web/https://keepassxc.org/download/
  [316]: https://web.archive.org/web/https://keepassxc.org/docs/KeePassXC_GettingStarted.html
  ...
  [342]: media/image32.jpeg
  [343]: media/image33.jpeg
  [344]: media/image34.jpeg
  ...
  [412]: https://web.archive.org/web/https://njal.la/
  ...
  [534]: https://web.archive.org/web/https://proprivacy.com/vpn/guides/create-your-own-vpn-server
  [535]: https://web.archive.org/web/https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/
  [536]: https://web.archive.org/web/https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel
  [537]: https://web.archive.org/web/https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/
  [538]: https://web.archive.org/web/https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/
  [539]: https://web.archive.org/web/https://www.putty.org/
  [540]: media/image47.jpeg 
  [541]: https://web.archive.org/web/https://tails.boum.org/contribute/design/Unsafe_Browser/
  [542]: https://web.archive.org/web/https://archive.flossmanuals.net/bypassing-censorship/index.html
  [543]: media/image48.jpeg 
  [544]: https://web.archive.org/web/https://www.alfa.com.tw/
  [545]: https://web.archive.org/web/https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/
  [546]: https://web.archive.org/web/https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html
  [547]: https://yewtu.be/watch?v=8FV2QZ1BPnw
  [548]: https://web.archive.org/web/https://www.netally.com/products/
  [549]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-windows/
  [550]: https://web.archive.org/web/https://www.ivpn.net/apps-windows
  [551]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-windows-vpn-application/
  [552]: https://web.archive.org/web/https://mullvad.net/en/help/install-and-use-mullvad-app-macos/
  [553]: https://web.archive.org/web/https://www.ivpn.net/apps-macos/
  [554]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-mac-vpn-application/
  [555]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-linux/
  [556]: https://web.archive.org/web/https://www.ivpn.net/apps-linux/
  [557]: https://web.archive.org/web/https://protonvpn.com/support/linux-vpn-setup/
  [558]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/
  [559]: https://web.archive.org/web/https://protonvpn.com/support/what-is-kill-switch/
  [560]: https://web.archive.org/web/https://mullvad.net/en/help/using-mullvad-vpn-app/
  [561]: https://web.archive.org/web/https://protonvpn.com/blog/macos-vpn-kill-switch/
  [562]: https://web.archive.org/web/https://mullvad.net/en/help/wireguard-and-mullvad-vpn/
  [563]: https://web.archive.org/web/https://mullvad.net/en/help/linux-openvpn-installation/
  [564]: https://web.archive.org/web/https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md
  [565]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/
  [566]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/
  [567]: https://web.archive.org/web/https://ooni.org/install/
  [568]: https://web.archive.org/web/https://www.gpg4win.org/
  [569]: https://web.archive.org/web/https://gpgtools.org/
  [570]: https://web.archive.org/web/https://support.torproject.org/tbb/how-to-verify-signature/
  [571]: https://web.archive.org/web/https://tails.boum.org/install/vm-download/index.en.html
  [572]: https://web.archive.org/web/https://www.whonix.org/wiki/Verify_the_Whonix_images
  [573]: https://web.archive.org/web/https://www.clamav.net/
  [574]: https://web.archive.org/web/https://github.com/rfxn/linux-malware-detect
  [575]: https://web.archive.org/web/http://www.chkrootkit.org/
  [576]: https://web.archive.org/web/https://developers.virustotal.com/v3.0/docs/search-by-hash
  [577]: https://web.archive.org/web/https://github.com/rshipp/awesome-malware-analysis
  [578]: https://web.archive.org/web/https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy
  [579]: https://web.archive.org/web/https://blog.didierstevens.com/programs/pdf-tools/
  [580]: https://web.archive.org/web/https://github.com/QubesOS/qubes-app-linux-pdf-converter
  [581]: https://web.archive.org/web/https://github.com/firstlookmedia/pdf-redact-tools
  [582]: https://web.archive.org/web/https://github.com/firstlookmedia/dangerzone
  [583]: https://web.archive.org/web/https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf
  [584]: https://web.archive.org/web/https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf
  [585]: media/image49.jpeg 
  [586]: media/image50.jpeg 
  [587]: https://yewtu.be/watch?v=nwkiU6GG-YU
  [588]: https://web.archive.org/web/https://telegra.ph/AME-Download-Guide-09-07
  [589]: https://web.archive.org/web/https://wiki.ameliorated.info/doku.php?id=documentation_20H2
  [590]: https://web.archive.org/web/https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf
  [591]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Don%27t_be_evil
  [592]: https://yewtu.be/watch?v=6DGNZnfKYnU
  [593]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Open-source_intelligence
  [594]: https://yewtu.be/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY
  [595]: https://web.archive.org/web/https://en.wikipedia.org/wiki/4chan
  [596]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/
  [597]: https://web.archive.org/web/https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf
  [598]: https://web.archive.org/web/https://xkcd.com/538/
  [599]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Threat_model
  [600]: https://web.archive.org/web/https://www.bellingcat.com/
  [601]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Doxing
  [602]: https://yewtu.be/watch?v=muoR8Td44UE
  [603]: https://web.archive.org/web/https://www.bbc.com/news/technology-50150981
  [604]: https://web.archive.org/web/https://2019.www.torproject.org/about/torusers.html.en
  [605]: https://web.archive.org/web/https://www.whonix.org/wiki/Anonymity
  [606]: https://web.archive.org/web/https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F
  [607]: https://web.archive.org/web/https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/
  [608]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IANAL
  [609]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trust,_but_verify
  [610]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IP_address
  [611]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Data_retention
  [612]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tor_(anonymity_network)
  [613]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_private_network
  [614]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Domain_Name_System
  [615]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_blocking
  [616]: https://web.archive.org/web/https://arxiv.org/pdf/2001.08288.pdf
  [617]: https://web.archive.org/web/https://labzilla.io/blog/force-dns-pihole
  [618]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_HTTPS
  [619]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_TLS
  [620]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pi-hole
  [621]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Server_Name_Indication
  [622]: https://web.archive.org/web/https://www.usenix.org/system/files/foci19-paper_chai_0.pdf
  [623]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Content_delivery_network
  [624]: https://web.archive.org/web/https://blog.cloudflare.com/encrypted-client-hello/
  [625]: https://web.archive.org/web/https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/
  [626]: https://web.archive.org/web/https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/
  [627]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Online_Certificate_Status_Protocol
  [628]: https://web.archive.org/web/https://madaidans-insecurities.github.io/encrypted-dns.html
  [629]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OCSP_stapling
  [630]: https://web.archive.org/web/https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf
  [631]: https://web.archive.org/web/https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries
  [632]: https://web.archive.org/web/https://nymity.ch/tor-dns/
  [633]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Near-field_communication
  [634]: https://web.archive.org/web/https://shop.samsonite.com/accessories/rfid-accessories/
  [635]: https://web.archive.org/web/https://support.google.com/accounts/answer/3467281?hl=en
  [636]: https://web.archive.org/web/https://support.apple.com/en-us/HT207056
  [637]: https://web.archive.org/web/https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf
  [638]: https://web.archive.org/web/https://digi.ninja/jasager/
  [639]: https://web.archive.org/web/https://shop.hak5.org/products/wifi-pineapple
  [640]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack
  [641]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Captive_portal
  [642]: https://web.archive.org/web/https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html
  [643]: https://web.archive.org/web/https://distrinet.cs.kuleuven.be/software/tor-wf-dl/
  [644]: https://web.archive.org/web/https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/
  [645]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/
  [646]: https://web.archive.org/web/https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/
  [647]: https://web.archive.org/web/https://www.pnas.org/content/early/2020/11/24/2011893117
  [648]: https://web.archive.org/web/https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/
  [649]: https://web.archive.org/web/https://support.apple.com/en-us/HT210515
  [650]: https://web.archive.org/web/https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/
  [651]: https://web.archive.org/web/https://support.apple.com/en-us/HT204756
  [652]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Bluetooth_Low_Energy
  [653]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity
  [654]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_mobile_subscriber_identity
  [655]: https://web.archive.org/web/https://source.android.com/devices/tech/config/device-identifiers
  [656]: https://web.archive.org/web/https://policies.google.com/privacy/embedded?hl=en-US
  [657]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/
  [658]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/
  [659]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Closed-circuit_television
  [660]: https://web.archive.org/web/https://www.apple.com/legal/transparency/device-requests.html
  [661]: https://web.archive.org/web/https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/
  [662]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IMSI-catcher
  [663]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stingray_phone_tracker
  [664]: https://web.archive.org/web/https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778
  [665]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Man-in-the-middle_attack
  [666]: https://web.archive.org/web/https://shop.puri.sm/shop/librem-5/
  [667]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MAC_address
  [668]: https://web.archive.org/web/https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf
  [669]: https://web.archive.org/web/https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf
  [670]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Central_processing_unit
  [671]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Intel_Management_Engine
  [672]: https://web.archive.org/web/https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor
  [673]: https://web.archive.org/web/https://libreboot.org/
  [674]: https://web.archive.org/web/https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf
  [675]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Differential_privacy
  [676]: https://web.archive.org/web/https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT
  [677]: https://web.archive.org/web/https://www.zdnet.com/article/apple-data-collection-stored-request/
  [678]: https://web.archive.org/web/https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27
  [679]: https://web.archive.org/web/https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/
  [680]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/
  [681]: https://web.archive.org/web/https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases
  [682]: https://web.archive.org/web/https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/
  [683]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59
  [684]: https://web.archive.org/web/https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/
  [685]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sensorvault
  [686]: https://web.archive.org/web/https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/
  [687]: https://web.archive.org/web/https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html
  [688]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/
  [689]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Geo-fence_warrant
  [690]: https://web.archive.org/web/https://www.vice.com/en/article/y3g97x/location-data-apps-drone-strikes-iowa-national-guard
  [691]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Room_641A
  [692]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Edward_Snowden
  [693]: https://web.archive.org/web/https://en.wikipedia.org/wiki/XKeyscore
  [694]: https://web.archive.org/web/https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html
  [695]: https://web.archive.org/web/https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)
  [696]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SORM
  [697]: https://web.archive.org/web/https://en.wikipedia.org/wiki/PRISM_(surveillance_program)
  [698]: https://web.archive.org/web/https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/
  [699]: https://web.archive.org/web/https://www.imdb.com/title/tt11464826/
  [700]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/
  [701]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stylometry
  [702]: https://web.archive.org/web/https://paul.reviews/behavioral-profiling-the-password-you-cant-change/
  [703]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sentiment_analysis
  [704]: https://web.archive.org/web/https://coveryourtracks.eff.org/
  [705]: https://web.archive.org/web/https://people.eecs.berkeley.edu/~dawnsong/papers/2012%20On%20the%20Feasibility%20of%20Internet-Scale%20Author%20Identification.pdf
  [706]: https://web.archive.org/web/https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics
  [707]: https://web.archive.org/web/https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/
  [708]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/
  [709]: https://web.archive.org/web/https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/
  [710]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/
  [711]: https://web.archive.org/web/https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html
  [712]: https://web.archive.org/web/https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/
  [713]: https://web.archive.org/web/https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/
  [714]: https://web.archive.org/web/https://brown.columbia.edu/open-source-investigation/
  [715]: https://web.archive.org/web/https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/
  [716]: https://web.archive.org/web/https://patents.google.com/patent/US20150242679
  [717]: https://web.archive.org/web/https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a
  [718]: https://web.archive.org/web/https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/
  [719]: https://web.archive.org/web/https://www.nature.com/articles/s41598-020-79310-1
  [720]: https://web.archive.org/web/https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html
  [721]: https://web.archive.org/web/https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804
  [722]: https://web.archive.org/web/https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy
  [723]: https://web.archive.org/web/https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/
  [724]: https://web.archive.org/web/https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/
  [725]: https://web.archive.org/web/https://www.anyvision.co/
  [726]: https://web.archive.org/web/https://www.buzzfeednews.com/article/ryanmac/clearview-ai-local-police-facial-recognition
  [727]: https://web.archive.org/web/https://www.nec.com/en/global/solutions/biometrics/face/neofacewatch.html
  [728]: https://web.archive.org/web/https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology
  [729]: https://yewtu.be/watch?v=lH2gMNrUuEY
  [730]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/
  [731]: https://web.archive.org/web/https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/
  [732]: https://web.archive.org/web/https://www.bbc.com/news/uk-wales-43711477
  [733]: https://web.archive.org/web/https://patents.justia.com/patent/10891948
  [734]: https://web.archive.org/web/https://www.imdb.com/title/tt0119177/
  [735]: https://web.archive.org/web/https://www.imdb.com/title/tt1839578
  [736]: https://web.archive.org/web/https://www.imdb.com/title/tt0181689
  [737]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deepfake
  [738]: https://web.archive.org/web/https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278
  [739]: https://web.archive.org/web/https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/
  [740]: https://web.archive.org/web/https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/
  [741]: https://web.archive.org/web/https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf
  [742]: https://web.archive.org/web/https://www.bbc.com/news/technology-55573802
  [743]: https://web.archive.org/web/http://diglib.uwgb.edu/digital/api/collection/p17003coll4/id/71/download
  [744]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Phishing
  [745]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Social_engineering_(security)
  [746]: https://web.archive.org/web/https://www.bbc.com/news/technology-56071437
  [747]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Exploit_(computer_security)
  [748]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Freedom_Hosting
  [749]: https://web.archive.org/web/https://www.wired.com/2013/09/freedom-hosting-fbi/
  [750]: https://web.archive.org/web/https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach
  [751]: https://web.archive.org/web/https://www.bbc.com/news/blogs-china-blog-48552907
  [752]: https://web.archive.org/web/https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/
  [753]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sandbox_(computer_security)
  [754]: https://web.archive.org/web/https://www.wired.com/2014/07/usb-security/
  [755]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stuxnet
  [756]: https://web.archive.org/web/https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work
  [757]: https://web.archive.org/web/https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/
  [758]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark
  [759]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark
  [760]: https://web.archive.org/web/https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html
  [761]: https://web.archive.org/web/https://dtv.nagra.com/nexguard-forensic-watermarking
  [762]: https://web.archive.org/web/https://www.vobilegroup.com/solutions
  [763]: https://web.archive.org/web/https://www.cinavia.com/languages/english/pages/technology.html
  [764]: https://web.archive.org/web/https://www.imatag.com/
  [765]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Steganography
  [766]: https://web.archive.org/web/https://ieeexplore.ieee.org/document/4428921
  [767]: https://web.archive.org/web/https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140
  [768]: https://web.archive.org/web/https://ieeexplore.ieee.org/abstract/document/1188746
  [769]: https://web.archive.org/web/https://scholar.google.com/scholar?q=source+camera+identification
  [770]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Machine_Identification_Code
  [771]: https://web.archive.org/web/http://seeingyellow.com/
  [772]: https://web.archive.org/web/https://arxiv.org/abs/1107.4524
  [773]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/
  [774]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Know_your_customer
  [775]: https://web.archive.org/web/https://arxiv.org/pdf/1906.05754.pdf
  [776]: https://yewtu.be/playlist?list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y
  [777]: https://web.archive.org/web/https://monero.org/monero-vs-princeton-researchers/
  [778]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cryptocurrency_tumbler
  [779]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Security_through_obscurity
  [780]: https://web.archive.org/web/https://arxiv.org/abs/2009.14007
  [781]: https://web.archive.org/web/https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361
  [782]: https://web.archive.org/web/https://www.magnetforensics.com/products/magnet-axiom/cloud/
  [783]: https://web.archive.org/web/https://www.cellebrite.com/en/ufed-cloud/
  [784]: https://web.archive.org/web/https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms
  [785]: https://web.archive.org/web/https://wiki.mozilla.org/Fingerprinting
  [786]: https://web.archive.org/web/https://www.grayshift.com/
  [787]: https://web.archive.org/web/https://securephones.io/main.pdf
  [788]: https://web.archive.org/web/https://loup-vaillant.fr/articles/rolling-your-own-crypto
  [789]: https://web.archive.org/web/https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/
  [790]: https://web.archive.org/web/https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto
  [791]: https://yewtu.be/watch?v=loy84K3AJ5Q
  [792]: https://web.archive.org/web/https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/
  [793]: https://web.archive.org/web/https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369
  [794]: https://web.archive.org/web/https://blog.congruentlabs.co/military-grade-encryption/
  [795]: https://web.archive.org/web/https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588
  [796]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gag_order
  [797]: https://web.archive.org/web/https://en.wikipedia.org/wiki/National_security_letter
  [798]: https://web.archive.org/web/https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html
  [799]: https://web.archive.org/web/https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information
  [800]: https://web.archive.org/web/https://archive.is/XNuVw
  [801]: https://web.archive.org/web/https://archive.is/ag9w4
  [802]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Lavabit
  [803]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Warrant_canary
  [804]: https://web.archive.org/web/https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/
  [805]: https://web.archive.org/web/https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432
  [806]: https://web.archive.org/web/https://en.wikipedia.org/wiki/The_Lives_of_Others
  [807]: https://web.archive.org/web/https://www.wired.com/story/air-gap-researcher-mordechai-guri/
  [808]: https://web.archive.org/web/https://www.nassiben.com/lamphone
  [809]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OONI
  [810]: https://web.archive.org/web/https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws
  [811]: https://web.archive.org/web/https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html
  [812]: https://web.archive.org/web/https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf
  [813]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tails_(operating_system)
  [814]: https://web.archive.org/web/https://www.veracrypt.fr/en/Trim%20Operation.html
  [815]: https://web.archive.org/web/https://www.coreboot.org/
  [816]: https://web.archive.org/web/https://xkcd.com/936/
  [817]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface
  [818]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Booting
  [819]: https://web.archive.org/web/https://www.wired.com/2013/12/better-data-security-nail-polish/
  [820]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_machine
  [821]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Plausible_deniability
  [822]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deniable_encryption
  [823]: https://web.archive.org/web/https://privacytools.io/operating-systems/
  [824]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BitLocker
  [825]: https://web.archive.org/web/https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop
  [826]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Evil_maid_attack
  [827]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cold_boot_attack
  [828]: https://yewtu.be/watch?v=JDaicPIgn9U
  [829]: https://web.archive.org/web/https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems
  [830]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500
  [831]: https://web.archive.org/web/https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/
  [832]: https://web.archive.org/web/https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures
  [833]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545
  [834]: https://web.archive.org/web/https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf
  [835]: https://web.archive.org/web/https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf
  [836]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gatekeeper_(macOS)
  [837]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VeraCrypt
  [838]: https://web.archive.org/web/https://ostif.org/the-veracrypt-audit-results/
  [839]: https://web.archive.org/web/https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html
  [840]: https://web.archive.org/web/https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html
  [841]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html
  [842]: https://web.archive.org/web/https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds
  [843]: https://web.archive.org/web/https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance
  [844]: https://web.archive.org/web/https://veracrypt.eu/en/docs/trim-operation/
  [845]: https://web.archive.org/web/https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf
  [846]: https://web.archive.org/web/http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html
  [847]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VirtualBox
  [848]: https://web.archive.org/web/https://www.virtualbox.org/ticket/17987
  [849]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Whonix
  [850]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html
  [851]: https://web.archive.org/web/https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf
  [852]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)
  [853]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)
  [854]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation#By_Settings
  [855]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm
  [856]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Multi-factor_authentication
  [857]: https://web.archive.org/web/https://www.qubes-os.org/doc/system-requirements/
  [858]: https://web.archive.org/web/https://github.com/QubesOS/qubes-issues/issues/2414
  [859]: https://web.archive.org/web/https://en.wikipedia.org/wiki/CAPTCHA
  [860]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Turing_test
  [861]: https://web.archive.org/web/https://www.google.com/recaptcha/about/
  [862]: https://web.archive.org/web/https://www.hcaptcha.com/
  [863]: https://web.archive.org/web/https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service
  [864]: https://web.archive.org/web/https://nearcyan.com/you-probably-dont-need-recaptcha/
  [865]: https://web.archive.org/web/https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/
  [866]: https://web.archive.org/web/https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf
  [867]: https://web.archive.org/web/https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html
  [868]: https://web.archive.org/web/https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/
  [869]: https://web.archive.org/web/https://blog.cloudflare.com/cloudflare-supports-privacy-pass/
  [870]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Device_fingerprint
  [871]: https://web.archive.org/web/https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html
  [872]: https://web.archive.org/web/https://support.google.com/accounts/answer/10071085
  [873]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dark_pattern
  [874]: https://web.archive.org/web/https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight
  [875]: https://web.archive.org/web/https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html
  [876]: https://web.archive.org/web/https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id
  [877]: https://web.archive.org/web/https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/
  [878]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Zero_trust_security_model
  [879]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Espionage
  [880]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SIM_swap_scam
  [881]: https://web.archive.org/web/https://www.whonix.org/wiki/Tor
  [882]: https://web.archive.org/web/https://support.torproject.org/tbb/tbb-editing-torrc/
  [883]: https://web.archive.org/web/https://support.google.com/accounts/answer/114129?hl=en
  [884]: https://web.archive.org/web/https://support.google.com/google-ads/answer/7474263?hl=en
  [885]: https://web.archive.org/web/https://support.google.com/accounts/answer/40695
  [886]: https://web.archive.org/web/https://support.google.com/accounts/contact/disabled2
  [887]: https://web.archive.org/web/https://support.google.com/accounts/answer/1333913?hl=en
  [888]: https://web.archive.org/web/https://www.jumio.com/features/
  [889]: https://web.archive.org/web/https://privacytools.io/providers/email/
  ...
  [890]: https://web.archive.org/web/https://protonmail.com/support/knowledge-base/human-verification/
  [891]: https://web.archive.org/web/https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies
  [892]: https://web.archive.org/web/https://en.wikipedia.org/wiki/End-to-end_encryption
  [893]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Forward_secrecy
  [894]: https://web.archive.org/web/https://protonmail.com/blog/zero-access-encryption/
  [895]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
  [896]: https://web.archive.org/web/https://signal.org/blog/sealed-sender/
  [897]: https://web.archive.org/web/https://signal.org/blog/private-contact-discovery/
  [898]: https://web.archive.org/web/https://signal.org/blog/signal-private-group-system/
  [899]: https://web.archive.org/web/https://privacytools.io/software/file-sharing/
  [900]: https://web.archive.org/web/https://privacytools.io/software/real-time-communication/
  [901]: https://web.archive.org/web/https://www.praxisfilms.org/open-letter-from-laura-poitras/
  [902]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SecureDrop
  [903]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trusted_Platform_Module
  [904]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wear_leveling
  [905]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Write_amplification
  [906]: https://web.archive.org/web/https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/
  [907]: https://web.archive.org/web/https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function
  [908]: https://web.archive.org/web/https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/
  [909]: https://web.archive.org/web/https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/
  [910]: https://web.archive.org/web/https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand
  [911]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Parted_Magic
  [912]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Hdparm
  [913]: https://web.archive.org/web/https://github.com/linux-nvme/nvme-cli
  [914]: https://web.archive.org/web/https://partedmagic.com/secure-erase/
  [915]: https://web.archive.org/web/https://partedmagic.com/nvme-secure-erase/
  [916]: https://web.archive.org/web/https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php
  [917]: https://web.archive.org/web/https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html
  [918]: https://web.archive.org/web/https://www.privacytools.io/software/productivity/
  [919]: https://web.archive.org/web/https://www.whonix.org/wiki/Metadata
  [920]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/
  [921]: https://web.archive.org/web/https://disable-gatekeeper.github.io/
  [922]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/
  [923]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/
  [924]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dead_drop
  [925]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Obfuscation
  [926]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Kleptography
  [927]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Koalang
  [928]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/
  [929]: https://web.archive.org/web/https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-
  [930]: https://web.archive.org/web/https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e
  [931]: https://web.archive.org/web/https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375
  [932]: https://web.archive.org/web/https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/
  [933]: https://web.archive.org/web/https://dban.org/
  [934]: https://web.archive.org/web/https://crystalmark.info/en/software/crystaldiskinfo/
  [935]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Faraday_cage
  [936]: https://web.archive.org/web/https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf
  [937]: https://web.archive.org/web/https://arxiv.org/abs/1512.05616
  [938]: https://web.archive.org/web/https://dl.acm.org/doi/pdf/10.1145/3309074.3309076
  [939]: https://yewtu.be/watch?v=sO98kDLkh-M
  [940]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Touch_DNA
  [941]: https://web.archive.org/web/https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps
  [942]: https://web.archive.org/web/https://github.com/mhinkie/ooni-detection
  [943]: https://web.archive.org/web/https://en.wikipedia.org/wiki/File_verification
  [944]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cyclic_redundancy_check
  [945]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MD5
  [946]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_(computer_science)
  [947]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Secure_Hash_Algorithms
  [948]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SHA-2
  [949]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_resistance
  [950]: https://web.archive.org/web/https://wiki.gnupg.org/Gpg4win/CheckIntegrity
  [951]: https://web.archive.org/web/https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f
  [952]: https://web.archive.org/web/https://en.wikipedia.org/wiki/GNU_Privacy_Guard
  [953]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Public-key_cryptography
  [954]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Polymorphic_code
  [955]: https://web.archive.org/web/https://www.whonix.org/wiki/Malware_and_Firmware_Trojans
  [956]: https://web.archive.org/web/https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8
  [957]: https://web.archive.org/web/https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf
  [958]: https://web.archive.org/web/https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/
  [959]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/
  [960]: https://web.archive.org/web/https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot
  [961]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Linux_malware
  [962]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MacOS_malware
  [963]: https://web.archive.org/web/https://www.macworld.co.uk/feature/mac-viruses-list-3668354/
  [964]: https://web.archive.org/web/https://resources.jamf.com/documents/macmalware-2020.pdf
  [965]: https://web.archive.org/web/https://imagetragick.com/
  [966]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html
  [967]: https://web.archive.org/web/https://zeltser.com/analyzing-malicious-documents/
  [968]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Portable_application
  [969]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtualization
  [970]: https://web.archive.org/web/https://snowflake.torproject.org/
  [971]: https://web.archive.org/web/https://github.com/Yawning/obfs4/
  [972]: https://web.archive.org/web/https://tb-manual.torproject.org/circumvention/
